...
function candidatePriceReviewReason(input: CandidatePurchaseDecisionInput) {
  if (input.priceEvidence === "reference") return "프로젝트 기준가만 있어 후보 실제 판매 가격을 확정할 수 없습니다.";
  if (input.priceEvidence === "recorded") return "기록 가격의 데이터 품질이 완전하지 않아 후보 실제 판매 가격을 확정할 수 없습니다.";
  if (input.priceEvidence === "unknown" || !input.priceKnown) return "후보 또는 전체 견적 가격을 확정할 수 없습니다.";
  return "후보 가격 근거를 구매 전에 다시 확인해야 합니다.";
}
...
export function candidatePurchaseDecisionFor(input: CandidatePurchaseDecisionInput): CandidatePurchaseDecision {
  ...
  if (input.remainingBlockers > 0) holdReasons.push(`가상 적용 후 차단 오류 ${input.remainingBlockers}개가 남습니다.`);
  if (input.nextStatus === "incompatible") holdReasons.push("가상 적용 후 전체 견적이 호환 불가 상태입니다.");
  if (holdReasons.length > 0) {
    return { state: "hold", label: "적용 보류", summary: `${input.name}은(는) 현재 견적에 적용하지 말고 차단 원인을 먼저 해결하세요.`, reasons: holdReasons };
  }

  const reviewReasons: string[] = [];
  if (input.candidateRisk === "review" || input.decisionStatus === "review") reviewReasons.push("후보 판정에 확인 필요가 남습니다.");
  if (input.nextStatus === "needs_review") reviewReasons.push("가상 적용 후 전체 판정이 확인 필요입니다.");
  if (input.remainingWarnings > 0) reviewReasons.push(`가상 적용 후 주의 ${input.remainingWarnings}개가 남습니다.`);
  if (input.remainingUnknown > 0) reviewReasons.push(`가상 적용 후 확인 필요 ${input.remainingUnknown}개가 남습니다.`);
  ...
  if (input.physicalStatus === "review") reviewReasons.push("물리 장착 근거를 추가로 확인해야 합니다.");
  if (input.freshness === "stale" || input.freshness === "unknown") reviewReasons.push("부품 근거의 갱신 시점을 다시 확인해야 합니다.");
  if (input.recommendationTrustLevel === "low") reviewReasons.push(`추천 근거가 낮음${input.recommendationTrustScore !== undefined ? ` ${input.recommendationTrustScore}점` : ""}입니다.`);
  if (input.similarityScore !== undefined && input.similarityScore >= 95 && input.similarityConfidence === "unknown") reviewReasons.push("높은 성능 유사도 점수의 비교 근거가 확인되지 않았습니다.");
  ...
  if (input.similarityScore !== undefined && input.similarityScore >= 95 && (input.benchmarkFreshness === "stale" || input.benchmarkFreshness === "unknown")) reviewReasons.push("높은 성능 유사도에 사용된 benchmark 자료의 갱신 상태를 다시 확인해야 합니다.");
  if (input.similarityScore !== undefined && input.similarityScore >= 95 && input.benchmarkSourceCheckNeedsReview) reviewReasons.push("높은 성능 유사도에 사용된 benchmark 원문 검증을 다시 확인해야 합니다.");
  if (input.catalogSpecSourceCheckNeedsReview) reviewReasons.push("수동 보강 스펙의 제조사 원문 URL 접근과 모델 식별을 다시 확인해야 합니다.");
  if (input.analysisScoreDelta !== undefined && Number.isFinite(input.analysisScoreDelta) && input.analysisConfidence === "unknown") reviewReasons.push("가상 적용 후 성능 분석 근거가 확인되지 않았습니다.");
  if (input.analysisScoreDelta !== undefined && Number.isFinite(input.analysisScoreDelta) && input.analysisScoreDelta < 0) reviewReasons.push(`${input.analysisConfidence === "limited" ? "일부 스펙 기준으로 " : ""}가상 적용 후 전체 성능 지수가 현재보다 ${Math.abs(Math.round(input.analysisScoreDelta))}점 낮습니다.`);
  ...
  const reasons = ["가상 적용 후 차단·주의·확인 필요 없음"];
  ...
  return { state: "buy", label: "구매 추천", summary: `${input.name}은(는) 현재 확인된 호환·가격·근거 기준에서 우선 구매 후보입니다.`, reasons };
}
