...
function candidatePriceReviewReason(input: CandidatePurchaseDecisionInput) {
  if (input.priceEvidence === "reference") return "참고 가격만 있어 부품 실제 판매 가격을 알 수 없어요.";
  if (input.priceEvidence === "recorded") return "기록 가격의 데이터 상태가 완전하지 않아 부품 실제 판매 가격을 알 수 없어요.";
  if (input.priceEvidence === "unknown" || !input.priceKnown) return "부품 또는 전체 견적 가격을 알 수 없어요.";
  return "부품 가격 출처를 구매 전에 다시 확인해야 합니다.";
}

export function candidatePurchaseDecisionFor(input: CandidatePurchaseDecisionInput): CandidatePurchaseDecision {
  const holdReasons: string[] = [];
  if (input.candidateRisk === "unsafe" || input.decisionStatus === "avoid") holdReasons.push("부품 자체에 차단 위험이 있습니다.");
  ...
  if (input.candidateRisk === "review" || input.decisionStatus === "review") reviewReasons.push("부품 결과에 확인 필요가 남습니다.");
  ...
  else if (!input.totalPriceKnown) reviewReasons.push("전체 견적 가격을 알 수 없어요.");
  ...
  if (input.similarityScore !== undefined && input.similarityScore >= 95 && input.benchmarkSourceCheckNeedsReview) reviewReasons.push("성능 비교에 사용된 벤치마크 출처를 다시 확인해야 합니다.");
  if (input.catalogSpecSourceCheckNeedsReview) reviewReasons.push("직접 입력된 스펙의 제조사 페이지 접근과 모델 식별을 다시 확인해야 합니다.");
