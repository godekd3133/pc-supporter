...
export function purchaseDecisionFor(readiness: PurchaseReadiness, checklistProgress?: PurchaseChecklistProgress, assemblyVerification?: AssemblyVerificationSurfaceSummary, purchaseProgress?: PurchaseListExecutionProgress): PurchaseDecision {
  ...
  if (assemblyVerification?.state === "failed") return { state: "review", label: "실측 확인 후 진행", summary: "실제 조립 확인에 실패 기록이 있어 원인을 확인한 뒤 진행하세요.", ...evidence };
  if (assemblyVerification && assemblyVerification.recheckSignalCount > 0) return { state: "review", label: "실측 재확인 필요", summary: `동일 조건 실측에서 다시 볼 항목 ${assemblyVerification.recheckSignalCount}개가 감지되었습니다. 원인을 확인한 뒤 진행하세요.`, ...evidence };
  if (assemblyVerification?.state === "in_progress") return { state: "review", label: "실측 기록 진행 중", summary: "실제 조립 확인이 진행 중이므로 측정 결과를 확인한 뒤 최종 결정을 내리세요.", ...evidence };
  if (readiness.state === "review") return { state: "review", label: "확인 후 구매", summary: readiness.summary, ...evidence };
  if (!checklistProgress) return { state: "pending", label: "체크리스트 확인 중", summary: "검사 기준은 통과했지만 구매 전 실행 체크리스트를 불러오는 중이라 최종 결정을 아직 확정하지 않습니다.", ...evidence };
