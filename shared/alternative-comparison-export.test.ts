...
describe("alternative comparison export", () => {
  it("writes a readable text comparison without dropping unknown values", () => {
    ...
    expect(text).toContain("가격 근거: 다나와 수집가");
    expect(text).toContain("구매 조건: 다나와 수집가 · 신품·정식 유통");
    expect(text).toContain("가격 확인 필요");
    expect(text).toContain("게이밍 목표 근거: QHD · 144Hz · 권장 VRAM 12GB · 현재 8GB → 후보 12GB · 권장 기준 충족");
  ...
  it("quotes CSV values with commas and newlines", () => {
    ...
    expect(csv.startsWith("\uFEFF후보명,범주,부품 ID,핵심 스펙,가격,공유 당시 가격(원),가격 근거")).toBe(true);
    expect(csv).toContain('"테스트, 후보"');
    expect(csv).toContain('"가격 확인 필요\n후보"');
    expect(csv).toContain("\"1,200,000원\",1200000,다나와 수집가,다나와 수집가 · 신품·정식 유통,1,대안 43점 · 근거 충분,,QHD · 144Hz · 권장 VRAM 12GB · 현재 8GB → 후보 12GB · 권장 기준 충족,,높음 92점,VRAM 32GB → 12GB (-62.5%),호환 확인,추천 후보 · 현재 문제 해결 · 새 차단 없음,,확인 필요 · GPU·케이스 물리 근거를 구매 전 확인해야 합니다.,GPU · GPU-TEST-1 · rev-A · 검수 2026-09-01: GPU 제조사 설치 가이드 (https://vendor.example/gpu),다나와 최신,갱신 권장,2026-08-28,https://prod.danawa.com/info/?pcode=123");
  });
  ...
  it("returns an empty export envelope for no selected candidates", () => {
    ...
    expect(alternativeComparisonCsvFor([])).toBe("\uFEFF후보명,범주,부품 ID,핵심 스펙,가격,공유 당시 가격(원),가격 근거,구매 조건,추천 킷 수량,성능 유사도,성능 비교 근거,게이밍 목표 근거,가격 대비 유사도,추천 근거 신뢰도,성능 변화,호환 상태,판단 요약,가상 적용 판단,물리 근거,물리 근거 출처,데이터 품질,갱신 상태,갱신일,원문 링크,성능 근거");
  });
  ...
  it("preserves the current comparison baseline in text and JSON context without treating it as a candidate", () => {
    ...
    expect(csv.startsWith("\uFEFF후보명,범주,부품 ID,핵심 스펙,가격,공유 당시 가격(원),가격 근거,구매 조건,추천 킷 수량,성능 유사도,성능 비교 근거,게이밍 목표 근거,가격 대비 유사도,추천 근거 신뢰도,성능 변화,호환 상태,판단 요약,가상 적용 판단,물리 근거,물리 근거 출처,데이터 품질,갱신 상태,갱신일,원문 링크,성능 근거,비교 범주,현재 기준선,현재 기준선 스펙,현재 기준선 가격")).toBe(true);
  ...
  it("preserves structured similarity evidence and model-reference dimensions in shared exports", () => {
    const evidencedCandidate: AlternativeComparisonCandidate = {
      ...
      similarityEvidence: {
        ...
        reference: { partId: "gpu-reference", partName: "RTX 5070 검증 참조", category: "gpu", dataQuality: "live", updatedAt: "2026-09-01", transferredDimensions: ["gpuMemoryBandwidthGbps"], benchmarkSourceKind: "independent_review" },
    ...
    expect(text).toContain("성능 비교 근거: 근거 충분 · 확인 스펙 기반 · 비교 지표 2/3개 · 모델 참조 RTX 5070 검증 참조 · 보완 gpuMemoryBandwidthGbps · 지표별 VRAM 대역폭 224GB/s → 272GB/s (모델 참조)");
    expect(csv).toContain("성능 비교 근거");
    expect(csv).toContain("모델 참조 RTX 5070 검증 참조");
  ...
  it("preserves original CPU/GPU benchmark evidence across text, CSV, and JSON exports", () => {
    ...
    expect(text).toContain("성능 근거: 부분 근거 · 1/2개 · 3DMark Time Spy 21,000점 · 3DMark Port Royal 확인 필요 · 출처 독립 리뷰·벤치마크 DB · 독립 리뷰 측정표 · 원문 https://review.example/gpu · 점검 원문 검증됨");
    expect(csv).toContain("성능 근거");
  ...
  it("exports virtual application and purchase decision facts when present", () => {
    ...
    expect(text).toContain("가상 적용 판단: 확인 필요 · 차단 0 · 주의 1 · 확인 필요 2 · 성능 분석 74점 · 보완 권장 · 현재 대비 -8점 · 일부 스펙 기준 · 가격 변화 +45,000원 · 확인 후 구매 · 가격 이력 30일 4회 · 최저 100,000원 · 구매 전 확인 2개 · 확인됨 1 · 확인 필요 1 · 차단 0 · 주의·확인 필요를 확인한 뒤 구매하세요.");
    expect(csv).toContain("가상 적용 판단");
  ...
  it("exports the candidate tradeoff status and its evidence", () => {
    ...
    expect(text).toContain("효율 경계 · 위험 1점 · 가격 변화 +20,000원 · 분석 82점 · 근거 91점 · 다른 후보에 일방적으로 대체되지 않습니다.");
