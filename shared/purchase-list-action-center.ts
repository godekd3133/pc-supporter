...
export function purchaseListActionCenterFor(input: PurchaseListActionCenterInput): PurchaseListAction[] {
  ...
  if (input.dataReviewCount > 0) actions.push({ kind: "data-review", priority: 100, count: input.dataReviewCount, title: "스펙·상품 페이지 먼저 확인", summary: `오래됐거나 정보가 부족한 구매 항목 ${input.dataReviewCount}개를 주문 전에 다시 확인하세요.` });
  if (input.priceReviewCount > 0) actions.push({ kind: "price-review", priority: 90, count: input.priceReviewCount, title: "가격 먼저 재확인", summary: `현재 가격을 확인하지 못한 항목 ${input.priceReviewCount}개가 있습니다.` });
