...
describe("alternative comparison freshness", () => {
  it("does not overclaim a legacy snapshot without comparison metadata", () => {
    expect(alternativeComparisonFreshnessFor(snapshot({ catalogSnapshotAt: undefined, engineVersion: undefined }), meta())).toMatchObject({ tone: "unknown", label: "이전 확인 정보 없음" });
  });

  it("waits when current metadata has not loaded", () => {
    expect(alternativeComparisonFreshnessFor(snapshot(), null)).toMatchObject({ tone: "unknown", label: "현재 정보 확인 중" });
  });

  it("marks a newer catalog or engine as requiring recheck", () => {
    expect(alternativeComparisonFreshnessFor(snapshot(), meta({ catalogUpdatedAt: "2026-09-02T00:00:00.000Z" }))).toMatchObject({ tone: "stale", label: "최신 정보 확인 권장" });
  ...
  it("reports the same current basis and handles malformed legacy dates safely", () => {
    expect(alternativeComparisonFreshnessFor(snapshot(), meta())).toMatchObject({ tone: "current", label: "공유 당시와 같은 정보" });
    expect(alternativeComparisonFreshnessFor(snapshot({ catalogSnapshotAt: "not-a-date" }), meta())).toMatchObject({ tone: "unknown", label: "이전 확인일 확인 필요" });
  });
});
