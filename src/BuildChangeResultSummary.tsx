...
function priceText(result: CompatibilityResult) {
  return result.priceComplete ? `${result.totalPriceWon.toLocaleString("ko-KR")}원` : "가격 정보 없음";
}
...
function resourceText(result: CompatibilityResult) {
  ...
  const power = metrics.powerHeadroomW === undefined ? "전력 정보 없음" : `전력 ${metrics.powerHeadroomW >= 0 ? `${metrics.powerHeadroomW}W 여유` : `${Math.abs(metrics.powerHeadroomW)}W 부족`}`;
  const cooling = metrics.coolerHeadroomW === undefined ? "냉각 정보 없음" : `냉각 ${metrics.coolerHeadroomW >= 0 ? `${metrics.coolerHeadroomW}W 여유` : `${Math.abs(metrics.coolerHeadroomW)}W 부족`}`;
...
export function BuildChangeResultSummary({ comparison, onDismiss, onCopy, onDownloadJson, onSaveWithDecisionNote, onFocusFinding, decisionSaveLabel = "선택 이유에 첨부해 저장" }: { comparison: BuildChangeResultComparison; onDismiss: () => void; onCopy: () => void; onDownloadJson: () => void; onSaveWithDecisionNote?: () => void; onFocusFinding?: (ruleId: string) => void; decisionSaveLabel?: string }) {
  ...
  return <section className={`build-change-result-summary ${transition.direction}`} aria-label="변경 전후 견적 비교" data-testid="build-change-result-summary" tabIndex={-1}>
    ...
    <div className="build-change-result-summary-grid">
      ...
      <article className={transition.resourceBudgetChanged ? "changed" : undefined}><span>전력·냉각 여유</span><strong>{resourceText(comparison.afterResult)}</strong><small>{transition.resourceRiskIncreased ? "적용 후 여유가 줄었거나 정보가 부족해요." : transition.resourceRiskDecreased ? "적용 후 자원 여유가 개선되었습니다." : "적용 전후 자원 상태 비교"}</small></article>
      <article className={transition.benchmarkChanged || transition.benchmarkNeedsReview ? "changed" : undefined}><span>항목 변화</span><strong>{findingDiff.available ? `해결 ${transition.resolvedFindingCount}개 · 신규 ${transition.newFindingCount}개` : "항목별 비교 정보 없음"}</strong><small>{findingDiff.available ? `중요도 변경 ${transition.severityChangedFindingCount}개 · 내용 변경 ${transition.detailsChangedFindingCount}개` : "저장된 상세 결과가 없어 항목별 차이를 비교할 수 없어요."}</small></article>
    </div>
