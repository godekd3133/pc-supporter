...
export function AdminSeedCatalogPanel() {
  ...
  useEffect(() => {
    ...
    void api<CatalogSeedPreview>("/api/admin/catalog/seed-preview")
      ...
      .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "starter 기준값 대조를 계산하지 못했습니다."); })
      .finally(() => { if (!cancelled) setLoading(false); });
    return () => { cancelled = true; };
  }, [refreshKey]);

  return <section className="admin-seed-preview" data-testid="admin-seed-catalog-preview" aria-label="starter 기준값 대조">
    <div className="admin-seed-preview-heading">
      <div>
        <p className="eyebrow">BASELINE AUDIT</p>
        <h2>starter 기준값 대조</h2>
        <p>새 checkout의 프로젝트 기준 후보와 현재 운영 카탈로그를 ID·범주 기준으로 비교합니다. 이 화면은 병합·삭제·덮어쓰기를 실행하지 않습니다.</p>
      </div>
    ...
    {loading ? <div className="admin-seed-preview-state" role="status"><FiLoader className="spin" /> 현재 카탈로그와 starter 기준값을 대조하는 중...</div> : error ? <div className="admin-seed-preview-state error" role="alert"><FiAlertTriangle /><span>{error}</span></div> : preview && <>
      <div className="admin-seed-preview-summary">
        ...
        <div><span>starter 기준 후보</span><strong>{numberText(preview.starter.total)}개</strong><small>{numberText(preview.categoryRows.length)}개 핵심 범주</small></div>
        <div><span>현재 원본 카탈로그</span><strong>{numberText(preview.active.total)}개</strong><small>{preview.active.coreEligibleCount !== undefined ? `핵심 후보 ${numberText(preview.active.coreEligibleCount)}개` : "핵심 후보 별도 계산"}</small></div>
