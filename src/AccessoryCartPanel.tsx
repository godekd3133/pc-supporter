...
export function AccessoryCartPanel({ selections, accessoryMap, partMap, ssdSelections, rgbControllerAccessoryId, rgbDeviceCount, onChangeQuantity, onChangeTarget, onChangeHubTarget, onChangeRgbController, onRemove, AccessoryVisual }: { selections: AccessorySelection[]; accessoryMap: Map<string, AccessoryItem>; partMap: Map<string, Part>; ssdSelections: PartSelection[]; rgbControllerAccessoryId?: string; rgbDeviceCount?: number; onChangeQuantity: (index: number, quantity: number) => void; onChangeTarget: (index: number, targetPartId: string | undefined) => void; onChangeHubTarget: (index: number, targetAccessoryId: string | undefined) => void; onChangeRgbController: (targetAccessoryId: string | undefined) => void; onRemove: (index: number) => void; AccessoryVisual: AccessoryVisualRenderer }) {
  ...
  const rgbTargetLabel = rgbHubTargets.length === 1 ? "자동 · RGB 지원 허브 1개" : rgbHubTargets.length > 1 ? "RGB 컨트롤러 선택" : "RGB 연결 정보 부족";
  return <section className="accessory-cart-panel" data-testid="accessory-cart-panel" tabIndex={-1}>
    <div className="accessory-cart-heading"><div><h2>추가한 주변 부품</h2></div><div className="accessory-cart-total"><strong>{priceComplete ? `${total.toLocaleString("ko-KR")}원` : "가격 일부 미등록"}</strong><small data-testid="accessory-cart-price-evidence">예상 금액</small></div></div>
    {m2TargetableLines.length > 0 && <p className={`accessory-cart-note accessory-cart-target-guidance${m2Targets.length === 0 ? " unknown" : ""}`} data-testid="accessory-cart-target-guidance"><FiInfo /> {m2Targets.length === 0 ? "연결할 M.2 SSD가 없어요. SSD를 견적에 추가해 주세요." : m2Targets.length === 1 ? `M.2 SSD ${m2TargetQuantity}개가 선택되어 있어 모두 연결해 확인해요.` : `M.2 SSD ${m2Targets.length}종·총 ${m2TargetQuantity}개가 선택되어 있어요. 특정 SSD에 연결하려면 아래에서 골라 주세요.`}</p>}
    {totalRgbDeviceCount !== undefined && totalRgbDeviceCount > 0 && fanHubTargets.length > 0 && <div className="accessory-cart-rgb-target"><label className="accessory-cart-target"><span>RGB 연결 컨트롤러 · 연결 장치 {totalRgbDeviceCount}개{additionalRgbFanDeviceCount > 0 ? ` · 추가 RGB 팬 ${additionalRgbFanDeviceCount}개` : ""}</span><select aria-label="케이스 RGB 연결 컨트롤러" value={rgbControllerAccessoryId ?? ""} onChange={(event) => onChangeRgbController(event.target.value || undefined)}><option value="">{rgbTargetLabel}</option>{rgbControllerAccessoryId && !rgbTargetKnown && <option value={rgbControllerAccessoryId}>대상 정보 부족 · {rgbControllerAccessoryId}</option>}{fanHubTargets.map(({ selection, item }) => <option value={selection.accessoryId} key={selection.accessoryId}>{item.name}{rgbHubTargets.some(({ selection: rgbSelection }) => rgbSelection.accessoryId === selection.accessoryId) ? " · RGB 지원" : " · RGB 사양 정보 부족"}</option>)}</select></label><p className="accessory-cart-note"><FiInfo /> 케이스 기본 RGB 장치와 추가한 RGB 팬을 이 컨트롤러에 연결합니다. 여러 허브 중 RGB를 지원하는 제품을 선택하세요.</p></div>}
    <div className="accessory-cart-list">{lines.map(({ selection, item }, index) => { const targetable = isM2TargetableAccessory(item); const fanTargetable = isFanTargetableAccessory(item); const targetKnown = m2Targets.some(({ part }) => part.id === selection.targetPartId); const hubTargetKnown = fanHubTargets.some(({ selection: hubSelection }) => hubSelection.accessoryId === selection.targetAccessoryId); return <article className="accessory-cart-line" key={`${selection.accessoryId}-${index}`}><span className="accessory-cart-image">{item ? <AccessoryVisual item={item} /> : <FiTool />}</span><div className="accessory-cart-copy"><strong>{item?.name ?? selection.accessoryId}</strong><small>{item ? ACCESSORY_CATEGORY_LABELS[item.category] : "주변 부품 정보를 불러오는 중"}</small>{targetable && <label className="accessory-cart-target"><span>연결 대상 SSD</span><select aria-label={`${item?.name ?? "주변 부품"} 연결 대상 SSD`} value={selection.targetPartId ?? ""} onChange={(event) => onChangeTarget(index, event.target.value || undefined)}><option value="">자동 · 선택한 M.2 SSD 전체</option>{selection.targetPartId && !targetKnown && <option value={selection.targetPartId}>대상 미선택 · {selection.targetPartId}</option>}{m2Targets.map(({ part }) => <option value={part.id} key={part.id}>{part.name}</option>)}</select></label>}{fanTargetable && <label className="accessory-cart-target"><span>연결 대상 팬 허브</span><select aria-label={`${item?.name ?? "주변 부품"} 연결 대상 팬 허브`} value={selection.targetAccessoryId ?? ""} onChange={(event) => onChangeHubTarget(index, event.target.value || undefined)}><option value="">{fanHubTargets.length === 0 ? "허브 미선택 · 나중에 지정" : fanHubTargets.length === 1 ? "자동 · 선택한 허브 1개" : "선택 필요 · 허브를 지정하세요"}</option>{selection.targetAccessoryId && !hubTargetKnown && <option value={selection.targetAccessoryId}>대상 미선택 · {selection.targetAccessoryId}</option>}{fanHubTargets.map(({ selection: hubSelection, item: hub }) => <option value={hubSelection.accessoryId} key={hubSelection.accessoryId}>{hub.name}</option>)}</select></label>}</div><label className="accessory-cart-quantity"><span>수량</span><input type="number" min="1" max="99" value={selection.quantity} aria-label={`${item?.name ?? "주변 부품"} 수량`} onChange={(event) => onChangeQuantity(index, Number(event.target.value))} /></label><strong className="accessory-cart-line-price">{item?.priceWon !== undefined ? `${(item.priceWon * selection.quantity).toLocaleString("ko-KR")}원` : "가격 미확인"}</strong><button className="icon-button danger-button" type="button" aria-label={`${item?.name ?? "주변 부품"} 삭제`} onClick={() => onRemove(index)}><FiTrash2 /></button></article>; })}</div>
