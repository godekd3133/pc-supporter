  ...
  return <section className="saved-build-purchase-progress-comparison" aria-label="저장 견적 구매 진행률 비교" data-testid="saved-build-purchase-progress-comparison">
    ...
    <div className="saved-build-purchase-progress-comparison-grid">{rows.map((row) => {
      ...
      return <article className={`saved-build-purchase-progress-comparison-card ${row.summary.status}`} data-testid={`saved-build-purchase-progress-comparison-${row.id}`} key={row.id}>
        ...
        <div className="saved-build-purchase-progress-comparison-count"><strong>{row.summary.status === "unrecorded" ? "기록 없음" : `${row.summary.checked} / ${row.summary.total}개`}</strong><small>{row.summary.status === "unrecorded" ? "구매 완료 상태가 서버에 없습니다." : `${row.summary.remaining}개 남음 · ${row.summary.percent}%`}</small></div>
        ...
        <small className="saved-build-purchase-progress-comparison-meta">주문 {row.summary.stageCounts.ordered} · 수령 {row.summary.stageCounts.received} · 조립 {row.summary.stageCounts.installed} · {row.summary.revision !== undefined ? `기록 번호 ${row.summary.revision}` : "기록 번호 없음"}{row.summary.historyCount > 0 ? ` · 이전 이력 ${row.summary.historyCount}개` : ""}{row.summary.updatedAt ? ` · ${new Date(row.summary.updatedAt).toLocaleString("ko-KR", { dateStyle: "short", timeStyle: "short" })}` : ""}</small>
      </article>;
    })}</div>
