...
function App() {
    ...
    for (const alert of newAlerts) {
      try {
        const notification = new window.Notification(`PC Supporter · ${alert.buildName}`, {
          body: `${alert.title} · ${alert.message}${alert.findingTitles && alert.findingTitles.length > 0 ? ` · 영향 finding: ${alert.findingTitles.slice(0, 2).join(", ")}` : ""}`,
          tag: alert.id
        });
  ...
  function selectPickerPart(part: Part) {
    ...
    if (picker.findingRuleId) {
      ...
      if (view === "result") {
        ...
        openBuildChangePreview("대체 부품 적용", `${part.name}${quantityText}을 선택한 판정의 대체 후보로 적용합니다. 확인 후 전체 호환성 규칙으로 다시 검사합니다.`, nextBuild, [part], candidateEvidence);
      } else {
        rememberParts([part]);
        setCheckError(null);
        setBuild(nextBuild);
  ...
  function applySuggestion(category: PartCategory, part: Part, quantity?: number, affectedPartIds: string[] = [], candidateEvidence?: CandidateApplicationEvidence) {
    if (candidateApplicationBlockedFor(candidateEvidence)) {
      setToast("차단 오류가 확인된 후보는 적용하지 않습니다. 다른 후보를 선택하거나 판정 근거를 확인해 주세요.");
      return;
    }
    scenarioRequestSequenceRef.current += 1;
    setScenarioPreview(null);
  ...
  function compareCatalogScenarios(category: PartCategory, partsToCompare: CatalogPart[]) {
    if (!result || resultIsStale) {
      setToast("현재 견적을 먼저 검사한 뒤 후보 가상 비교를 시작해 주세요.");
      return;
    }
...
function MobileEditorSurface({ build, partMap, accessoryMap, checking, checkError, hasLastResult, recommendationPreferences, onRecommendationPreferencesChange, onOpenPicker, onCheck, onExportBuild, onImportBuild, onReset, onRefreshCatalogItem, onRefreshAllCatalogItems, refreshingPartId, onToast }: {
  ...
  return <section className="mobile-editor-surface" aria-label="모바일 견적 편집">
    ...
    <div className="mobile-editor-submit"><button className="mobile-primary-action" type="button" onClick={onCheck} disabled={checking || preflight.status !== "ready"}>{checking ? <><FiLoader className="spin" /><span>검사 중...</span></> : <><FiSearch /><span>호환성 검사하기</span><FiArrowRight /></>}</button><small>현재 카탈로그와 규칙 엔진 기준으로 판정합니다.</small></div>
  </section>;
}
...
function RecommendationControls({ preferences, onChange, onCommit, commitOnChange = true, compact = false, disabled = false }: { preferences: RecommendationPreferences; onChange: (next: RecommendationPreferences) => void; onCommit?: (next: RecommendationPreferences) => void; commitOnChange?: boolean; compact?: boolean; disabled?: boolean }) {
  ...
  return <section className={compact ? "recommendation-controls compact" : "recommendation-controls"} aria-label="추천 기준 설정">
    ...
    <p><strong>{RECOMMENDATION_PRIORITY_LABELS[preferences.priority]}</strong> · {RECOMMENDATION_PRIORITY_DESCRIPTIONS[preferences.priority]} 게임 해상도는 권장 VRAM 기준에, 목표 주사율은 CPU·GPU 성능 비교 가중치에 적용됩니다. 실제 FPS나 호환성 판정 자체는 보장·완화하지 않습니다.</p>
  </section>;
}
...
function UpgradeRecommendationPanel({ recommendations, onApply, onPreview, onWatchPart }: { recommendations: UpgradeRecommendation[]; onApply: (recommendation: UpgradeRecommendation) => void; onPreview: (recommendation: UpgradeRecommendation) => void; onWatchPart: PartWatchHandler }) {
  ...
  return <section className="upgrade-recommendation-panel" data-upgrade-sort={sortMode}>
    <p className="upgrade-price-evidence-legend" data-testid="upgrade-price-evidence-legend"><FiInfo /> 가격 숫자는 근거 유형을 함께 봐야 합니다. 다나와 수집가는 카탈로그 기록, 프로젝트 기준가는 실제 판매가가 아니며, 구매 전 원문·재고를 다시 확인하세요.</p>
    ...
    <div className="upgrade-recommendation-sort"><label><span>후보 정렬</span><select aria-label="업그레이드 후보 정렬" value={sortMode} onChange={(event) => setSortMode(event.target.value as UpgradeSortMode)}><option value="recommended">추천 순</option><option value="reliability">근거 충분한 순</option><option value="performance">성능 개선 폭</option><option value="expansion">확장성 개선 폭</option><option value="value">가성비</option><option value="saving">추가 지출 낮은 순</option></select></label><small>{sortMode === "recommended" ? "서버가 계산한 호환·우선순위 순서" : sortMode === "reliability" ? "추천 근거 신뢰도·비교 가능한 근거가 충분한 순" : sortMode === "performance" ? "비교 가능한 스펙 개선 비율이 큰 순" : sortMode === "expansion" ? "후보 적용 후 확장성 점수 변화가 큰 순" : sortMode === "value" ? "가격 변화 대비 개선 효율이 높은 순" : "교체 후 추가 지출이 낮은 순"}</small></div>
...
function AccessoryRecommendationPanel({ recommendations, selectedAccessories, onAddAccessory, onWatchAccessory, isAccessoryWatched }: { recommendations: AccessoryRecommendation[]; selectedAccessories: AccessorySelection[]; onAddAccessory: (item: AccessoryItem) => void; onWatchAccessory?: (item: AccessoryItem, targetPriceWon?: number) => boolean; isAccessoryWatched?: (item: AccessoryItem) => boolean }) {
  ...
  return <section className="peripheral-recommendation-panel" data-testid="peripheral-recommendation-panel">
    ...
    <p className="peripheral-recommendation-note"><FiInfo /> {visibleRecommendations.length}개 / 전체 {recommendations.length}개 추천 표시 · 주변 부품은 핵심 호환 판정과 별도의 보완 제안입니다. 케이스 내부 간섭·전원·커넥터는 구매 전 제조사 원문을 확인해 주세요.</p>
  </section>;
}
...
function AccessoryRecommendationCard({ recommendation, selected, compared, compareDisabled, onToggleCompare, onAddAccessory, onWatchAccessory, isAccessoryWatched }: { recommendation: AccessoryRecommendation; selected: boolean; compared: boolean; compareDisabled: boolean; onToggleCompare: () => void; onAddAccessory: (item: AccessoryItem) => void; onWatchAccessory?: (item: AccessoryItem, targetPriceWon?: number) => boolean; isAccessoryWatched?: (item: AccessoryItem) => boolean }) {
  ...
  return <article className={selected ? "peripheral-recommendation selected" : "peripheral-recommendation"}>
    ...
    <details className="peripheral-recommendation-details" data-testid="accessory-recommendation-details"><summary>스펙·근거 보기 <FiChevronDown /></summary><div className="peripheral-recommendation-detail-grid"><div><span>데이터 상태</span><strong>{DATA_QUALITY_LABELS[recommendation.item.dataQuality]}</strong></div><div><span>마지막 갱신</span><strong>{accessoryRecommendationUpdatedLabel(recommendation.item)}</strong></div><div><span>필수 누락</span><strong>{recommendation.item.missingFields.length > 0 ? `${recommendation.item.missingFields.length}개` : "없음"}</strong></div></div><div className="peripheral-recommendation-raw-spec"><span>저장된 원문 스펙</span><p>{recommendation.item.rawSpecText || "저장된 원문 스펙이 없습니다. 제조사·판매처 원문을 확인하세요."}</p></div></details>
  </article>;
}
    ...
    <div className="compatibility-link-grid">
      {links.map((link) => {
        ...
        return <article className={`compatibility-link ${link.status}`} key={link.id}>
          ...
          <p>{link.summary}</p>
          {onFocusFinding && focusableRuleIds.length > 0 && <button className="text-button compatibility-link-action" type="button" onClick={() => onFocusFinding(focusableRuleIds[0])}>관련 판정 {focusableRuleIds.length > 1 ? `${focusableRuleIds.length}개 ` : ""}보기 <FiExternalLink /></button>}
        </article>;
      })}
    </div>
...
function HistoryView({ builds, currentBuild, currentPreferences, partMap, accessoryMap, monitorAlerts, onMonitorAlertsChange, browserNotificationPermission, browserNotificationEnabled, onRequestBrowserNotifications, onBrowserNotificationsEnabledChange, onBack, onOpen, onRefreshSavedBuilds, onStart, onRevoke, onEditMetadata, revokingShare, onRecordCheck, recordingCheckId, openingBuildId, onShareVersionComparison, onToast }: { builds: SavedBuild[]; currentBuild: BuildSelection; currentPreferences: RecommendationPreferences; partMap: ReadonlyMap<string, Part>; accessoryMap: ReadonlyMap<string, AccessoryItem>; monitorAlerts: SavedBuildMonitorAlert[]; onMonitorAlertsChange: (alerts: SavedBuildMonitorAlert[]) => void; browserNotificationPermission: BrowserNotificationPermission; browserNotificationEnabled: boolean; onRequestBrowserNotifications: () => Promise<void>; onBrowserNotificationsEnabledChange: (enabled: boolean) => void; onBack: () => void; onOpen: (saved: SavedBuild, focus?: SavedBuildOpenFocus) => void; onRefreshSavedBuilds: () => Promise<boolean>; onStart: () => void; onRevoke: (id: string) => void; onEditMetadata: (saved: SavedBuild) => void; revokingShare: boolean; onRecordCheck: (id: string) => void; recordingCheckId: string | null; openingBuildId: string | null; onShareVersionComparison: (before: SavedBuild, after: SavedBuild) => void | Promise<void>; onToast: (message: string) => void }) {
  ...
  async function analyzePriorityAction(id: string) {
    ...
    try {
      ...
      if (isCurrent()) setPriorityActionStates((current) => ({ ...current, [id]: { status: "error", message: error instanceof Error ? error.message : "다음 조치 계산에 실패했습니다." } }));
    } finally {
...
function SavedBuildCheckRefreshImpactPanel({ report, findingChanges }: { report: CatalogRefreshReport; findingChanges: SavedBuildCheckFindingDiff[] }) {
  ...
  return <section className={`history-check-refresh-impact ${report.status}`} aria-label="원문 재확인과 finding 변화 연결" data-testid="saved-build-check-refresh-impact"><div className="history-check-refresh-impact-heading"><div><p className="eyebrow">REFRESH → FINDING IMPACT</p><strong>원문 확인과 판정 변화 연결</strong><small>대상 부품 ID와 before/after finding의 영향 부품 ID가 겹치는 관측만 연결합니다.</small></div><span>{linkedCount}개 연결</span></div>{report.failures.length > 0 && <p className="history-check-refresh-impact-failure"><FiAlertTriangle /> 원문 확인 실패 {report.failures.length}개는 판정 영향까지 연결할 수 없습니다.</p>}<div className="history-check-refresh-impact-list">{impacts.map(({ item, findingChanges: linkedChanges }) => <article className={linkedChanges.length > 0 ? "linked" : "unlinked"} key={`${item.target.kind}-${item.target.id}`}><div className="history-check-refresh-impact-item-heading"><strong>{item.name}</strong><span>{linkedChanges.length > 0 ? `연결 변화 ${linkedChanges.length}개` : "연결된 변화 없음"}</span></div><small>원문 변경 · {item.changedFields.length > 0 ? item.changedFields.join(" · ") : "변경된 영역 없음"}</small>{linkedChanges.length > 0 ? <ul>{linkedChanges.slice(0, 4).map((change) => <li key={`${change.key}-${change.change}`}><b>{savedCheckFindingChangeText(change.change)}</b><span>{(change.after ?? change.before)?.title ?? change.key}</span></li>)}</ul> : <p>이 두 검사 시점에서 해당 부품 ID와 겹치는 변경 finding이 없습니다. 원문 변경이 판정 원인이라고 단정하지 않습니다.</p>}</article>)}</div>{impacts.length === 0 && report.failures.length === 0 && <p className="history-check-refresh-impact-empty"><FiInfo /> 성공한 원문 확인 항목이 없어 finding 연결을 계산할 수 없습니다.</p>}<p className="history-check-refresh-impact-note"><FiInfo /> 이 패널은 영향 가능성이 있는 연결을 보여주는 참고 정보이며, 원문 변경과 판정 사이의 인과관계를 확정하지 않습니다.</p></section>;
}
...
function savedCheckBenchmarkText(snapshot: NonNullable<SavedBuild["checkSnapshot"]>) {
  ...
  if (!benchmark) return "benchmark snapshot 미기록";
...
function savedCheckDriftText(saved: SavedBuild, check: SavedBuildLiveCheck | undefined) {
  ...
  if (diff.benchmarkChanged) changes.push("benchmark 근거 변경");
  else if (diff.benchmarkNeedsReview) changes.push("benchmark 근거 확인 필요");
...
function SavedBuildCheckTransitionSummary({ summary, before, after }: { summary: SavedBuildCheckTransitionSummary; before: SavedBuildCheckSnapshot; after: SavedBuildCheckSnapshot }) {
  ...
  if (lines.length === 0) lines.push("두 시점의 검사 상태·위험 카운트·가격·전력·냉각 예산·benchmark 근거 변화가 없습니다.");
    ...
    : after.status === "needs_review"
      ...
      : after.warningCount > 0
        ...
        : after.resourceBudget?.state === "danger"
          ? "전력·냉각 예산 기준이 미달했으므로 부품 또는 냉각·전원 조건을 바꾼 뒤 다시 검사하세요."
        : after.resourceBudget?.state === "warning" || after.resourceBudget?.state === "unknown"
          ? "전력·냉각 여유 또는 원문 수치를 구매 전에 확인하세요."
    ...
    <section className="history-check-timeline" aria-label="검사 타임라인" data-testid="saved-build-check-timeline" tabIndex={-1}>
      ...
      {showDiff && history.length > 1 && before && after && (
        <div className="history-check-diff" data-testid="saved-build-check-diff">
          ...
          {topLevelDiff && <div className="history-check-diff-overview"><span className={topLevelDiff.statusChanged ? "changed" : ""}>판정 {topLevelDiff.statusChanged ? "변경" : "동일"}</span><span className={topLevelDiff.riskChanged ? "changed" : ""}>위험 카운트 {topLevelDiff.riskChanged ? "변경" : "동일"}</span><span className={topLevelDiff.accessoryRiskChanged ? "changed" : ""}>주변 부품 {topLevelDiff.accessoryRiskChanged ? "변경" : "동일"}</span><span className={topLevelDiff.priceChanged || topLevelDiff.priceCompletenessChanged ? "changed" : ""}>가격 {topLevelDiff.priceChanged || topLevelDiff.priceCompletenessChanged ? "변경" : "동일"}</span><span className={topLevelDiff.resourceBudgetChanged ? "changed" : ""}>전력·냉각 {topLevelDiff.resourceBudgetChanged ? "변경" : "동일"}</span><span className={topLevelDiff.benchmarkChanged || topLevelDiff.benchmarkNeedsReview ? "changed" : ""}>benchmark {topLevelDiff.benchmarkChanged ? "변경" : topLevelDiff.benchmarkNeedsReview ? "확인 필요" : "동일"}</span><span className={topLevelDiff.catalogChanged ? "changed" : ""}>카탈로그 {topLevelDiff.catalogChanged ? "기준 변경" : "동일"}</span></div>}
...
function similarityEvidenceText(evidence?: SimilarityEvidence) {
  ...
  return `${confidenceLabel}${basisLabel === "비교 근거 확인 필요" ? "" : ` · ${basisLabel}`} · 공통 스펙 ${evidence.comparedDimensions}/${evidence.totalDimensions}개${referenceLabel}`;
}
