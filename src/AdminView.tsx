...
function catalogWorkPrioritySummary(action: CatalogWorkPriority) {
  ...
  if (action.kind === "pcie") return "x" + (action.pcieRequiredWidth ?? 4) + " 이상 슬롯 조건의 원문 정보 확인 " + action.complete.toLocaleString("ko-KR") + " / " + action.total.toLocaleString("ko-KR") + "개 · " + action.gapCount.toLocaleString("ko-KR") + "개 확인 필요";
...
function CatalogCategoryIntegrityReviewQueue({ integrity }: { integrity?: ServiceMeta["catalogCategoryIntegrity"] }) {
  ...
  return <div className="catalog-category-integrity-review" data-testid="admin-catalog-category-integrity-review" aria-label="카테고리 불일치 원본 확인 목록">
    <div className="catalog-category-integrity-review-heading"><div><span>RAW RECORD REVIEW</span><strong>분리된 원본 확인 목록</strong><small>자동 재분류·삭제 없이, 카테고리 불일치 정보와 원문을 확인하는 읽기 전용 목록입니다.</small></div><span>{mismatch > 0 ? `${mismatch.toLocaleString("ko-KR")}개 대상` : "확인 대상 없음"}</span></div>
...
function AdminBuildVersionAuditPanel({ audit, loading, error, onRefresh, migrationPreview, migrationPreviewLoading, migrationPreviewError, onMigrationPreview, migrationApplyLoading, migrationApplyError, migrationApplyResult, onApplyMigration, rollbackLoading, rollbackError, rollbackResult, onRollbackMigration, backups, backupDetail, backupDetailLoading, backupDetailError, backupDetailId, onOpenBackupDetail, onCloseBackupDetail, onStartCategoryCrawl, categoryCrawlRunning }: AdminBuildVersionAuditPanelProps) {
  ...
  const seedCatalogPanel = <DeferredAdminPanel label="starter 기준값 대조" anchorId="admin-seed-catalog-preview-panel"><Suspense fallback={<AdminPanelLoading label="starter 기준값 대조" />}><AdminSeedCatalogPanel /></Suspense></DeferredAdminPanel>;
...
export function AdminView({ meta, onMetaRefresh, onToast }: { meta: ServiceMeta | null; onMetaRefresh: () => void; onToast: (message: string) => void }) {
  ...
  async function startCrawl(all = false, category?: PartCategory) {
    if (category) {
      const confirmation = all
        ? `${CATEGORY_LABELS[category]} 전체 상품과 상세 스펙을 끝까지 수집할까요? 선택한 다나와 범주만 교체하고 seed·수동 확인 데이터는 보존합니다. 상품 수에 따라 오래 걸릴 수 있습니다.`
        : `${CATEGORY_LABELS[category]} live 상품을 빠르게 수집할까요? 기존 다나와 카탈로그가 갱신되고, 완료되면 매핑 후보와 원문 수집 목록이 자동으로 다시 계산됩니다.`;
 ...
 const currentAccessoryStatus: AccessoryCrawlStatus = accessoryStatus ?? { status: "idle", mode: "sample", details: true, onlyIncomplete: false, category: undefined, categoriesCompleted: 0, categoriesTotal: 10, pagesVisited: 0, pagesExpected: 0, listedProducts: 0, productsSeen: 0, expectedProducts: 0, productsUpdated: 0, detailFetched: 0, detailFailed: 0, missingProducts: 0, incompleteSpecs: 0, coverage: "partial", specCoverage: "partial" };
  return <div className="admin-page">
    <div className="workspace-heading"><div><button className="back-link" onClick={() => window.history.back()}><FiArrowLeft /> 이전으로</button><p className="eyebrow">CATALOG CONTROL CENTER</p><h1>부품 데이터 센터</h1><p>다나와 수집·정규화·정보 확인와 호환성 데이터 coverage를 관리합니다.</p></div><div className="admin-heading-actions"><span className="admin-badge"><FiShield /> {meta?.adminAuthEnabled ? "인증 보호됨" : "개발용 관리자"}</span>{session?.enabled && session.authenticated && <button className="text-button admin-logout-button" type="button" onClick={() => void logout()} disabled={loginLoading}><FiLogOut /> {loginLoading ? "종료 중..." : "로그아웃"}</button>}</div></div>
