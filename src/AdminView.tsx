...
function AdminCrawlResumeControl() {
  ...
  return <section className="admin-crawl-resume-control" data-testid="admin-crawl-resume-control" aria-live="polite">
    <div className="admin-crawl-resume-copy">
      <span className="eyebrow">중단된 수집</span>
...
function AdminCrawlProgressDetail() {
  ...
  return <section className="admin-crawl-progress-detail" data-testid="admin-crawl-progress-detail" aria-live="polite">
    ...
    {visibleRetryHistory.length > 0 && <div className="admin-crawl-retry-history" data-testid="admin-crawl-retry-history">
      <div className="admin-crawl-retry-history-heading"><span>재시도 이력</span><small>최근 {Math.min(5, retryHistory.length)}건 · 전체 {retryHistory.length}건</small></div>
...
function catalogWorkPriorityTitle(action: CatalogWorkPriority) {
  if (action.kind === "benchmark") return (action.category === "gpu" ? "GPU 3DMark" : "CPU Cinebench R23") + " 정보 보강";
...
function catalogWorkPrioritySummary(action: CatalogWorkPriority) {
  if (action.kind === "benchmark") return "완전 세트 " + action.complete.toLocaleString("ko-KR") + " / " + action.total.toLocaleString("ko-KR") + "개 · " + action.gapCount.toLocaleString("ko-KR") + "개 확인 필요";
  if (action.kind === "pcie") return "x" + (action.pcieRequiredWidth ?? 4) + " 이상 슬롯 조건의 정보 확인 " + action.complete.toLocaleString("ko-KR") + " / " + action.total.toLocaleString("ko-KR") + "개 · " + action.gapCount.toLocaleString("ko-KR") + "개 확인 필요";
...
function catalogCategoryIntegritySourceLabel(source: Part["source"]) {
  return source === "danawa" ? "다나와 원본" : source === "seed" ? "기본 정보" : "수동 입력";
}
...
function CatalogCategoryIntegrityReviewQueue({ integrity }: { integrity?: ServiceMeta["catalogCategoryIntegrity"] }) {
  ...
  useEffect(() => {
    ...
    api<CatalogCategoryIntegrityReviewPackage>(`/api/admin/catalog-category-integrity/review-package?limit=${CATEGORY_INTEGRITY_REVIEW_PAGE_LIMIT}&offset=0`)
      ...
      .catch((error: unknown) => {
        if (active && reviewRequestVersionRef.current === requestVersion) setReviewError(error instanceof Error ? error.message : "카테고리 불일치 원본 목록을 불러오지 못했습니다.");
      })
      .finally(() => {
  ...
  async function loadNextPage() {
    ...
    try {
      ...
      if (nextPackage.queueChanged) {
        setReviewError("카탈로그가 갱신되어 확인 목록이 바뀌었습니다. 관리자 화면을 새로고침해 첫 페이지부터 다시 확인해 주세요.");
        return;
      }
  ...
  return <div className="catalog-category-integrity-review" data-testid="admin-catalog-category-integrity-review" aria-label="카테고리 불일치 원본 확인 목록">
    <div className="catalog-category-integrity-review-heading"><div><strong>분리된 원본 확인 목록</strong><small>자동 재분류·삭제 없이, 카테고리 정보가 맞지 않는 항목과 원본 페이지를 확인하는 목록입니다. 이 화면에서는 값을 바꾸거나 삭제하지 않습니다.</small></div><span>{mismatch > 0 ? `${mismatch.toLocaleString("ko-KR")}개 대상` : "확인 대상 없음"}</span></div>
    ...
    {reviewPackage && reviewPackage.items.length === 0 && <p className="catalog-category-integrity-review-state"><FiCheckCircle /> 현재 확인 목록에 표시할 원본이 없습니다.</p>}
    <p className="catalog-category-integrity-review-note"><FiInfo /> 목록의 원본은 보존됩니다. 카테고리 변경은 페이지 확인 후 별도 운영 절차에서 판단해야 하며, 이 화면에서는 어떤 레코드도 수정하지 않습니다.</p>
  </div>;
}
...
function CatalogCategoryIntegrityPanel({ integrity }: { integrity?: ServiceMeta["catalogCategoryIntegrity"] }) {
  if (!integrity || integrity.checkedCount === 0) return null;
  const mismatch = integrity.mismatchCount;
  return <div className={`catalog-category-integrity ${mismatch > 0 ? "warning" : "good"}`} data-testid="admin-catalog-category-integrity" aria-label="카탈로그 카테고리 일치 여부"><div className="catalog-category-integrity-heading"><div><strong>메인보드 카테고리 일치 여부</strong><small>제품 정체성이 명확히 다른 원본만 핵심 호환 부품과 PCIe evidence 대상에서 분리합니다.</small></div><span>{mismatch > 0 ? `${mismatch.toLocaleString("ko-KR")}개 분리` : "일치 여부 확인"}</span></div><div className="catalog-category-integrity-stats"><span>검사 범위 <strong>{integrity.checkedCount.toLocaleString("ko-KR")}개</strong></span><span>오염 부품 <strong>{mismatch.toLocaleString("ko-KR")}개</strong></span><span>규칙 버전 <strong>v{integrity.ruleVersion}</strong></span></div>{integrity.bySignal.length > 0 && <div className="catalog-category-integrity-signals">{integrity.bySignal.map((signal) => <span key={signal.signal}>{signal.label} <b>{signal.count.toLocaleString("ko-KR")}</b></span>)}</div>}<p><FiInfo /> 원본 레코드는 삭제하지 않고 보존합니다. `센서`·`모듈`·`전원부` 같은 일반 단어는 오탐을 막기 위해 분리 정보로 사용하지 않습니다.</p><CatalogCategoryIntegrityReviewQueue integrity={integrity} /></div>;
}
...
function CatalogPcieSlotCoveragePanel({ coverage }: { coverage: CatalogSpecCoverage }) {
  ...
  return <section className="catalog-pcie-slot-coverage" data-testid="admin-catalog-pcie-coverage" aria-label="메인보드 PCIe 슬롯 정보 비율">
...
function CatalogDataPriorityPanel({ coverage, benchmarkCoverage, accessoryCoverage, onStartCategoryCrawl, onStartCategoryCrawlAll, categoryCrawlRunning, onStartAccessoryCrawl, accessoryCrawlRunning, onOpenBenchmarkReview }: { coverage?: CatalogSpecCoverage; benchmarkCoverage?: ServiceMeta["benchmarkCoverage"]; accessoryCoverage?: ServiceMeta["accessoryCoverage"]; onStartCategoryCrawl?: (category: PartCategory) => void; onStartCategoryCrawlAll?: (category: PartCategory) => void; categoryCrawlRunning?: boolean; onStartAccessoryCrawl?: (category: AccessoryCategory) => void; accessoryCrawlRunning?: boolean; onOpenBenchmarkReview?: () => void }) {
  ...
  return <section className="catalog-data-priority-panel" data-testid="admin-catalog-work-priority" aria-label="다음 데이터 보강 작업"><div className="catalog-data-priority-heading"><div><p className="eyebrow">다음 데이터 작업</p><strong>다음 보강 작업</strong><small>핵심·주변 부품의 누락 사양, PCIe 슬롯 정보, 벤치마크 자료 부족 현황을 모아 보강 순서를 정합니다.</small></div><span>{actionCount}개 우선 작업</span></div><div className="catalog-data-priority-list">{actions.map((action) => <article className={"catalog-data-priority-item " + action.kind} data-testid={"admin-catalog-work-priority-" + action.id.replace(":", "-")} key={action.id}><div className="catalog-data-priority-item-heading"><span className={"catalog-data-priority-kind " + action.kind}>{action.kind === "benchmark" ? "성능 정보" : action.kind === "pcie" ? "PCIe 정보" : "스펙 완성도"}</span><strong>{catalogWorkPriorityTitle(action)}</strong></div><p>{catalogWorkPrioritySummary(action)}</p><small>완전 {action.complete.toLocaleString("ko-KR")} / {action.total.toLocaleString("ko-KR")}개 · 정보 확인률 {action.coveragePercent}%</small><div className="catalog-data-priority-actions">{(action.kind === "spec" || action.kind === "pcie") && <a className="text-button" data-testid="admin-catalog-work-priority-open-missing" href={catalogWorkPriorityPath(action)}><FiSearch /> {action.kind === "pcie" ? "PCIe 정보 부족 보드 보기" : "누락 목록 보기"}</a>}{action.kind === "pcie" && <a className="text-button" data-testid="admin-catalog-work-priority-open-review" href={catalogWorkPriorityReviewPath(action)}><FiRefreshCw /> 정보 보강 목록 열기</a>}{(action.kind === "spec" || action.kind === "pcie") && onStartCategoryCrawl && <button className="text-button" type="button" data-testid="admin-catalog-work-priority-start-category" data-category={action.category} onClick={() => onStartCategoryCrawl(action.category)} disabled={categoryCrawlRunning}><FiRefreshCw /> 범주 빠른 수집</button>}{(action.kind === "spec" || action.kind === "pcie") && onStartCategoryCrawlAll && <button className="text-button exhaustive" type="button" data-testid="admin-catalog-work-priority-start-category-all" data-category={action.category} onClick={() => onStartCategoryCrawlAll(action.category)} disabled={categoryCrawlRunning} title="선택한 범주의 상품 목록과 상세 사양을 처음부터 끝까지 다시 수집합니다."><FiDatabase /> 범주 전체 수집</button>}{action.kind === "benchmark" && onOpenBenchmarkReview && <button className="text-button" type="button" data-testid="admin-catalog-work-priority-open-benchmark" onClick={onOpenBenchmarkReview}><FiSearch /> 벤치마크 확인 목록 열기</button>}</div></article>)}</div>{accessoryActions.length > 0 && <><div className="catalog-data-priority-subheading"><strong>주변 부품 보강</strong><span>{accessoryActions.length}개 우선 작업</span></div><div className="catalog-data-priority-list accessory">{accessoryActions.map((action) => <AccessoryDataPriorityCard action={action} onStartAccessoryCrawl={onStartAccessoryCrawl} accessoryCrawlRunning={accessoryCrawlRunning} key={action.id} />)}</div></>}<p className="catalog-data-priority-note"><FiInfo /> 우선순위는 확인할 순서를 제안합니다. 사양을 임의로 입력하거나 실제 성능을 추정하지 않습니다.</p></section>;
}
...
function CatalogSpecCoveragePanel({ coverage, benchmarkCoverage, accessoryCoverage, onStartCategoryCrawl, onStartCategoryCrawlAll, categoryCrawlRunning, onStartAccessoryCrawl, accessoryCrawlRunning, onOpenBenchmarkReview }: { coverage?: CatalogSpecCoverage; benchmarkCoverage?: ServiceMeta["benchmarkCoverage"]; accessoryCoverage?: ServiceMeta["accessoryCoverage"]; onStartCategoryCrawl?: (category: PartCategory) => void; onStartCategoryCrawlAll?: (category: PartCategory) => void; categoryCrawlRunning?: boolean; onStartAccessoryCrawl?: (category: AccessoryCategory) => void; accessoryCrawlRunning?: boolean; onOpenBenchmarkReview?: () => void }) {
  ...
  return <><CatalogDataPriorityPanel coverage={coverage} benchmarkCoverage={benchmarkCoverage} accessoryCoverage={accessoryCoverage} onStartCategoryCrawl={onStartCategoryCrawl} onStartCategoryCrawlAll={onStartCategoryCrawlAll} categoryCrawlRunning={categoryCrawlRunning} onStartAccessoryCrawl={onStartAccessoryCrawl} accessoryCrawlRunning={accessoryCrawlRunning} onOpenBenchmarkReview={onOpenBenchmarkReview} /><section className="catalog-spec-coverage" data-testid="admin-catalog-spec-coverage" aria-label="카탈로그 스펙 완성도">
    <div className="catalog-spec-coverage-heading"><div><strong>스펙 완성도·보강 우선순위</strong><small>필수 사양 중 빠진 항목과 정보 상태를 집계합니다. 사양을 모르는 부품은 호환되는 것으로 세지 않습니다.</small></div><span className={`catalog-spec-coverage-badge ${coverage.coveragePercent < 90 ? "review" : "good"}`}>{coverage.coveragePercent}% 완전</span></div>
...
function AdminBuildVersionBackupDetailPanel({ detail, loading, error, onClose }: { detail: SavedBuildVersionBackupDetail | null; loading: boolean; error: string | null; onClose: () => void }) {
  ...
  return <div className="version-backup-detail" data-testid="admin-build-version-backup-detail">
    <div className="version-audit-subheading"><strong>백업 변경 내역</strong><span>{detail ? `${detail.items.length}개 변경` : loading ? "불러오는 중" : "조회 실패"}</span><button className="icon-button" aria-label="백업 변경 내역 닫기" onClick={onClose}><FiXCircle /></button></div>
    {loading ? <p className="version-audit-empty"><FiLoader className="spin" /> 백업에 저장된 버전 정보 변경을 확인하고 있습니다.</p> : error ? <div className="version-audit-error"><FiAlertTriangle /><span>{error}</span></div> : detail && <>
      <div className="version-backup-detail-summary"><span>백업 ID {detail.backupId.slice(0, 12)}</span><span>{new Date(detail.createdAt).toLocaleString("ko-KR")}</span><strong className={detail.rollbackAvailable ? "available" : "unavailable"}>{detail.rollbackAvailable ? "복구 가능" : "현재 데이터 변경됨"}</strong></div>
      {detail.items.length === 0 ? <p className="version-audit-clear"><FiCheckCircle /> 변경된 버전 메타데이터가 없습니다.</p> : <div className="version-backup-detail-list">{detail.items.map((item) => <div className="version-backup-detail-row" key={item.buildId}><div className="version-backup-detail-name"><strong>{item.name}</strong><span>{item.changedFields.map((field) => SAVED_BUILD_VERSION_METADATA_LABELS[field] ?? field).join(" · ")}</span></div><div className="version-backup-detail-values"><span><small>그룹</small><code>{item.before.versionGroupId ?? "이전 형식"} → {item.after.versionGroupId ?? "이전 형식"}</code></span><span><small>버전</small><code>{item.before.versionNumber === undefined ? "없음" : `v${item.before.versionNumber}`} → v{item.after.versionNumber ?? "-"}</code></span>{item.before.derivedFromBuildId !== undefined || item.after.derivedFromBuildId !== undefined ? <span><small>부모</small><code>{item.before.derivedFromBuildId ?? "없음"} → {item.after.derivedFromBuildId ?? "없음"}</code></span> : null}</div></div>)}</div>}
      <p className="version-audit-note"><FiInfo /> 구성 부품·가격·견적 소유권 정보는 표시하지 않고 버전 메타데이터만 비교합니다. 현재 데이터 식별값 {detail.currentFingerprint.slice(0, 12)}…</p>
    </>}
  </div>;
}
  ...
  return <>{seedCatalogPanel}{seedMappingPanel}<section className={`admin-card version-audit-card ${audit.status}`} data-testid="admin-build-version-audit">
    ...
    <div className="version-audit-body">
      <div className="version-audit-findings">
        <div className="version-audit-subheading"><strong>확인 결과</strong><span>{audit.versionedCount}개 명시 · {audit.legacyCount}개 이전 형식</span></div>
        {issueCount === 0 ? <p className="version-audit-clear"><FiCheckCircle /> 중복 버전과 부모 견적 연결 오류가 없습니다.</p> : <ul>
          {audit.legacyCount > 0 && <li><FiAlertTriangle /><span>명시적 버전 정보가 없는 견적 <b>{audit.legacyCount}개</b></span></li>}
        ...
        <p className="version-audit-note"><FiInfo /> 이 화면은 읽기 전용 확인용입니다. 마이그레이션이나 삭제는 수행하지 않습니다.</p>
      </div>
    ...
    {(migrationPreviewLoading || migrationPreviewError || migrationPreview) && <div className="version-migration-preview" data-testid="admin-build-version-migration-preview">
      ...
      {migrationPreviewLoading ? <p className="version-audit-empty"><FiLoader className="spin" /> 이전 형식 견적에 적용할 버전 정보를 계산하고 있습니다.</p> : migrationPreviewError ? <div className="version-audit-error"><FiAlertTriangle /><span>{migrationPreviewError}</span><button className="button button-secondary" onClick={onMigrationPreview}><FiSearch /> 다시 계산</button></div> : migrationPreview && <>
        <div className={`version-migration-summary ${migrationPreview.status}`}><span><b>{migrationPreview.changedCount}개</b> 이전 형식 대상</span><span><b>{migrationPreview.items.length - migrationPreview.changedCount}개</b> 메타데이터 유지</span><strong>{migrationPreview.status === "ready" ? "적용 가능" : "수정 후 재검토"}</strong></div>
...
export function AdminView({ meta, onMetaRefresh, onToast }: { meta: ServiceMeta | null; onMetaRefresh: () => void; onToast: (message: string) => void }) {
  ...
  async function loadBackupDetail(backupId: string) {
    ...
    try {
      ...
      if (isCurrent()) setBackupDetailError(error instanceof Error ? error.message : "백업 변경 내역를 불러오지 못했습니다.");
    } finally {
  ...
  async function applyMigration() {
    ...
    if (!window.confirm(`버전 정보가 없는 저장 견적 ${migrationPreview.changedCount}개에 버전 정보를 적용할까요? 원본 백업을 만든 뒤 진행합니다.`)) return;
  ...
  async function rollbackMigration(backup?: SavedBuildVersionBackupSummary) {
    ...
    if (!rollbackTarget) {
      onToast("현재 데이터가 바뀌었거나 복구할 수 있는 백업이 없습니다.");
      return;
    }
    if (!window.confirm("마지막 버전 메타데이터 적용을 되돌릴까요? 적용 후 저장 견적이 바뀌었다면 되돌릴 수 없습니다.")) return;
    const requestVersion = ++migrationMutationRequestRef.current;
    const isCurrent = () => migrationMutationRequestRef.current === requestVersion;
    ...
    try {
      ...
      if (isCurrent()) setRollbackError(error instanceof Error ? error.message : "되돌리기 작업을 완료하지 못했습니다.");
    } finally {
  ...
  async function startCrawl(all = false, category?: PartCategory) {
    if (category) {
      const confirmation = all
        ? `${CATEGORY_LABELS[category]} 전체 상품과 상세 스펙을 끝까지 수집할까요? 선택한 다나와 범주만 교체하고 기본·직접 확인 데이터는 보존합니다. 상품 수에 따라 오래 걸릴 수 있습니다.`
        : `${CATEGORY_LABELS[category]} 수집 상품을 빠르게 수집할까요? 기존 다나와 카탈로그가 갱신되고, 완료되면 매핑 부품과 상품 페이지 수집 목록이 자동으로 다시 계산됩니다.`;
 ...
 const currentAccessoryStatus: AccessoryCrawlStatus = accessoryStatus ?? { status: "idle", mode: "sample", details: true, onlyIncomplete: false, category: undefined, categoriesCompleted: 0, categoriesTotal: 10, pagesVisited: 0, pagesExpected: 0, listedProducts: 0, productsSeen: 0, expectedProducts: 0, productsUpdated: 0, detailFetched: 0, detailFailed: 0, missingProducts: 0, incompleteSpecs: 0, coverage: "partial", specCoverage: "partial" };
  return <div className="admin-page">
    ...
    <section className="admin-card accessory-admin-card">
        ...
        <div className="crawl-summary-line"><span>모드 <strong>{currentAccessoryStatus.mode === "all" ? "전체" : currentAccessoryStatus.onlyIncomplete ? "미확인 배치" : "배치"}</strong></span><span>상세 <strong>{currentAccessoryStatus.details ? "보강 실행" : "목록만"}</strong></span><span>목록 수집 <strong className={currentAccessoryStatus.coverage}>{currentAccessoryStatus.coverage === "complete" ? "완전" : "부분"}</strong></span><span>스펙 <strong className={currentAccessoryStatus.specCoverage}>{currentAccessoryStatus.specCoverage === "complete" ? "완전" : "부분"}</strong></span></div>
      ...
      </section><section className="pipeline-card"><div className="section-title-row"><div><p className="eyebrow">데이터 처리 흐름</p><h2>수집부터 결과까지</h2></div><span className="muted-count">자동 갱신 주기: 24시간</span></div><div className="pipeline-flow"><PipelineStep Icon={FiSearch} title="다나와 목록" text="카테고리·상품 코드" /><span className="pipeline-arrow">→</span><PipelineStep Icon={FiDatabase} title="상세 스펙" text="메타·상품 정보" /><span className="pipeline-arrow">→</span><PipelineStep Icon={FiTool} title="정규화" text="소켓·단위·규격" /><span className="pipeline-arrow">→</span><PipelineStep Icon={FiShield} title="검사 규칙" text="설명 가능한 결과" /></div><p className="pipeline-note"><FiInfo /> 요청 지연과 상세 페이지 보강을 적용합니다. 전체 수집에서는 상품 목록과 상세 페이지를 모두 읽어야 목록 수집이 완료로 표시됩니다. 페이지에 없는 사양은 따로 누락 항목으로 집계합니다.</p></section></div>;
}
