...
function CatalogCategoryIntegrityReviewQueue({ integrity }: { integrity?: ServiceMeta["catalogCategoryIntegrity"] }) {
  ...
  return <div className="catalog-category-integrity-review" data-testid="admin-catalog-category-integrity-review" aria-label="카테고리 불일치 원본 확인 목록">
    <div className="catalog-category-integrity-review-heading"><div><span>RAW RECORD REVIEW</span><strong>분리된 원본 확인 목록</strong><small>자동 재분류·삭제 없이, 카테고리 불일치 정보와 실제 페이지를 확인하는 읽기 전용 목록입니다.</small></div><span>{mismatch > 0 ? `${mismatch.toLocaleString("ko-KR")}개 대상` : "확인 대상 없음"}</span></div>
    ...
    {reviewPackage && reviewPackage.items.length > 0 && <>
      <div className="catalog-category-integrity-review-summary"><span>검사 범위 <strong>{reviewPackage.checkedCount.toLocaleString("ko-KR")}개</strong></span><span>전체 목록 <strong>{reviewPackage.queueTotal.toLocaleString("ko-KR")}개</strong></span><span>현재 표시 <strong>{reviewPackage.offset + 1}–{reviewPackage.offset + reviewPackage.includedCount}</strong></span><span>fingerprint <strong>{reviewPackage.queueFingerprint}</strong></span></div>
...
function CatalogPcieSlotCoveragePanel({ coverage }: { coverage: CatalogSpecCoverage }) {
  ...
  return <section className="catalog-pcie-slot-coverage" data-testid="admin-catalog-pcie-coverage" aria-label="메인보드 PCIe 슬롯 정보 비율">
    <div className="catalog-pcie-slot-coverage-heading"><div><span>PCIE SLOT EVIDENCE</span><strong>메인보드 PCIe 슬롯 정보</strong><small>확장 슬롯 페이지에서 확인한 폭 정보를 기준으로, 요구 폭별 조건 필터에 사용할 수 있는 범위를 표시합니다.</small></div><span>{pcieCoverage.total.toLocaleString("ko-KR")}개 보드</span></div>
    ...
    <p className="catalog-pcie-slot-coverage-note"><FiInfo /> 이 coverage는 전체 카탈로그의 `완전` 수치를 바꾸지 않습니다. PCIe 조건을 적용할 때 페이지 폭 정보가 모두 확인된 보드만 부품으로 사용하며, 정보 부족 보드는 별도 보강 대상으로 확인합니다.</p>
  ...
  return <>{seedCatalogPanel}{seedMappingPanel}<section className={`admin-card version-audit-card ${audit.status}`} data-testid="admin-build-version-audit">
    ...
    <div className="version-audit-body">
      <div className="version-audit-findings">
        <div className="version-audit-subheading"><strong>확인 결과</strong><span>{audit.versionedCount}개 명시 · {audit.legacyCount}개 legacy fallback</span></div>
...
export function AdminView({ meta, onMetaRefresh, onToast }: { meta: ServiceMeta | null; onMetaRefresh: () => void; onToast: (message: string) => void }) {
  ...
  async function startCrawl(all = false, category?: PartCategory) {
    if (category) {
      const confirmation = all
        ...
        : `${CATEGORY_LABELS[category]} live 상품을 빠르게 수집할까요? 기존 다나와 카탈로그가 갱신되고, 완료되면 매핑 부품과 상품 페이지 수집 목록이 자동으로 다시 계산됩니다.`;
 ...
 const currentAccessoryStatus: AccessoryCrawlStatus = accessoryStatus ?? { status: "idle", mode: "sample", details: true, onlyIncomplete: false, category: undefined, categoriesCompleted: 0, categoriesTotal: 10, pagesVisited: 0, pagesExpected: 0, listedProducts: 0, productsSeen: 0, expectedProducts: 0, productsUpdated: 0, detailFetched: 0, detailFailed: 0, missingProducts: 0, incompleteSpecs: 0, coverage: "partial", specCoverage: "partial" };
  return <div className="admin-page">
    <div className="workspace-heading"><div><button className="back-link" onClick={() => window.history.back()}><FiArrowLeft /> 이전으로</button><h1>부품 데이터 센터</h1><p>다나와 수집·정규화·정보 확인와 호환성 정보 비율을 관리합니다.</p></div><div className="admin-heading-actions"><span className="admin-badge"><FiShield /> {meta?.adminAuthEnabled ? "인증 보호됨" : "개발용 관리자"}</span>{session?.enabled && session.authenticated && <button className="text-button admin-logout-button" type="button" onClick={() => void logout()} disabled={loginLoading}><FiLogOut /> {loginLoading ? "종료 중..." : "로그아웃"}</button>}</div></div>
    <section className="admin-hero"><div className="admin-hero-icon"><FiDatabase /></div><div><p className="eyebrow">다나와 수집</p><h2>카탈로그 자동 갱신</h2><p>카테고리 목록과 상세 스펙을 수집하고, 누락 필드·출처·coverage를 분리해 추적합니다.</p></div><div className="crawl-actions"><button className="button button-secondary" onClick={() => void startCrawl(false)} disabled={running || currentStatus.status === "running"}>{running || currentStatus.status === "running" ? <><FiLoader className="spin" /> 갱신 중...</> : <><FiRefreshCw /> 빠른 갱신</>}</button><button className="button button-primary" onClick={() => void startCrawl(true)} disabled={running || currentStatus.status === "running"}><FiDatabase /> 전체 부품 수집</button></div></section>
