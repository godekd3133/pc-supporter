  ...
  return <main className="shared-generator-variants-page">
    <div className="shared-generator-variants-shell">
      ...
      {!loading && !error && snapshot && summary && <>
        ...
        <div className="shared-generator-variants-summary" data-testid="shared-generator-variants-summary"><article><span>구성</span><strong>{summary.configurationCount}종</strong><small>{snapshot.payload.items.length}개 안 비교</small></article><article><span>예상 합계</span><strong>{summary.priceText}</strong><small>공유 당시 금액</small></article><article><span>부품 변경</span><strong>{summary.changedCategories.length === 0 ? "없음" : `${summary.changedCategories.length}개 항목`}</strong><small>{summary.changedCategories.length === 0 ? "모든 안의 부품 동일" : summary.changedCategories.map((category) => CATEGORY_LABELS[category]).join(" · ")}</small></article></div>
        <div className="shared-generator-variants-table-wrap"><table><caption>공유 당시 우선순위별 자동 구성 비교</caption><thead><tr><th scope="col">비교 항목</th>{snapshot.payload.items.map((item) => <th scope="col" key={item.priority}>{item.label}</th>)}</tr></thead><tbody><tr><th scope="row">상태</th>{snapshot.payload.items.map((item) => <td key={`${item.priority}-status`}>{item.status}</td>)}</tr><tr><th scope="row">예상 합계</th>{snapshot.payload.items.map((item) => <td key={`${item.priority}-price`}>{formatWon(item.totalPriceWon)}</td>)}</tr>{PART_CATEGORIES.map((category) => <tr key={category}><th scope="row">{CATEGORY_LABELS[category]}</th>{snapshot.payload.items.map((item) => <td key={`${item.priority}-${category}`}>{lineText(item, category)}</td>)}</tr>)}</tbody></table></div>
        <div className="shared-generator-variant-cards">{snapshot.payload.items.map((item) => <article key={item.priority}><div><strong>{item.label}</strong><span>{item.status}</span></div>{item.error && <p>{item.error}</p>}{item.draft?.warnings?.length ? <p><FiInfo /> {item.draft.warnings[0]}</p> : null}</article>)}</div>
        ...
        {refreshState.status === "ready" && refreshState.items && currentSummary && <section className="shared-generator-variants-current" data-testid="shared-generator-variants-current"><div className="shared-generator-variants-current-heading"><div><p className="eyebrow">새 구성 비교</p><h2>현재 부품으로 다시 비교</h2><small>공유한 견적은 그대로 두고, 같은 조건으로 새로 만든 결과예요.</small></div><span>{currentSummary.configurationCount}종 구성</span></div><div className="shared-generator-variants-current-summary"><article><span>현재 예상 합계</span><strong>{currentSummary.priceText}</strong><small>현재 등록된 가격 기준</small></article><article><span>현재 부품 변경</span><strong>{currentSummary.changedCategories.length === 0 ? "없음" : `${currentSummary.changedCategories.length}개 항목`}</strong><small>{currentSummary.changedCategories.length === 0 ? "현재 3안의 부품 동일" : currentSummary.changedCategories.map((category) => CATEGORY_LABELS[category]).join(" · ")}</small></article></div><div className="shared-generator-variants-current-grid">{refreshState.items.map((item) => { const original = snapshot.payload.items.find((candidate) => candidate.priority === item.priority); const priceDelta = item.totalPriceWon !== undefined && original?.totalPriceWon !== undefined ? item.totalPriceWon - original.totalPriceWon : undefined; return <article key={item.priority}><div><strong>{item.label}</strong><span>{item.status}</span></div><p>현재 합계 {formatWon(item.totalPriceWon)}</p><small>공유 당시 대비 가격 {priceDelta === undefined ? "비교 불가" : `${priceDelta > 0 ? "+" : ""}${formatWon(priceDelta)}`}</small></article>; })}</div></section>}
        <p className="shared-generator-variants-note"><FiInfo /> 공유 시점의 결과만 보존합니다. 구매 전에는 현재 부품 정보로 새 견적을 만들고 호환성을 다시 확인해 주세요.</p>
