...
export function SharedGeneratorVariantsView({ onBack, onToast }: { onBack: () => void; onToast: (message: string) => void }) {
  ...
  async function refreshCurrentCatalog() {
    ...
    try {
      ...
      setRefreshState({ status: "error", error: reason instanceof Error ? reason.message : "다시 비교하지 못했습니다. 잠시 후 다시 시도해 주세요." });
    }
  }
  ...
  return <main className="shared-generator-variants-page">
    <div className="shared-generator-variants-shell">
      ...
      {!loading && !error && snapshot && summary && <>
        ...
        {refreshState.status === "ready" && refreshState.items && currentSummary && <section className="shared-generator-variants-current" data-testid="shared-generator-variants-current"><div className="shared-generator-variants-current-heading"><div><p className="eyebrow">새 구성 비교</p><h2>현재 부품으로 다시 비교</h2><small>공유한 견적은 그대로 두고, 같은 조건으로 새로 만든 결과예요.</small></div><span>{currentSummary.configurationCount}종 구성</span></div><div className="shared-generator-variants-current-summary"><article><span>현재 예상 합계</span><strong>{currentSummary.priceText}</strong><small>현재 등록된 가격 기준</small></article><article><span>현재 부품 점수</span><strong>{currentSummary.analysisText}</strong><small>부품 구성 비교 점수</small></article><article><span>현재 부품 변경</span><strong>{currentSummary.changedCategories.length === 0 ? "없음" : `${currentSummary.changedCategories.length}개 항목`}</strong><small>{currentSummary.changedCategories.length === 0 ? "현재 3안의 부품 동일" : currentSummary.changedCategories.map((category) => CATEGORY_LABELS[category]).join(" · ")}</small></article></div><div className="shared-generator-variants-current-grid">{refreshState.items.map((item) => { const original = snapshot.payload.items.find((candidate) => candidate.priority === item.priority); const priceDelta = item.totalPriceWon !== undefined && original?.totalPriceWon !== undefined ? item.totalPriceWon - original.totalPriceWon : undefined; const scoreDelta = item.analysisScore !== undefined && original?.analysisScore !== undefined ? item.analysisScore - original.analysisScore : undefined; return <article key={item.priority}><div><strong>{item.label}</strong><span>{item.status}</span></div><p>현재 합계 {formatWon(item.totalPriceWon)}</p><small>공유 당시 대비 가격 {priceDelta === undefined ? "비교 불가" : `${priceDelta > 0 ? "+" : ""}${formatWon(priceDelta)}`}</small><small>공유 당시 대비 부품 점수 {scoreDelta === undefined ? "비교 불가" : `${scoreDelta > 0 ? "+" : ""}${scoreDelta}점`}</small></article>; })}</div></section>}
        <p className="shared-generator-variants-note"><FiInfo /> 공유한 결과는 읽기 전용이며 현재 견적에는 자동 적용되지 않습니다. 점수는 게임 FPS가 아닌 구성 비교값이에요.</p>
        {refreshState.status === "ready" && refreshState.items && snapshot.request && <div className="shared-generator-variants-current-actions" data-testid="shared-generator-variants-current-actions"><strong>현재 결과 이어가기</strong><small>공유한 결과는 그대로 두고, 선택한 기준으로 새 견적을 시작합니다.</small><div>{refreshState.items.map((item) => <a className="button button-secondary" key={item.priority} href={`/recommend?${generatorVariantsConditionsSearchFor({ ...snapshot.request!, priority: item.priority })}`} data-testid={`shared-generator-variants-current-start-${item.priority}`}><FiZap /> {item.label} 새 견적 시작</a>)}</div></div>}
        {refreshState.status === "ready" && refreshState.items && <div className="shared-generator-variants-current-draft-actions" data-testid="shared-generator-variants-current-draft-actions"><strong>현재 결과 바로 이어가기</strong><small>선택한 현재 구성을 편집기에서 이어서 살펴보거나 검사·저장할 수 있습니다.</small><div>{refreshState.items.filter((item) => item.draft).map((item) => <span key={item.priority}><button className="button button-secondary" type="button" data-testid={`shared-generator-variants-current-edit-${item.priority}`} onClick={() => transferCurrentDraft(item, "edit")}>{item.label} 편집기로 가져가기</button><button className="button button-primary" type="button" data-testid={`shared-generator-variants-current-check-${item.priority}`} onClick={() => transferCurrentDraft(item, "check")}>{item.label} 바로 검사</button><button className="button button-light" type="button" data-testid={`shared-generator-variants-current-save-${item.priority}`} onClick={() => transferCurrentDraft(item, "save")}>{item.label} 새 견적으로 저장</button></span>)}</div></div>}
      </>}
    </div>
  </main>;
}
