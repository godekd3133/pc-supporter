...
export function UpgradeBundleScenarioPreviewPanel({ state, currentResult, onApply, onRetry, onClose, formatWon }: { state: UpgradeBundleScenarioPreviewState; currentResult: CompatibilityResult; onApply: () => void; onRetry: () => void; onClose: () => void; formatWon: (value: number | undefined) => string }) {
  ...
  const outcomeNote = comparison.direction === "improved"
    ...
    : comparison.direction === "worsened"
      ? "조합 전체를 적용한 결과 위험이 늘어 실제 적용하지 않는 편이 안전합니다."
      : comparison.direction === "changed"
        ? "결과 또는 가격이 바뀌었지만 위험 수준은 같으므로 변경 정보를 확인해 주세요."
  ...
  return <section className={`upgrade-bundle-scenario-preview ${comparison.direction}`} aria-label="업그레이드 조합 미리 확인" data-testid="upgrade-bundle-scenario-preview">
    <div className="upgrade-bundle-scenario-heading"><div><p className="eyebrow">BUNDLE WHAT-IF CHECK</p><h2>업그레이드 조합 미리 확인</h2><p>현재 견적은 바꾸지 않고 {state.bundle.changes.length}개 부품 조합을 전체 검사 규칙에 적용한 결과입니다.</p></div><div className="upgrade-bundle-scenario-heading-actions"><span className={`upgrade-bundle-scenario-direction ${comparison.direction}`}>{directionLabel(comparison.direction)}</span><button className="icon-button" type="button" onClick={onClose} aria-label="업그레이드 조합 미리 확인 닫기"><FiXCircle /></button></div></div>
    ...
    <div className="upgrade-bundle-scenario-findings"><div><strong>조합 적용 후 남는 항목</strong><span>{nextFindings.length > 0 ? `${nextResult.findings.filter((finding) => finding.severity !== "info").length}개 중 최대 5개 표시` : "차단·주의·확인 필요 없음"}</span></div>{nextFindings.length > 0 && <ul>{nextFindings.map((finding) => <li key={finding.id}><b>{finding.severity === "blocker" ? "차단" : finding.severity === "warning" ? "주의" : "확인"}</b>{finding.title}</li>)}</ul>}</div>
    ...
    <p className="upgrade-bundle-scenario-note"><FiInfo /> 조합 전체 미리 확인은 현재 선택·저장 견적·검사 결과를 바꾸지 않습니다. 실제 적용은 변경 예정·가격을 확인하는 미리보기를 거친 뒤 다시 검사합니다.{unsafe ? " 위험이 늘어난 조합은 적용을 막았습니다." : ""}</p>
  </section>;
}
