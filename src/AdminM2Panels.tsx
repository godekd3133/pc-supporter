...
export function M2SlotOverridePanel({ onToast, onMetaRefresh }: { onToast: (message: string) => void; onMetaRefresh: () => void }) {
  ...
  return <section className="admin-card m2-override-card"><div className="admin-card-heading"><div><h3>메인보드별 M.2 슬롯 매핑</h3></div><FiHardDrive /></div><p className="admin-card-description">제조사 매뉴얼에서 확인한 슬롯별 인터페이스·PCIe 세대·연결 주체·공유 대상을 별도 override로 저장합니다. 원본 다나와 데이터는 수정하지 않습니다.</p><div className="m2-override-summary"><span>저장된 매핑 <strong>{overrides.length}개</strong></span>{overrideLoading && <span>불러오는 중...</span>}{overrideError && <span className="m2-override-error">{overrideError}</span>}</div><M2CoveragePanel refreshKey={coverageRefreshKey} onSelectBoard={(partId) => void openCoverageBoard(partId)} onToast={onToast} /><M2ReviewTablePanel refreshKey={coverageRefreshKey} onToast={onToast} onSaved={() => setCoverageRefreshKey((current) => current + 1)} /><M2BatchImportPanel json={batchJson} validation={batchValidation} validatedInput={batchValidatedInput} busy={batchBusy || saving} onChange={handleBatchJsonChange} onValidate={() => void validateBatch()} onSave={() => void saveBatch()} onExport={() => void exportBatch()} />
...
function M2CoveragePanel({ refreshKey, onSelectBoard, onToast }: { refreshKey: number; onSelectBoard: (partId: string) => void; onToast: (message: string) => void }) {
  ...
  return <section className="m2-coverage-panel" aria-label="M.2 매핑 확인 현황">
    <div className="m2-coverage-heading"><div><h3>매핑 커버리지와 확인 목록</h3><p>판매량을 추정하지 않고, 다중 슬롯·복수 PCIe 세대처럼 구조가 복잡한 보드부터 매뉴얼 확인 순서를 제안합니다.</p></div><FiActivity /></div>
