...
export function AdminSeedCollectionQueuePanel({ refreshKey, onFocusMapping, onStartCategory, categoryCrawlRunning }: { refreshKey: number; onFocusMapping?: (starterPartId: string) => void; onStartCategory?: (category: PartCategory) => void; categoryCrawlRunning?: boolean }) {
  ...
  useEffect(() => {
    ...
    void api<CatalogSeedCollectionQueue>("/api/admin/catalog/seed-collection-queue")
      ...
      .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "원문 수집 작업 큐를 불러오지 못했습니다."); })
  ...
  return <section className="admin-seed-collection-queue" data-testid="admin-seed-collection-queue" aria-label="starter 원문 수집 작업 큐" aria-busy={loading}>
    <div className="admin-seed-collection-queue-heading">
      <div>
        <p className="eyebrow">SOURCE COLLECTION PLAN</p>
        <h3>원문 수집 작업 큐</h3>
        <p>자동 후보가 없는 starter를 범주 수집·검색·재확인 작업으로 나눠 다음 수집 범위를 결정합니다. 큐 계산은 읽기 전용이며, 빠른 수집은 확인 후 해당 범주의 기존 수집기를 실행합니다.</p>
      </div>
      <div className="admin-seed-collection-queue-actions">
        ...
        <button className="icon-button" type="button" aria-label="원문 수집 작업 큐 새로고침" title="다시 계산" onClick={() => setReloadKey((current) => current + 1)} disabled={loading}><FiRefreshCw className={loading ? "spin" : undefined} /></button>
      </div>
    </div>
    {loading ? <div className="admin-seed-collection-queue-state" role="status"><FiLoader className="spin" /> 수집 작업 범위를 계산하는 중...</div> : error ? <div className="admin-seed-collection-queue-state error" role="alert"><FiInfo /> {error}</div> : queue && <>
      ...
      <p className="admin-seed-collection-queue-note"><FiInfo /> 작업 큐 fingerprint <code>{queue.queueFingerprint}</code> · 범주 빠른 수집은 최대 16개 상품을 확인하는 샘플 실행이며, 화면에서 시작한 수집은 완료 후 매핑 후보와 큐를 자동으로 다시 계산합니다. 외부 CLI 실행 후에는 `다시 계산`을 눌러 주세요. 수집 전 상품을 이 화면에서 임의로 생성하지 않습니다.</p>
    </>}
  </section>;
}
