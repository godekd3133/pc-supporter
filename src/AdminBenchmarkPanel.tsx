...
function BenchmarkSourceCheckTools({ overrides, busy, sourceCheckingPartId, sourceHistoryPartId, sourceHistory, sourceHistoryLoading, sourceHistoryError, onCheck, onCheckBatch, onToggleHistory }: { overrides: BenchmarkOverrideListItem[]; busy: boolean; sourceCheckingPartId: string | null; sourceHistoryPartId: string | null; sourceHistory: BenchmarkSourceCheckHistoryEntry[]; sourceHistoryLoading: boolean; sourceHistoryError: string | null; onCheck: (partId: string) => void; onCheckBatch: () => void; onToggleHistory: (partId: string) => void }) {
  ...
  return <section className="benchmark-source-check-tools" aria-label="벤치마크 원문 검증" data-testid="benchmark-source-check-tools"><div className="benchmark-source-check-heading"><div><span>SOURCE CHECK</span><strong>벤치마크 원문 검증</strong><small>저장된 HTTPS 원문에 실제 접근하고 부품 모델명/SKU가 본문에 있는지 확인합니다. 검증 결과는 추천 근거에 반영됩니다.</small></div><FiSearch /></div><div className="benchmark-source-check-batch-actions"><button className="button button-light" type="button" data-testid="benchmark-source-check-batch" onClick={onCheckBatch} disabled={busy}>{sourceCheckingPartId === "__batch__" ? <><FiLoader className="spin" /> 일괄 점검 중...</> : <><FiSearch /> 저장 URL 일괄 점검</>}</button><span>최대 50개 · 현재 필터 기준</span></div><div className="benchmark-source-check-list">{targets.slice(0, 12).map((override) => { const sourceUrl = safeHttpsUrl(override.sourceUrl); return <article key={override.partId}><div><strong>{override.partName ?? override.partId}</strong><small>{override.partId} · {benchmarkSourceCheckText(override.sourceCheck)}{override.sourceCheck?.checkedAt ? ` · ${new Date(override.sourceCheck.checkedAt).toLocaleDateString("ko-KR")}` : ""}</small></div><div>{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer"><FiExternalLink /> 원문</a>}<button className="text-button" type="button" onClick={() => onCheck(override.partId)} disabled={busy}>{sourceCheckingPartId === override.partId ? <><FiLoader className="spin" /> 점검 중...</> : <><FiSearch /> {override.sourceCheck ? "다시 점검" : "원문 점검"}</>}</button><button className="text-button" type="button" onClick={() => onToggleHistory(override.partId)} disabled={busy}>{sourceHistoryPartId === override.partId ? "이력 닫기" : "이력 보기"}</button></div></article>; })}</div>{sourceHistoryPartId && <BenchmarkSourceCheckHistoryPanel entries={sourceHistory} loading={sourceHistoryLoading} error={sourceHistoryError} />}{targets.length > 12 && <small className="benchmark-source-check-more">그 외 {targets.length - 12}개는 저장된 보강 목록에서 확인하세요.</small>}</section>;
}
...
function Benchmark3DMarkWorkPackagePanel({ busy, onToast, onOpenItem, onPrepareBatchInput, refreshToken }: { busy: boolean; onToast: (message: string) => void; onOpenItem: (item: Benchmark3DMarkReviewWorkItem) => void; onPrepareBatchInput: (items: Benchmark3DMarkReviewWorkItem[]) => void; refreshToken: number }) {
    ...
    void api<Benchmark3DMarkReviewWorkPackage>(`/api/admin/benchmark-review/work-package?${params.toString()}`)
      .then((value) => {
        ...
        if (value.queueChanged && offset > 0) {
          ...
          onToast("3DMark GPU 검수 큐가 갱신되어 첫 작업 패키지부터 다시 표시합니다.");
          return;
        }
      ...
      .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "3DMark GPU 검수 작업 패키지를 불러오지 못했습니다."); })
  ...
  function resetProgress() {
    ...
    setRetryNonce((current) => current + 1);
    onToast("3DMark GPU 검수 작업 패키지를 처음부터 다시 표시합니다.");
  }
...
export function BenchmarkOverridePanel({ onToast, onMetaRefresh, storageMode }: { onToast: (message: string) => void; onMetaRefresh: () => void; storageMode?: ServiceMeta["storageMode"] }) {
  ...
  async function checkSource(partId: string) {
    ...
    try {
      ...
      setReviewQueueRefreshToken((current) => current + 1);
      onMetaRefresh();
      onToast(`benchmark 원문 점검 완료: ${benchmarkSourceCheckText(result.sourceCheck)}${result.historyRecorded ? " · 이력 기록됨" : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "benchmark 원문 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
  ...
  async function checkSourcesBatch() {
    ...
    if (targets.length === 0) {
      onToast("점검할 benchmark HTTPS 원문 URL이 없습니다.");
      return;
    }
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    ...
    try {
      ...
      setReviewQueueRefreshToken((current) => current + 1);
      onMetaRefresh();
      onToast(`benchmark 원문 ${result.checkedCount}개 점검 완료 · 검증됨 ${result.passedCount}개 · 재확인 ${result.reviewCount}개${result.persistFailureCount > 0 ? ` · 저장 실패 ${result.persistFailureCount}개` : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "benchmark 원문 일괄 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
        setSourceCheckingPartId(null);
        setBusy(false);
  ...
  async function toggleSourceHistory(partId: string) {
    ...
    try {
      ...
      if (!isCurrent()) return;
      setSourceHistory([]);
      setSourceHistoryError(reason instanceof Error ? reason.message : "benchmark 원문 점검 이력을 불러오지 못했습니다.");
    } finally {
  ...
  function openReviewItemInComposer(item: { partId: string; partName: string; category: "cpu" | "gpu" }) {
    ...
    onToast(`${item.partName}을 행 작성 도구에서 불러오는 중입니다. 원문 URL을 입력해 검수하세요.`);
  }
  ...
  async function preview3DMarkResult() {
    ...
    try {
      ...
      onToast(result.identityStatus === "matched" ? `${result.benchmarkLabel} ${result.score.toLocaleString("ko-KR")}점의 GPU 식별자가 일치합니다. 검수 행에 반영할 수 있습니다.` : `${result.benchmarkLabel} 결과를 읽었습니다. GPU 식별자를 수동 대조한 뒤 반영해 주세요.`);
    } catch (reason: unknown) {
      if (!mountedRef.current || benchmark3DMarkRequestVersionRef.current !== requestVersion) return;
  ...
  function add3DMarkPreviewToJson() {
    ...
    if (!preview || !selectedPart || preview.identityStatus !== "matched") {
      onToast("GPU 식별자가 일치하는 3DMark 미리보기만 검수 행에 반영할 수 있습니다.");
      return;
    }
    ...
    try {
      ...
      onToast(`${preview.benchmarkLabel} ${preview.score.toLocaleString("ko-KR")}점을 입력 JSON에 반영했습니다. JSON 검증 후 저장해 주세요.`);
    } catch {
      onToast("기존 JSON 형식이 올바르지 않아 3DMark 결과를 반영하지 못했습니다. JSON을 먼저 수정해 주세요.");
    }
  }
  ...
  return <section className="admin-card benchmark-override-admin-card" aria-label="벤치마크 보강 관리">
    ...
    <p className="admin-card-description">원문에 없거나 별도로 검수한 Cinebench·3DMark 점수를 부품 ID에 연결합니다. 모든 값은 서버에서 부품 종류·양수 정수·근거 메모·HTTPS URL을 검증한 뒤 원자적으로 저장하며, 저장된 원문은 실제 접근·모델 식별 점검까지 실행할 수 있습니다.</p>
