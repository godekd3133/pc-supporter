...
export function BenchmarkOverridePanel({ onToast, onMetaRefresh, storageMode }: { onToast: (message: string) => void; onMetaRefresh: () => void; storageMode?: ServiceMeta["storageMode"] }) {
  ...
  async function checkSource(partId: string) {
    ...
    try {
      ...
      setReviewQueueRefreshToken((current) => current + 1);
      onMetaRefresh();
      onToast(`벤치마크 원문 점검 완료: ${benchmarkSourceCheckText(result.sourceCheck)}${result.historyRecorded ? " · 이력 기록됨" : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "벤치마크 원문 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
  ...
  async function checkSourcesBatch() {
    ...
    try {
      ...
      setReviewQueueRefreshToken((current) => current + 1);
      onMetaRefresh();
      onToast(`벤치마크 원문 ${result.checkedCount}개 점검 완료 · 확인됨 ${result.passedCount}개 · 재확인 ${result.reviewCount}개${result.persistFailureCount > 0 ? ` · 저장 실패 ${result.persistFailureCount}개` : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "벤치마크 원문 일괄 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
        setSourceCheckingPartId(null);
        setBusy(false);
  ...
  async function toggleSourceHistory(partId: string) {
    ...
    try {
      ...
      if (!isCurrent()) return;
      setSourceHistory([]);
      setSourceHistoryError(reason instanceof Error ? reason.message : "벤치마크 원문 점검 이력을 불러오지 못했습니다.");
    } finally {
  ...
  function openReviewItemInComposer(item: { partId: string; partName: string; category: "cpu" | "gpu" }) {
    ...
    onToast(`${item.partName}을 행 작성 도구에서 불러오는 중입니다. 원문 URL을 입력해 확인하세요.`);
  }
  ...
  return <section className="admin-card benchmark-override-admin-card" aria-label="벤치마크 보강 관리">
    ...
    <p className="admin-card-description">원문에 없거나 별도로 확인한 Cinebench·3DMark 점수를 부품 ID에 연결합니다. 모든 값은 서버에서 부품 종류·양수 정수·출처 메모·HTTPS URL을 확인한 뒤 원자적으로 저장하며, 저장된 원문은 실제 접근·모델 식별 점검까지 실행할 수 있습니다.</p>
    ...
    <div className="benchmark-override-composer">
      ...
      {composerPartsLoading ? <p className="benchmark-override-composer-state"><FiLoader className="spin" /> 부품 검색 중...</p> : composerParts.length > 0 ? <div className="benchmark-override-composer-results">{composerParts.map((part) => <button className={part.id === composerSelectedPartId ? "selected" : ""} type="button" onClick={() => setComposerSelectedPartId(part.id)} key={part.id}><strong>{part.name}</strong><small>{part.id} · {part.dataQuality === "live" ? "다나와 최신" : part.dataQuality === "manual" ? "수동 확인" : part.dataQuality === "incomplete" ? "일부 스펙 부족" : "프로젝트 데이터"}</small></button>)}</div> : <p className="benchmark-override-composer-state">검색 결과가 없습니다.</p>}
