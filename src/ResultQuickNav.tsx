...
export function ResultQuickNav({ result, onFocusSection }: { result: CompatibilityResult; onFocusSection: (targetId: string) => void }) {
  ...
  const items: Array<{ targetId: string; label: string; detail: string; Icon: IconType }> = [
    ...
    { targetId: "build-action-center", label: "우선 조치", detail: actionCount > 0 ? `${actionCount}개 확인 항목` : "최종 확인", Icon: FiZap },
    { targetId: "result-findings", label: "상세 판정", detail: `${result.findings.length}개 finding`, Icon: FiSearch },
    { targetId: "purchase-checklist", label: "구매 전 체크", detail: "실행 항목 관리", Icon: FiCheckCircle },
    { targetId: "purchase-list-panel", label: "구매 목록", detail: "가격·구매 상태", Icon: FiShoppingCart }
  ];
  return <nav className="result-quick-nav" aria-label="검사 결과 바로가기" data-testid="result-quick-nav">
    <div className="result-quick-nav-intro"><p className="eyebrow">QUICK JUMP</p><strong>필요한 근거로 바로 이동</strong><span>긴 결과도 핵심 조치·상세 판정·구매 준비를 한 번에 확인하세요.</span></div>
