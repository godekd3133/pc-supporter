...
export function CaseRgbLoadOverridePanel({ onToast, onMetaRefresh }: { onToast: (message: string) => void; onMetaRefresh: () => void }) {
  ...
  async function removeOverride(partId: string) {
    if (!window.confirm("이 케이스의 수동 RGB 부하값을 삭제할까요? 삭제 후 페이지에서 읽은 값이 있으면 그 값을 사용합니다. 페이지에 값이 없으면 연결 계획에 확인이 필요하다고 표시됩니다.")) return;
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    setBusy(true);
    try {
  ...
  return <section className="admin-card case-rgb-load-card" data-testid="admin-case-rgb-load">
    <div className="admin-card-heading"><div><h3>케이스 RGB 부하 확인</h3><p className="admin-card-description">상품 페이지에 없는 RGB 장치 1개의 소비전력이나 전류를 제조사 자료에서 확인해 기록합니다. 저장값은 기본 카탈로그를 바꾸지 않고 호환성 검사에만 반영됩니다.</p></div><FiShield /></div>
    {error && <div className="case-rgb-load-error" role="alert"><FiXCircle /> {error}</div>}
    <div className="case-rgb-load-coverage"><div><strong>{coverage?.totalRgbCases.toLocaleString("ko-KR") ?? "-"}</strong><span>RGB 케이스</span></div><div><strong>{coverage?.registeredCount.toLocaleString("ko-KR") ?? "-"}</strong><span>부하 보강 등록</span></div><div><strong>{coverage?.missingCount.toLocaleString("ko-KR") ?? "-"}</strong><span>정보 미등록</span></div><div><strong>{coverage ? `${coverage.coveragePercent}%` : "-"}</strong><span>정보 확인률</span></div></div>
    <div className="case-rgb-load-grid">
      <div className="case-rgb-load-editor">
        ...
        {selectedPart && <form className="case-rgb-load-form" onSubmit={(event) => void saveSelected(event)}>
          ...
          <p className="case-rgb-load-help"><FiInfo /> 장치당 전류 또는 전력 중 하나만 입력하면 됩니다. 검사할 때 RGB 전압, 장치 수, 컨트롤러의 해당 전압 출력 한도를 함께 비교합니다.</p>
        </form>}
      </div>
    ...
    <p className="case-rgb-load-note"><FiInfo /> 저장값은 기본 카탈로그와 따로 관리합니다. 삭제하면 상품 페이지에서 읽은 값으로 돌아갑니다. 페이지에도 RGB 부하 정보가 없으면 연결 계획에 확인이 필요하다고 표시됩니다.</p>
  </section>;
}
