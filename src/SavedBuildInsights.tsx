...
import { FiActivity, FiAlertTriangle, FiCopy, FiDownload, FiExternalLink, FiGitBranch, FiInfo, FiLayers, FiLoader, FiRefreshCw, FiSave, FiShare2, FiZap, FiXCircle } from "react-icons/fi";
...
import { savedBuildOriginAvailabilityLabelFor, savedBuildOriginComparisonTextFor, savedBuildOriginDetailFor, savedBuildOriginLabelFor, type SavedBuildOriginAvailability } from "../shared/saved-build-origin";
...
function savedBuildOriginKeyFor(build: SavedBuild) {
  const origin = build.origin;
  return origin ? [origin.kind, origin.sourceShareId ?? "", origin.sourcePriority ?? ""].join("|") : "none";
}
...
function currentCheckChangeFor(build: SavedBuild, check: SavedBuildLiveCheck | undefined, originStatus?: SavedBuildOriginAvailability) {
  ...
  const compatibilityChanged = statusRank(result.status) > statusRank(snapshot.status) || result.blockerCount > snapshot.blockerCount || result.warningCount > snapshot.warningCount || result.unknownCount > snapshot.unknownCount;
...
export function SavedBuildPriorityPanel({ rows, actionStates, openingBuildId, onAnalyzeAction, onOpen }: { rows: SavedBuildPriorityRow[]; actionStates: Record<string, SavedBuildPriorityActionViewState>; openingBuildId: string | null; onAnalyzeAction: (id: string) => void; onOpen: (id: string) => void }) {
  ...
  return <section className="saved-build-priority-panel" aria-label="저장 견적 우선 확인 보드" data-testid="saved-build-priority-board">
    ...
    <p className="saved-build-priority-note"><FiInfo /> 위험 점수는 차단 100점·주의 10점·확인 필요 1점으로 계산한 정렬용 신호입니다. 실제 구매 가능 여부를 대신하지 않으며, 각 행의 견적 보기에서 전체 내용과 대체 부품을 확인해 주세요.</p>
  </section>;
}

export function SavedBuildVersionPanel({ groups, openingBuildId, onOpen, onShareVersionComparison, onSaveVersion, BuildComparisonPanel, partMap, accessoryMap, fallbackPreferences, originAvailability = {} }: { groups: SavedBuildVersionGroup[]; openingBuildId: string | null; onOpen: (saved: SavedBuild, focus?: "purchase-list") => void; onShareVersionComparison?: (before: SavedBuild, after: SavedBuild) => void | Promise<void>; onSaveVersion?: (saved: SavedBuild) => void; BuildComparisonPanel: ComponentType<{ builds: SavedBuild[]; onOpenBuild?: (build: SavedBuild, focus?: "purchase-list") => void; openingBuildId?: string | null; onLiveChecksChange?: (checks: Record<string, SavedBuildLiveCheck>) => void }>; partMap: ReadonlyMap<string, Part>; accessoryMap: ReadonlyMap<string, AccessoryItem>; fallbackPreferences: RecommendationPreferences; originAvailability?: Readonly<Record<string, SavedBuildOriginAvailability>> }) {
  ...
  const [currentChangeFilter, setCurrentChangeFilter] = useState<"all" | "price" | "compatibility" | "origin">("all");
  ...
  const originDifferentPair = selectedGroup?.builds.flatMap((before, index) => selectedGroup.builds.slice(index + 1).map((after) => [before, after] as const)).find(([before, after]) => savedBuildOriginKeyFor(before) !== savedBuildOriginKeyFor(after));
  const currentChangeRows = selectedGroup?.builds.map((build) => ({ build, change: currentCheckChangeFor(build, liveChecks[build.id], build.origin?.sourceShareId ? originAvailability[build.origin.sourceShareId] : undefined) })) ?? [];
  const currentConcernVersions = currentChangeRows.filter(({ change }) => change.changed && (currentChangeFilter === "all" || currentChangeFilter === "price" && change.priceChanged || currentChangeFilter === "compatibility" && change.compatibilityChanged || currentChangeFilter === "origin" && change.originUnavailable)).map(({ build }) => build);
  const currentChangeCounts = {
    all: currentChangeRows.filter(({ change }) => change.changed).length,
    price: currentChangeRows.filter(({ change }) => change.priceChanged).length,
    compatibility: currentChangeRows.filter(({ change }) => change.compatibilityChanged).length,
    origin: currentChangeRows.filter(({ change }) => change.originUnavailable).length
  };
  const currentConcernPair = currentConcernVersions.length >= 2 ? currentConcernVersions.slice(0, 2) : currentConcernVersions.length === 1 ? [currentConcernVersions[0], selectedGroup?.builds.find((build) => build.id !== currentConcernVersions[0].id)] : undefined;
  ...
  function renderVersionRow(build: SavedBuild, index: number) {
    const previous = selectedGroup.builds[index - 1];
    const parent = build.derivedFromBuildId ? selectedGroup.builds.find((candidate) => candidate.id === build.derivedFromBuildId) : undefined;
    const baseline = parent ?? previous;
    const versionDelta = baseline ? savedBuildVersionDeltaFor(baseline, build) : undefined;
    const transferDiff = baseline ? buildTransferDiffFor(baseline.selection, baseline.recommendationPreferences ?? fallbackPreferences, build.selection, build.recommendationPreferences ?? fallbackPreferences, { partName: (partId) => partMap.get(partId)?.name, accessoryName: (accessoryId) => accessoryMap.get(accessoryId)?.name }) : undefined;
    ...
    const currentCheckText = !currentCheck || currentCheck.status === "loading"
      ...
      : currentCheck.status === "error"
        ...
        : currentChange.changed
          ? `현재 기준 변화 · ${currentChange.reason}`
          : "현재 기준 변화 없음";
    const currentCheckMetric = currentCheck?.status === "ready" ? `현재 ${currentCheckPriceText(currentCheck)} · ${currentCheckStatusText(currentCheck)}` : undefined;
    return <article className="saved-build-version-row" key={build.id}>
      <span className="saved-build-version-number">{savedBuildVersionLabelFor(build)}</span>
      <div>
        ...
        {build.origin && <small className="saved-build-version-origin" data-testid={`saved-build-version-origin-${build.id}`}><FiGitBranch /> {savedBuildOriginLabelFor(build.origin)}{savedBuildOriginDetailFor(build.origin) ? ` · ${savedBuildOriginDetailFor(build.origin)}` : ""}</small>}
      ...
      <button className="text-button" type="button" onClick={() => onOpen(build)} disabled={openingBuildId !== null}>{openingBuildId === build.id ? <><FiLoader className="spin" /> 불러오는 중...</> : <><FiExternalLink /> 버전 열기</>}</button>
    </article>;
  }
  ...
  return <section className="saved-build-version-panel" aria-label="저장 견적 버전 비교" data-testid="saved-build-version-panel">
    ...
    {comparedVersions.length === 2 && (() => { const [before, after] = comparedVersions; const delta = savedBuildVersionDeltaFor(before, after); const transition = delta.transition; return <section className={`saved-build-version-summary ${transition?.direction ?? "unknown"}`} data-testid="saved-build-version-summary" aria-label="선택한 두 버전 적용 요약"><div className="saved-build-version-summary-heading"><div><strong>선택한 두 버전 적용 요약</strong><small>{savedBuildVersionLabelFor(before)} → {savedBuildVersionLabelFor(after)} · {versionDeltaDirectionLabel(transition?.direction)}</small></div><FiActivity /></div><div className="saved-build-version-summary-grid"><div><span>구성</span><strong>{delta.selectionChangedCategoryCount > 0 ? `${delta.selectionChangedCategoryCount}개 범주 변경` : "변화 없음"}</strong></div><div><span>위험 변화</span><strong>{transition ? `차단 ${signedVersionDelta(transition.blockerDelta)} · 주의 ${signedVersionDelta(transition.warningDelta)} · 확인 ${signedVersionDelta(transition.unknownDelta)}` : "비교 기준 없음"}</strong></div><div><span>항목 변화</span><strong>{delta.resolvedFindingCount !== undefined ? `해결 ${delta.resolvedFindingCount} · 신규 ${delta.newFindingCount} · 변경 ${delta.changedFindingCount}` : "비교 기준 없음"}</strong></div><div><span>총액</span><strong>{transition?.priceDeltaWon !== undefined ? `${transition.priceDeltaWon > 0 ? "+" : ""}${transition.priceDeltaWon.toLocaleString("ko-KR")}원` : "가격 확인 필요"}</strong></div><div><span>분석 점수</span><strong>{transition?.analysisScoreDelta !== undefined ? `${transition.analysisScoreDelta > 0 ? "+" : ""}${transition.analysisScoreDelta}점` : "확인 필요"}</strong></div><div data-testid="saved-build-version-origin-diff"><span>생성 출처</span><strong>{savedBuildOriginComparisonTextFor(before.origin, after.origin)}</strong></div></div></section>; })()}
    <div className="saved-build-version-list">{selectedGroup.builds.map(renderVersionRow)}</div>
    {comparedVersions.length === 2 && <details className="saved-build-version-origin-detail" data-testid="saved-build-version-origin-detail"><summary><FiGitBranch /> 생성 출처 상세 <strong>{savedBuildOriginComparisonTextFor(comparedVersions[0].origin, comparedVersions[1].origin)}</strong></summary><div className="saved-build-version-origin-detail-grid">{comparedVersions.map((build) => { const sourceShareId = build.origin?.sourceShareId; const status = sourceShareId ? originAvailability[sourceShareId] : undefined; return <article key={build.id}><div><span>{savedBuildVersionLabelFor(build)}</span><strong>{build.name}</strong></div>{build.origin ? <><small>{savedBuildOriginLabelFor(build.origin)}</small><small>{savedBuildOriginDetailFor(build.origin) ?? "추가 출처 정보 없음"}</small>{build.checkSnapshot && <small>저장 검사 · {savedCheckStatusText(build.checkSnapshot.status)} · {build.checkSnapshot.priceComplete ? formatWon(build.checkSnapshot.totalPriceWon) : "가격 확인 필요"} · {new Date(build.checkSnapshot.catalogSnapshotAt).toLocaleString("ko-KR", { dateStyle: "short", timeStyle: "short" })}</small>}<em className={`saved-build-version-origin-status ${status ?? "unknown"}`}>{savedBuildOriginAvailabilityLabelFor(status)}</em>{status === "unavailable" && <p><FiAlertTriangle /> 원본 snapshot을 열 수 없습니다. 구매 전 현재 catalog 기준으로 다시 검사하세요.</p>}{sourceShareId && <a href={`/generator-variants/${encodeURIComponent(sourceShareId)}`} className="text-button">원본 비교 열기 <FiExternalLink /></a>}</> : <small>생성 출처 기록 없음</small>}</article>; })}</div><div className="saved-build-version-origin-live" data-testid="saved-build-version-origin-live"><strong>저장 시점 ↔ 현재 catalog 재검사</strong><div className="saved-build-version-origin-live-grid">{comparedVersions.map((build) => <article key={build.id}><span>{savedBuildVersionLabelFor(build)}</span><small>저장 {build.checkSnapshot?.priceComplete ? formatWon(build.checkSnapshot.totalPriceWon) : "가격 확인 필요"} · {build.checkSnapshot ? savedCheckStatusText(build.checkSnapshot.status) : "검사 기록 없음"}</small><small>현재 {currentCheckPriceText(liveChecks[build.id])} · {currentCheckStatusText(liveChecks[build.id])}</small></article>)}</div></div></details>}
