...
export function PurchaseListPanel({ rows, storageKey, inputFingerprint, budgetWon, savedBuildId, savedBuildOwnerToken, onCopy, onDownload, focusStatus, onProgressChange, onServerProgressChange, onServerPriceHistoryChange, onWatchEntry, isWatchedEntry, onOpenCatalogItem, onRefreshAll, refreshingItemId, catalogRefreshReport }: { rows: PurchaseListRow[]; storageKey: string; inputFingerprint: string; budgetWon?: number; savedBuildId?: string; savedBuildOwnerToken?: string; onCopy: (checkedIds?: ReadonlySet<string>, rows?: PurchaseListRow[], itemStates?: ReadonlyArray<PurchaseListItemStatus>) => void; onDownload: (checkedIds?: ReadonlySet<string>, rows?: PurchaseListRow[], itemStates?: ReadonlyArray<PurchaseListItemStatus>) => void; focusStatus?: PurchaseItemStatus; onProgressChange?: (progress: PurchaseListExecutionProgress) => void; onServerProgressChange?: (progress?: SavedBuildPurchaseProgress) => void; onServerPriceHistoryChange?: (history?: SavedBuildPurchasePriceHistory) => void; onWatchEntry?: (target: PurchaseListWatchTarget) => boolean; isWatchedEntry?: (target: Pick<PurchaseListWatchTarget, "kind" | "itemId">) => boolean; onOpenCatalogItem?: (row: PurchaseListRow) => void; onRefreshAll?: (targets: RefreshTarget[]) => void; refreshingItemId?: string | null; catalogRefreshReport?: CatalogRefreshReport | null }) {
  ...
  return (
    <section className="purchase-list-panel" aria-label="구매 목록" data-testid="purchase-list-panel" tabIndex={-1}>
      ...
      {rows.length > 0 && <section className={`purchase-list-price-evidence-summary${priceEvidenceSummary.reviewCount > 0 || priceEvidenceSummary.untrackedCount > 0 ? " review" : " clear"}`} aria-label="구매 목록 가격 출처" data-testid="purchase-list-price-evidence-summary"><div className="purchase-list-price-evidence-summary-heading"><div><strong>가격 출처</strong><small>가격 숫자와 실제 구매 가능한 현재 판매가는 다를 수 있습니다.</small></div><span>{priceEvidenceSummary.reviewCount + priceEvidenceSummary.untrackedCount > 0 ? `${priceEvidenceSummary.reviewCount + priceEvidenceSummary.untrackedCount}개 재확인` : "확인 완료"}</span></div><div className="purchase-list-price-evidence-facts"><span className="confirmed">확인 가격 <b>{priceEvidenceSummary.confirmedCount}개</b></span><span className="reference">참고 가격 <b>{priceEvidenceSummary.referenceCount}개</b></span><span className="recorded">예전 가격 <b>{priceEvidenceSummary.recordedCount}개</b></span><span className="unknown">미확인 <b>{priceEvidenceSummary.unknownCount + priceEvidenceSummary.untrackedCount}개</b></span></div>{priceEvidenceSummary.reviewCount > 0 && <p><FiInfo /> 참고 가격·예전 가격은 예산 참고용으로만 사용하고, 구매 전 상품 페이지에서 현재 가격·재고·유통 조건을 확인해 주세요.</p>}{priceEvidenceSummary.untrackedCount > 0 && <p><FiInfo /> {priceEvidenceSummary.untrackedCount}개 행은 가격 출처가 기록되지 않은 이전 데이터입니다. 실제 상품 페이지를 확인한 뒤 목록을 새로 만들어 주세요.</p>}</section>}
      {rows.length > 0 && <section className={dataReviewRows.length > 0 ? "purchase-list-data-review-queue" : "purchase-list-data-review-queue clear"} aria-label="구매 목록 정보 다시 확인 목록" data-testid="purchase-list-data-review-queue"><div className="purchase-list-data-review-queue-heading"><div><strong>스펙·상품 페이지 검토 목록</strong><small>{dataReviewRows.length > 0 ? "오래됐거나 확인 시점이 없는 행을 제조사·다나와 상품 페이지에서 다시 확인해요." : "현재 구매 목록의 데이터가 최근 확인 기준을 충족합니다."}</small></div><span>{dataReviewRows.length}개</span></div><div className="purchase-list-data-review-queue-actions"><button className="text-button" type="button" data-testid="purchase-list-data-review-filter" onClick={() => setPriceFilter("data_review")} disabled={dataReviewRows.length === 0}>검토 행만 보기</button>{onRefreshAll && dataRefreshTargets.length > 0 && <button className="button button-small button-light" type="button" data-testid="purchase-list-data-review-refresh" onClick={() => onRefreshAll(dataRefreshTargets)} disabled={refreshingItemId !== null}>{refreshingItemId !== null ? <><FiRefreshCw className="spin" /> 정보 확인 중...</> : <><FiRefreshCw /> 정보 다시 확인 {dataRefreshTargets.length}개</>}</button>}</div>{dataReviewRows.length > dataRefreshTargets.length && <small className="purchase-list-data-review-queue-note">정보를 다시 확인할 수 있는 항목 {dataRefreshTargets.length}개 · 출처 또는 식별자 확인이 필요한 항목 {dataReviewRows.length - dataRefreshTargets.length}개</small>}</section>}
      ...
      <p className="purchase-list-note"><FiInfo /> 구매 완료 상태는 이 견적 입력·카탈로그 기준·검사 버전에 묶어 브라우저에 저장합니다. 저장 견적 소유자는 현재 상태를 서버에 명시적으로 저장하거나 서버 상태를 불러올 수 있으며, 차이가 있을 때 자동 병합하지 않습니다. 현재 가격 확인은 다나와 상품 페이지가 연결된 항목을 실제 페이지에서 순차 확인하고, 그 외 항목은 저장된 카탈로그 가격으로 구분해 구매 목록 금액만 갱신합니다. 기준 단가·가격 추적 기준값·서버 진행률은 자동으로 바꾸지 않습니다. CSV와 복사 목록에는 가격을 확인하지 못한 항목을 임의로 0원 처리하지 않으며, 주변 부품 금액은 핵심 부품 합계와 분리해 표시합니다.</p>
    </section>
  );
}
