...
function priceHistoryServerSyncLabelFor(local: PurchaseListPriceHistory, server: PurchaseListPriceHistory) {
  ...
  if (localIncoming.newObservationCount === 0 && serverIncoming.newObservationCount === 0 && localIncoming.mergedObservationCount === serverIncoming.mergedObservationCount) return "로컬과 서버 기록이 같습니다";
  const labels: string[] = [];
  if (localIncoming.newObservationCount > 0) labels.push(`로컬 새 샘플 ${localIncoming.newObservationCount}개`);
  if (serverIncoming.newObservationCount > 0) labels.push(`서버 새 샘플 ${serverIncoming.newObservationCount}개`);
  return labels.join(" · ") || "로컬·서버 기록을 확인하세요";
}
...
function syncStateLabel(state: "synced" | "local-only" | "server-only" | "diverged") {
  if (state === "synced") return "로컬과 서버가 일치합니다";
  if (state === "local-only") return "브라우저에만 완료 항목이 있습니다";
  if (state === "server-only") return "서버에만 완료 항목이 있습니다";
  return "로컬과 서버가 서로 다릅니다";
}
...
export function PurchaseListPanel({ rows, storageKey, inputFingerprint, budgetWon, savedBuildId, savedBuildOwnerToken, onCopy, onDownload, focusStatus, onProgressChange, onServerProgressChange, onServerPriceHistoryChange, onWatchEntry, isWatchedEntry, onOpenCatalogItem, onRefreshAll, refreshingItemId, catalogRefreshReport }: { rows: PurchaseListRow[]; storageKey: string; inputFingerprint: string; budgetWon?: number; savedBuildId?: string; savedBuildOwnerToken?: string; onCopy: (checkedIds?: ReadonlySet<string>, rows?: PurchaseListRow[], itemStates?: ReadonlyArray<PurchaseListItemStatus>) => void; onDownload: (checkedIds?: ReadonlySet<string>, rows?: PurchaseListRow[], itemStates?: ReadonlyArray<PurchaseListItemStatus>) => void; focusStatus?: PurchaseItemStatus; onProgressChange?: (progress: PurchaseListExecutionProgress) => void; onServerProgressChange?: (progress?: SavedBuildPurchaseProgress) => void; onServerPriceHistoryChange?: (history?: SavedBuildPurchasePriceHistory) => void; onWatchEntry?: (target: PurchaseListWatchTarget) => boolean; isWatchedEntry?: (target: Pick<PurchaseListWatchTarget, "kind" | "itemId">) => boolean; onOpenCatalogItem?: (row: PurchaseListRow) => void; onRefreshAll?: (targets: RefreshTarget[]) => void; refreshingItemId?: string | null; catalogRefreshReport?: CatalogRefreshReport | null }) {
  ...
  const priceFilterOptions: Array<[PurchaseListPriceFilter, string]> = [["all", "전체"], ["remaining", "구매 예정"], ["changed", "가격 변동"], ["decreased", "가격 인하"], ["increased", "가격 상승"], ["needs_review", "가격 재확인 필요"], ["data_review", "데이터 확인 필요"]];
  ...
  const livePriceSourceSummary = [livePriceSummary.sourceConfirmedCount > 0 ? `원문 확인 ${livePriceSummary.sourceConfirmedCount}개` : "", livePriceSummary.catalogConfirmedCount > 0 ? `저장 카탈로그 ${livePriceSummary.catalogConfirmedCount}개` : ""].filter(Boolean).join(" · ");
  ...
  function clearPriceHistory() {
    ...
    setPriceHistory({});
    setPriceHistoryTransferPreview(null);
    setActionMessage("이 견적의 브라우저 가격 확인 기록을 초기화했습니다. 현재 가격 확인 결과는 그대로 유지됩니다.");
  }
  ...
  function downloadPriceHistory() {
    ...
    setActionMessage("이 견적의 가격 확인 기록을 JSON으로 저장했습니다.");
  }
  ...
  function downloadPriceHistoryCsv() {
    ...
    setActionMessage("이 견적의 가격 확인 기록을 CSV로 저장했습니다.");
  }
  ...
  async function importPriceHistory(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > LOCAL_IMPORT_MAX_BYTES) {
      setPriceHistoryTransferPreview(null);
      setActionMessage("가격 확인 기록 JSON은 1MB 이하 파일만 가져올 수 있습니다.");
      return;
    }
    try {
      ...
      setPriceHistoryTransferPreview(null);
      setActionMessage("가격 확인 기록 JSON 파일을 읽지 못했습니다.");
    }
  }
  ...
  function applyPriceHistoryTransferPreview() {
    ...
    if (!purchaseListPriceHistoryTransferMatchesCurrentFor(rowKeys, priceHistoryTransferPreview.rowKeys)) {
      setPriceHistoryTransferPreview(null);
      setActionMessage("현재 견적의 구매 목록 행이 달라 가격 확인 기록을 가져올 수 없습니다. 같은 견적에서 다시 내보내 주세요.");
      return;
    }
    ...
    setPriceHistoryTransferPreview(null);
    setActionMessage(`가격 확인 기록을 병합했습니다. 새 샘플 ${diff.newObservationCount}개 · 병합 후 ${diff.mergedObservationCount}개${priceHistoryTransferPreview.ignoredRowKeys.length > 0 ? ` · 현재 없는 행 ${priceHistoryTransferPreview.ignoredRowKeys.length}개는 제외` : ""}`);
  }
  ...
  async function refreshLivePrices(targets: Array<{ row: PurchaseListRow; index: number }> = rows.map((row, index) => ({ row, index })), refreshMode: "all" | "review" = "all") {
    ...
    try {
      ...
      const actionLabel = refreshMode === "review" ? "가격 재확인 완료" : "현재 가격 확인 완료";
      const sourceSummary = [summary.sourceConfirmedCount > 0 ? `원문 확인 ${summary.sourceConfirmedCount}개` : "", summary.catalogConfirmedCount > 0 ? `저장 카탈로그 ${summary.catalogConfirmedCount}개` : ""].filter(Boolean).join(" · ");
  ...
  function downloadServerPriceHistoryCsv() {
    ...
    setActionMessage("서버 가격 확인 이력과 revision을 CSV로 저장했습니다.");
  }
  ...
  function loadServerPriceHistory() {
    ...
    setPriceHistoryTransferPreview(null);
    setActionMessage(`저장 견적 서버에서 가격 확인 이력 revision ${serverPriceHistory.revision} 상태를 불러왔습니다.`);
  }
  ...
  async function syncServerPriceHistory() {
    ...
    try {
      ...
      setActionMessage(`가격 확인 이력을 저장 견적 서버에 저장했습니다. revision ${saved.purchasePriceHistory?.revision ?? "?"}`);
    } catch (reason: unknown) {
      if (!isCurrent()) return;
      if (reason instanceof ApiError && reason.status === 409 && apiErrorCode(reason) === "PURCHASE_PRICE_HISTORY_CONFLICT") {
        setActionMessage("서버 가격 확인 이력이 먼저 변경되어 저장을 막았습니다. 최신 서버 상태를 다시 확인했으며, 브라우저 기록은 바꾸지 않았습니다.");
        setServerProgressRefreshNonce((current) => current + 1);
      } else {
        setServerPriceHistoryError(reason instanceof Error ? reason.message : "가격 확인 이력을 서버에 저장하지 못했습니다.");
      }
    } finally {
  ...
  async function restoreServerPriceHistory() {
    ...
    try {
      ...
      if (!isCurrent()) return;
      if (!restored) {
        setServerPriceHistoryError("선택한 가격 확인 이력을 복원했지만 서버 상태를 확인하지 못했습니다.");
        setServerProgressRefreshNonce((current) => current + 1);
        return;
      }
      ...
      setPriceHistoryTransferPreview(null);
      setActionMessage(`서버 가격 확인 이력 revision ${targetRevision} 상태를 새 revision ${restored.revision}으로 복원했습니다. 브라우저 기록도 복원된 상태에 맞췄습니다.`);
    } catch (reason: unknown) {
      if (!isCurrent()) return;
      ...
      if (reason instanceof ApiError && reason.status === 409 && (code === "PURCHASE_PRICE_HISTORY_CONFLICT" || code === "PURCHASE_PRICE_HISTORY_HISTORY_UNAVAILABLE")) {
        setActionMessage("서버 가격 확인 이력이 먼저 변경되어 복원을 막았습니다. 최신 서버 상태를 다시 확인했으며, 브라우저 기록은 바꾸지 않았습니다.");
        setServerProgressRefreshNonce((current) => current + 1);
      } else {
        setServerPriceHistoryError(reason instanceof Error ? reason.message : "가격 확인 이력을 복원하지 못했습니다.");
      }
    } finally {
  ...
  async function restoreServerProgress() {
    ...
    try {
      ...
      setTransferPreview(null);
      setActionMessage(`서버 구매 진행률 revision ${targetRevision} 상태를 새 revision ${restored.revision}으로 복원했습니다. 브라우저 상태도 복원된 상태에 맞췄습니다.`);
    } catch (reason: unknown) {
      if (!isCurrent()) return;
      const code = apiErrorCode(reason);
  ...
  const budgetStatusText = !budgetSummary ? "" : !budgetSummary.priceComplete ? "가격 확인 필요" : budgetSummary.deltaWon === 0 ? "예산에 맞음" : budgetSummary.withinBudget ? `${Math.abs(budgetSummary.deltaWon ?? 0).toLocaleString("ko-KR")}원 여유` : `${Math.abs(budgetSummary.deltaWon ?? 0).toLocaleString("ko-KR")}원 초과`;
      ...
      : serverPriceHistory && serverPriceHistory.inputFingerprint !== inputFingerprint
        ...
        : serverPriceHistory && !serverPriceHistoryRowsMatch
          ...
          : serverPriceHistory && serverPriceHistoryMatches
            ? `서버 revision ${serverPriceHistory.revision} · 기록 ${Object.keys(serverPriceHistory.priceHistory).length}개 · ${formatSyncTime(serverPriceHistory.updatedAt)}`
            : "아직 서버에 가격 확인 이력이 없음";
  ...
  return (
    <section className="purchase-list-panel" aria-label="구매 목록" data-testid="purchase-list-panel" tabIndex={-1}>
      <div className="purchase-list-heading">
        <div><p className="eyebrow">PURCHASE LIST</p><h2>구매 목록</h2><p>선택한 핵심·주변 부품을 구매용 목록으로 정리합니다.</p></div>
        ...
        <div className="purchase-list-actions"><button className="button button-small button-light" type="button" onClick={() => onCopy(checkedIdSet, displayRows, currentItemStatesForPersistence())}><FiCopy /> 목록 복사</button><button className="button button-small button-light" type="button" onClick={() => onDownload(checkedIdSet, displayRows, currentItemStatesForPersistence())}><FiDownload /> CSV 저장</button>{scopedView && <><button className="button button-small button-light" type="button" data-testid="purchase-list-visible-copy" onClick={copyVisibleRows} disabled={visibleExportRows.length === 0}><FiCopy /> 표시 목록 복사</button><button className="button button-small button-light" type="button" data-testid="purchase-list-visible-csv" onClick={downloadVisibleRows} disabled={visibleExportRows.length === 0}><FiDownload /> 표시 목록 CSV</button></>}<button className="button button-small button-light" type="button" data-testid="purchase-list-live-price-refresh" title="다나와 원문 연결이 있는 항목은 원문을 다시 확인하고, 그 외 항목은 저장 카탈로그 가격을 확인합니다." onClick={() => void refreshLivePrices()} disabled={livePriceLoading || livePriceCooldownRemainingSeconds > 0}>{livePriceLoading ? <><FiRefreshCw className="spin" /> 가격 확인 중...</> : livePriceCooldownRemainingSeconds > 0 ? <><FiClock /> {livePriceCooldownRemainingSeconds}초 후 다시 확인</> : <><FiRefreshCw /> 현재 가격 확인</>}</button></div>
      </div>
      {(livePriceCheckedAt || priceHistoryRowCount > 0) && <p className="purchase-list-live-price-status" data-testid="purchase-list-live-price-status" role="status">{livePriceCheckedAt ? <>현재 가격 확인 {formatSyncTime(livePriceCheckedAt)} · 확인 {livePriceSummary.availableCount}개 · 변동 {livePriceSummary.changedCount}개{livePriceSourceSummary ? ` · ${livePriceSourceSummary}` : ""}{livePriceSummary.discoveredCount > 0 ? ` · 새 가격 ${livePriceSummary.discoveredCount}개` : ""}{livePriceSummary.unavailableCount > 0 ? ` · 확인 불가 ${livePriceSummary.unavailableCount}개` : ""}{livePriceSummary.errorCount > 0 ? ` · 실패 ${livePriceSummary.errorCount}개` : ""}</> : "이 견적의 브라우저 가격 확인 기록"}{priceHistoryRowCount > 0 && <> · 기록 {priceHistoryRowCount}개</>}{priceHistoryRowCount > 0 && <button className="purchase-list-price-history-clear" type="button" onClick={clearPriceHistory}>확인 기록 초기화</button>}</p>}
      ...
      {rows.length > 0 && <section className={priceReviewRows.length > 0 ? "purchase-list-price-review" : "purchase-list-price-review clear"} aria-label="구매 목록 가격 재확인 목록" data-testid="purchase-list-price-review"><div className="purchase-list-price-review-heading"><div><strong>{livePriceCheckedAt ? "가격 재확인 목록" : "가격 확인 목록"}</strong><small>{livePriceCheckedAt ? (priceReviewRows.length > 0 ? "현재 가격을 확인하지 못한 행만 다시 조회합니다." : "현재 구매 목록의 가격 확인이 완료되었습니다.") : "아직 현재 가격을 확인하지 않은 구매 행을 모아 조회합니다."}</small></div><span>{priceReviewRows.length}개</span></div><div className="purchase-list-price-review-actions"><button className="text-button" type="button" data-testid="purchase-list-price-review-filter" onClick={() => setPriceFilter("needs_review")} disabled={priceReviewRows.length === 0}>검토 행만 보기</button><button className="button button-small button-light" type="button" data-testid="purchase-list-price-review-refresh" onClick={() => void refreshLivePrices(priceReviewRows, "review")} disabled={livePriceLoading || livePriceCooldownRemainingSeconds > 0 || priceReviewRows.length === 0}>{livePriceLoading ? <><FiRefreshCw className="spin" /> 확인 중...</> : livePriceCooldownRemainingSeconds > 0 ? <><FiClock /> {livePriceCooldownRemainingSeconds}초 후 다시 확인</> : <><FiRefreshCw /> {livePriceCheckedAt ? "미확인 행 다시 조회" : "가격 확인 시작"}</>}</button></div></section>}
      <section className="purchase-list-data-freshness" aria-label="구매 목록 부품 데이터 상태" data-testid="purchase-list-data-freshness">
        <div className="purchase-list-data-freshness-heading"><div><strong>부품 데이터 상태</strong><small>가격이 있어도 오래된 스펙은 구매 전에 다시 확인하세요.</small></div><button className="purchase-list-data-freshness-action" type="button" data-testid="purchase-list-data-review-action" aria-pressed={priceFilter === "data_review"} onClick={() => setPriceFilter("data_review")} disabled={dataFreshnessCounts.needsReview === 0}>{dataFreshnessCounts.needsReview}개 확인 필요</button></div>
        <div className="purchase-list-data-freshness-facts"><span className="fresh">{DATA_FRESHNESS_LABELS.fresh} <b>{dataFreshnessCounts.fresh}개</b></span><span className="aging">{DATA_FRESHNESS_LABELS.aging} <b>{dataFreshnessCounts.aging}개</b></span><span className="stale">{DATA_FRESHNESS_LABELS.stale} <b>{dataFreshnessCounts.stale}개</b></span><span className="unknown">{DATA_FRESHNESS_LABELS.unknown} <b>{dataFreshnessCounts.unknown}개</b></span></div>
      </section>
      {rows.length > 0 && <section className={`purchase-list-price-evidence-summary${priceEvidenceSummary.reviewCount > 0 || priceEvidenceSummary.untrackedCount > 0 ? " review" : " clear"}`} aria-label="구매 목록 가격 출처" data-testid="purchase-list-price-evidence-summary"><div className="purchase-list-price-evidence-summary-heading"><div><strong>가격 출처</strong><small>가격 숫자와 실제 구매 가능한 현재 판매가는 다를 수 있습니다.</small></div><span>{priceEvidenceSummary.reviewCount + priceEvidenceSummary.untrackedCount > 0 ? `${priceEvidenceSummary.reviewCount + priceEvidenceSummary.untrackedCount}개 재확인` : "확인 완료"}</span></div><div className="purchase-list-price-evidence-facts"><span className="confirmed">확인 가격 <b>{priceEvidenceSummary.confirmedCount}개</b></span><span className="reference">참고 가격 <b>{priceEvidenceSummary.referenceCount}개</b></span><span className="recorded">예전 가격 <b>{priceEvidenceSummary.recordedCount}개</b></span><span className="unknown">미확인 <b>{priceEvidenceSummary.unknownCount + priceEvidenceSummary.untrackedCount}개</b></span></div>{priceEvidenceSummary.reviewCount > 0 && <p><FiInfo /> 참고 가격·예전 가격는 예산 참고용으로만 사용하며, 구매 전 상품 원문에서 현재 가격·재고·유통 조건을 확인하세요.</p>}{priceEvidenceSummary.untrackedCount > 0 && <p><FiInfo /> {priceEvidenceSummary.untrackedCount}개 행은 가격 출처가 기록되지 않은 이전 데이터입니다. 원문을 확인한 뒤 목록을 새로 생성하세요.</p>}</section>}
      {rows.length > 0 && <section className={dataReviewRows.length > 0 ? "purchase-list-data-review-queue" : "purchase-list-data-review-queue clear"} aria-label="구매 목록 원문 재확인 목록" data-testid="purchase-list-data-review-queue"><div className="purchase-list-data-review-queue-heading"><div><strong>스펙·원문 검토 목록</strong><small>{dataReviewRows.length > 0 ? "오래됐거나 확인 시점이 없는 행을 제조사·다나와 원문 기준으로 다시 확인합니다." : "현재 구매 목록의 데이터가 최근 확인 기준을 충족합니다."}</small></div><span>{dataReviewRows.length}개</span></div><div className="purchase-list-data-review-queue-actions"><button className="text-button" type="button" data-testid="purchase-list-data-review-filter" onClick={() => setPriceFilter("data_review")} disabled={dataReviewRows.length === 0}>검토 행만 보기</button>{onRefreshAll && dataRefreshTargets.length > 0 && <button className="button button-small button-light" type="button" data-testid="purchase-list-data-review-refresh" onClick={() => onRefreshAll(dataRefreshTargets)} disabled={refreshingItemId !== null}>{refreshingItemId !== null ? <><FiRefreshCw className="spin" /> 원문 확인 중...</> : <><FiRefreshCw /> 원문 재확인 {dataRefreshTargets.length}개</>}</button>}</div>{dataReviewRows.length > dataRefreshTargets.length && <small className="purchase-list-data-review-queue-note">원문 재확인 가능한 항목 {dataRefreshTargets.length}개 · 출처 또는 식별자 확인이 필요한 항목 {dataReviewRows.length - dataRefreshTargets.length}개</small>}</section>}
      ...
      <div className="purchase-list-progress" data-testid="purchase-list-progress">
        ...
        {savedBuildId && <div className="purchase-list-server-sync" data-testid="purchase-list-server-sync">
          <div><strong>저장 견적 구매 진행률</strong><small>{serverProgressStatusText}{savedBuildOwnerToken ? " · owner 인증 가능" : " · 읽기 전용"}</small><div className="purchase-list-server-price-history-copy"><strong>가격 확인 이력 서버</strong><small>{serverPriceHistoryStatusText}{serverPriceHistorySyncSummary ? ` · ${serverPriceHistorySyncSummary}` : ""}{savedBuildOwnerToken ? " · owner 인증 가능" : " · 읽기 전용"}</small></div></div>
          <div className="purchase-list-server-sync-actions">{savedBuildId && <button className="text-button" type="button" data-testid="purchase-list-server-refresh" onClick={refreshServerProgress} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}><FiRefreshCw className={serverProgressLoading ? "spin" : undefined} /> 서버 상태 새로고침</button>}{serverProgressMatches && <button className="text-button" type="button" onClick={loadServerProgress} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}><FiDownload /> 서버 상태 불러오기</button>}{serverSyncAvailable && <button className="text-button" type="button" onClick={() => void syncServerProgress()} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}>{serverProgressSyncing ? <><FiRefreshCw className="spin" /> 저장 중...</> : <><FiServer /> 현재 상태 서버 저장</>}</button>}{serverPriceHistory && <button className="text-button" type="button" data-testid="purchase-list-price-history-server-csv-export" onClick={downloadServerPriceHistoryCsv} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}><FiDownload /> 서버 이력 CSV</button>}{serverPriceHistoryMatches && <button className="text-button" type="button" data-testid="purchase-list-price-history-server-load" onClick={loadServerPriceHistory} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}><FiDownload /> 가격 이력 불러오기</button>}{serverPriceHistorySyncAvailable && <button className="text-button" type="button" data-testid="purchase-list-price-history-server-save" onClick={() => void syncServerPriceHistory()} disabled={serverProgressLoading || serverProgressSyncing || serverProgressRestoring || serverPriceHistorySaving}>{serverPriceHistorySaving ? <><FiRefreshCw className="spin" /> 이력 저장 중...</> : <><FiServer /> 가격 이력 서버 저장</>}</button>}</div>
