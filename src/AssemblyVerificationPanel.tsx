...
export function AssemblyVerificationPanel({ storageKey, savedBuildId, savedBuildOwnerToken, savedVerificationHistory, onServerSync, onSummaryChange }: { storageKey: string; savedBuildId?: string; savedBuildOwnerToken?: string; savedVerificationHistory?: ReadonlyArray<AssemblyVerificationSavedSnapshot>; onServerSync?: (saved: SavedBuild) => void; onSummaryChange?: (summary: AssemblyVerificationSurfaceSummary) => void }) {
  ...
  return <section className={`assembly-verification-panel ${state}`} aria-label="실제 조립 확인 로그" data-testid="assembly-verification-panel" data-storage-key={storageKey}>
    <div className="assembly-verification-heading"><div><p className="eyebrow">REAL BUILD LOG</p><h2>실제 조립 확인 로그</h2><p>호환성 검사와 별도로, 조립 후 실제 부팅·인식·온도·소음 결과를 이 견적에 기록합니다. 이 기록은 자동으로 호환 결과를 통과시키지 않습니다.</p></div><span className={`assembly-verification-state ${state}`}>{state === "failed" ? <FiXCircle /> : state === "passed" ? <FiCheckCircle /> : state === "in_progress" ? <FiAlertTriangle /> : <FiInfo />} {assemblyVerificationStateLabel(state)}</span></div>
    {serverSnapshotRestored && <div className="assembly-verification-server-restored" data-testid="assembly-verification-server-restored" role="status"><FiInfo /><div><strong>저장된 조립 확인 요약을 이 기기에 복원했습니다.</strong><small>서버에는 공유 가능한 compact 요약만 있어 원본 측정 시계열·개별 메모는 포함되지 않습니다. 상세 그래프가 필요하면 원본 JSON/CSV를 같은 견적에서 가져오세요.</small></div></div>}
    ...
    <p className="assembly-verification-note"><FiInfo /> 실측 로그는 이 브라우저의 현재 견적 식별자에 묶어 저장합니다. 다른 부품·수량·추천 기준·카탈로그 기준의 견적에는 자동으로 재사용하지 않습니다. 측정 CSV는 인식된 유효 행의 온도·팬 RPM 최대값과 주변 온도 평균값으로 집계하며, 이 값도 자동 안전·위험 판단이 아니라 재확인 정보로만 사용합니다.</p>
