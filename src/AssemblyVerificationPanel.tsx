...
export function AssemblyVerificationPanel({ storageKey, savedBuildId, savedBuildOwnerToken, savedVerificationHistory, onServerSync, onSummaryChange }: { storageKey: string; savedBuildId?: string; savedBuildOwnerToken?: string; savedVerificationHistory?: ReadonlyArray<AssemblyVerificationSavedSnapshot>; onServerSync?: (saved: SavedBuild) => void; onSummaryChange?: (summary: AssemblyVerificationSurfaceSummary) => void }) {
  ...
  return <section className={`assembly-verification-panel ${state}`} aria-label="실제 조립 확인 로그" data-testid="assembly-verification-panel" data-storage-key={storageKey}>
    ...
    {log.measurementSeries && log.measurementSeries.length > 1 && <section className="assembly-verification-telemetry-analysis" aria-label="시계열 관찰 해석" data-testid="assembly-verification-telemetry-analysis"><div className="assembly-verification-subheading"><strong>시계열 관찰 해석</strong><span>관찰값 요약 · 안전/위험 자동 판별 아님</span></div><div className="assembly-verification-telemetry-analysis-grid">{(["cpuTempC", "gpuTempC", "cpuFanRpm", "gpuFanRpm", "cpuUsagePercent", "gpuUsagePercent", "cpuClockMHz", "gpuClockMHz", "cpuPowerW", "gpuPowerW"] as const).map((metric) => { const analysis = telemetryAnalysis.metrics[metric]; return analysis ? <article key={metric}><div><strong>{telemetryMetricLabel(metric)}</strong><span>{telemetryTrendText(analysis.trend)}</span></div><p>{telemetryAnalysisMetricText(analysis)}</p><small>상승 단계 {analysis.positiveStepCount}/{analysis.stepCount} · 유효 샘플 {analysis.sampleCount}개</small></article> : null; })}</div><p className="assembly-verification-telemetry-analysis-note"><FiInfo /> 변화율은 시간 열이 있는 경우에만 계산합니다. `상승`은 첫 유효 샘플보다 마지막 유효 샘플이 높다는 관찰이며, 특정 온도가 안전하다는 보증이나 고장 결과가 아닙니다.</p></section>}
