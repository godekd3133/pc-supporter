...
function deltaText(value: number | undefined, comparable: boolean | undefined, blockReason?: string) {
  if (value === undefined) return comparable === false ? blockReason === "measurement-quality-review" || blockReason === "measurement-continuity-gapped" ? "측정 품질 확인" : blockReason === "condition-missing" ? "조건 기록 없음" : "조건 변경" : "-";
...
function loadSegmentDurationText(segment: AssemblyVerificationLoadSegment) {
  if (segment.durationSeconds === undefined) return "시간축 기록 없음";
...
function loadSegmentObservationText(segment: AssemblyVerificationLoadSegment) {
  ...
  return [usage, temperature, power, thermal, stability].filter((value): value is string => Boolean(value)).join(" · ") || "센서값 기록 없음";
}
...
function decisionStatusText(status: AssemblyVerificationDecisionDimensionStatus | "improved" | "unchanged" | "recheck" | "inconclusive") {
  return status === "improved" ? "개선 관찰" : status === "unchanged" ? "변화 없음" : status === "recheck" ? "재정보 부족" : status === "inconclusive" ? "비교 불충분" : status === "observational-lower" ? "관찰상 낮음" : status === "observational-higher" ? "관찰상 높음" : "정보 부족";
}

function measurementQualityStatusText(status: AssemblyVerificationMeasurementQuality["status"]) {
  return status === "complete" ? "입력 품질 양호" : status === "partial" ? "부분 측정" : "정보 부족";
}
...
export function AssemblyVerificationPanel({ storageKey, savedBuildId, savedBuildOwnerToken, savedVerificationHistory, onServerSync, onSummaryChange }: { storageKey: string; savedBuildId?: string; savedBuildOwnerToken?: string; savedVerificationHistory?: ReadonlyArray<AssemblyVerificationSavedSnapshot>; onServerSync?: (saved: SavedBuild) => void; onSummaryChange?: (summary: AssemblyVerificationSurfaceSummary) => void }) {
  ...
  function addRun() {
    ...
    const run = { ...nextRun, runId, runLabel: `조립 확인 ${history.runs.length + 1}회차`, createdAt };
  ...
  async function importLog(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (parsed.errors.length > 0 || !parsed.history) {
        setImportError(parsed.errors.join(" ") || "조립 확인 이력을 가져오지 못했습니다.");
        return;
      }
      ...
      setCsvImportPreview(null);
      setResetPending(false);
      setMessage(`${parsed.history.runs.length}회차 조립 확인 이력을 가져왔습니다.`);
    } catch {
      setImportError("조립 확인 로그 JSON 파일을 읽지 못했습니다.");
    }
  }

  return <section className={`assembly-verification-panel ${state}`} aria-label="실제 조립 확인 로그" data-testid="assembly-verification-panel" data-storage-key={storageKey}>
    <div className="assembly-verification-heading"><div><h2>실제 조립 확인 로그</h2><p>조립 후 부팅·인식·온도·소음을 기록합니다. 이 기록으로 호환성 검사를 통과 처리하진 않습니다.</p></div><span className={`assembly-verification-state ${state}`}>{state === "failed" ? <FiXCircle /> : state === "passed" ? <FiCheckCircle /> : state === "in_progress" ? <FiAlertTriangle /> : <FiInfo />} {assemblyVerificationStateLabel(state)}</span></div>
    {serverSnapshotRestored && <div className="assembly-verification-server-restored" data-testid="assembly-verification-server-restored" role="status"><FiInfo /><div><strong>저장된 조립 확인 요약을 이 기기에 복원했습니다.</strong><small>서버에서 복원한 기록은 요약본입니다. 그래프·메모가 필요하면 원본 JSON·CSV를 이 견적에 불러오세요.</small></div></div>}
    <div className="assembly-verification-progress"><div><span>현재 회차 실측 확인 진행률</span><strong>{progress.checked} / {progress.total}개</strong><em>{progress.percent}%</em></div><div className="assembly-verification-progress-track" role="progressbar" aria-label={`현재 조립 확인 ${progress.percent}% 완료`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress.percent}><span style={{ width: `${progress.percent}%` }} /></div><small>{progress.failed > 0 ? `실패 확인 ${progress.failed}개 · 원인을 해결한 뒤 다시 기록하세요.` : progress.remaining > 0 ? `미확인 ${progress.remaining}개 · 조립 후 실제 결과를 기록하세요.` : "모든 항목에 결과가 기록됐습니다."}</small></div>
