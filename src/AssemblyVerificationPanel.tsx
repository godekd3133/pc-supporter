...
export function AssemblyVerificationPanel({ storageKey, savedBuildId, savedBuildOwnerToken, savedVerificationHistory, onServerSync, onSummaryChange }: { storageKey: string; savedBuildId?: string; savedBuildOwnerToken?: string; savedVerificationHistory?: ReadonlyArray<AssemblyVerificationSavedSnapshot>; onServerSync?: (saved: SavedBuild) => void; onSummaryChange?: (summary: AssemblyVerificationSurfaceSummary) => void }) {
  ...
  return <section className={`assembly-verification-panel ${state}`} aria-label="실제 조립 확인 로그" data-testid="assembly-verification-panel" data-storage-key={storageKey}>
    ...
    {history.runs.length > 0 && <section className="assembly-verification-report-export" aria-label="실측 리포트 내보내기" data-testid="assembly-verification-report-export"><div className="assembly-verification-subheading"><strong>실측 리포트</strong><span>{trendFilter === "same-load" ? "같은 부하 조건" : "전체 회차"} · 원본 CSV 제외</span></div><p>현재 비교 필터의 회차·조건·대표값·델타·다시 볼 항목를 검토용 파일로 저장합니다. 원본 CSV는 포함하지 않고 압축 시계열 포인트 수와 출처 메타데이터만 기록합니다.</p><div className="assembly-verification-report-actions"><button className="button button-light" type="button" onClick={() => exportReport("markdown")} disabled={syncing}><FiDownload /> Markdown 저장</button><button className="button button-light" type="button" onClick={() => exportReport("csv")} disabled={syncing}><FiDownload /> CSV 저장</button><button className="button button-light" type="button" onClick={() => exportReport("json")} disabled={syncing}><FiDownload /> 요약 JSON 저장</button></div></section>}
