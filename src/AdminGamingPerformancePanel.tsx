...
export function AdminGamingPerformancePanel({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <section className="admin-card gaming-performance-evidence-admin-card" data-testid="admin-gaming-performance-evidence">
    <div className="admin-card-heading"><div><p className="eyebrow">GAME FPS DATA</p><h3>게임별 FPS 자료 관리</h3></div><FiActivityIcon /></div>
    ...
    <p className="gaming-performance-evidence-admin-note"><FiInfo /> 평균 FPS가 목표 프레임 이상이어도 특정 패치·드라이버·온도·모니터 환경을 절대적으로 보장하지 않습니다. 저장은 확인한 원본 자료를 제품 추천에 연결하는 작업입니다.</p>
  </section>;
}
