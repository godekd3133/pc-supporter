...
export function AdminGamingPerformancePanel({ onToast }: { onToast: (message: string) => void }) {
  ...
  function applyRequestJson() {
    ...
    setRecordFilter("all");
    onToast(`${request.gameIds.length}개 게임의 측정 요청 조건을 coverage 필터에 적용했습니다.`);
  }
  ...
  function appendEvidenceRecord(record: GamingPerformanceEvidenceRecord) {
    ...
    if (!current.valid) {
      setError("현재 JSON에 오류가 있어 새 레코드를 추가하지 않았습니다. 먼저 저장 전 검증으로 오류를 확인해 주세요.");
      return;
    }
    setError(null);
    setValidation(null);
  ...
  return <section className="admin-card gaming-performance-evidence-admin-card" data-testid="admin-gaming-performance-evidence">
    <div className="admin-card-heading"><div><p className="eyebrow">GAME FPS EVIDENCE</p><h3>게임별 FPS 자료 관리</h3></div><FiActivityIcon /></div>
    <p className="admin-card-description">실제 측정 자료를 게임·해상도·목표 프레임·그래픽 옵션·선택 GPU와 연결합니다. 저장 전 모든 레코드의 GPU ID·측정값·측정일·HTTPS 출처를 확인하며, 자료가 없으면 사용자 결과는 계속 확인 필요로 남습니다.</p>
    ...
    {(requestGameIdsForDisplay.length > 0 || requestGpuPartIdForDisplay) && <div className="gaming-performance-evidence-admin-request-context" data-testid="gaming-performance-evidence-request-context"><strong>MEASUREMENT REQUEST</strong><span>{appliedRequest ? "붙여넣은 측정 요청 조건이 coverage에 적용됨" : "사용자 결과에서 전달된 exact-condition 요청"}</span><small>{requestGameIdsForDisplay.length > 0 ? `게임 · ${requestGameIdsForDisplay.join(" · ")}` : "게임 조건 확인 필요"}{requestGpuPartIdForDisplay ? ` · GPU · ${requestGpuPartIdForDisplay}` : " · GPU 조건 확인 필요"}</small></div>}
    ...
    <p className="gaming-performance-evidence-admin-note"><FiInfo /> 평균 FPS가 목표 프레임 이상이어도 특정 패치·드라이버·온도·모니터 환경을 절대적으로 보장하지 않습니다. 저장은 검증된 원본 자료를 제품 추천에 연결하는 작업입니다.</p>
  </section>;
}
