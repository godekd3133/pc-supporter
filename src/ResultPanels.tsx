  ...
  return <div className="result-page stale-result-page">
    ...
    <section className="stale-result-hero" role="alert">
      ...
      <div>
        ...
        <h1>견적이 바뀌었어요.</h1>
        <p>새 구성의 호환 결과를 확인해 주세요. 이전 추천은 현재 견적에 적용되지 않아요.</p>
        <small>이전 결과 · {new Date(lastCheckedAt).toLocaleString("ko-KR", { dateStyle: "medium", timeStyle: "short" })}</small>

      </div>
      <button className="button button-primary" onClick={onCheck} disabled={checking}>{checking ? <><FiLoader className="spin" /> 호환 결과 계산 중...</> : <><FiActivity /> 호환 결과 보기</>}</button>
    </section>
...
export function BuildScenarioPreviewPanel({ preview, currentResult, onApply, onRetry, onClose }: { preview: BuildScenarioPreviewState; currentResult: CompatibilityResult; onApply: () => void; onRetry: () => void; onClose: () => void }) {
  ...
  return <section className={`build-scenario-preview ${comparison.direction}`} aria-label="구성 미리 보기" data-testid="build-scenario-preview">
    ...
    <div className="build-scenario-preview-actions"><button className="button button-light" type="button" onClick={onClose}>계속 비교</button><button className="button button-primary" type="button" onClick={onApply}><FiActivity /> 이 구성으로 바꾸기</button></div>
    <p className="build-scenario-preview-note"><FiInfo /> 미리보기는 현재 견적을 바꾸지 않아요. 이 구성을 선택하면 견적과 호환 결과가 함께 바뀝니다.</p>
...
export function BuildHealthPanel({ metrics, gpuSelected, psuSelected, caseSelected }: { metrics: BuildMetrics; gpuSelected: boolean; psuSelected: boolean; caseSelected: boolean }) {
  const healthItems = [
    {
      label: "전력 여유",
      value: metrics.powerHeadroomW === undefined ? "정보 부족" : `${metrics.powerHeadroomW.toLocaleString("ko-KR")}W`,
      detail: metrics.psuWattageW !== undefined && metrics.recommendedPsuW !== undefined ? `${metrics.psuWattageW}W 파워 · 권장 ${metrics.recommendedPsuW}W` : "파워 용량 정보 부족",
      Icon: FiZap,
    ...
    {
      label: "M.2 슬롯",
      value: metrics.m2Used === undefined ? "정보 부족" : `${metrics.m2Used}개 사용`,
      detail: metrics.m2Used !== undefined && metrics.m2SlotsTotal !== undefined ? `${metrics.m2Used} / ${metrics.m2SlotsTotal} 슬롯` : "M.2 슬롯 정보 없음",
      Icon: FiHardDrive,
    ...
    {
      label: "PSU 장착 여유",
      value: metrics.psuClearanceMm === undefined ? psuSelected && caseSelected ? "정보 부족" : "미선택" : `${metrics.psuClearanceMm}mm`,
...
export function upgradeCompatibilityText(evidence: UpgradeCompatibilityEvidence) {
  ...
  return details.length > 0 ? details.join(" · ") : "확장 여유 정보 부족";
}
...
export function upgradeBudgetText(evidence: UpgradeBudgetEvidence | undefined) {
  ...
  if (!evidence.priceComplete || evidence.afterCoreTotalPriceWon === undefined) {
    return `목표 ${formatWon(evidence.budgetWon)} · 핵심 금액 정보 부족`;
  }
...
export function UpgradeRecommendationPanel({ recommendations, onApply, onPreview, onWatchPart }: { recommendations: UpgradeRecommendation[]; onApply: (recommendation: UpgradeRecommendation) => void; onPreview: (recommendation: UpgradeRecommendation) => void; onWatchPart: PartWatchHandler }) {
  ...
  return <section className="upgrade-recommendation-panel" data-testid="upgrade-recommendation-panel" data-upgrade-sort={sortMode} tabIndex={-1}>
    ...
    <div className="upgrade-recommendation-heading"><div><h2>업그레이드 추천</h2><p>현재 구성보다 성능·용량·확장성 지표가 나아지는 부품 후보예요.</p><small className="upgrade-recommendation-sort-note">최대 3개까지 비교할 수 있어요.</small></div><span className="upgrade-recommendation-icon"><FiZap /></span></div>
...
export function accessoryRecommendationSpecRows(category: AccessoryCategory) {
  ...
  if (category === "cooling_fan") {
    rows.push(
      ...
      { label: "팬 개수", value: (item) => item.specs.fanCount === undefined ? "정보 부족" : `${item.specs.fanCount}개` },
      { label: "팬 소비전류", value: (item) => item.specs.fanCurrentA === undefined ? "정보 부족" : `${item.specs.fanCurrentA.toFixed(2)}A/팬` },
      { label: "RGB 장치 전압", value: (item) => item.specs.rgbDeviceVoltage ?? "정보 부족" }
    );
  } else if (category === "storage_accessory" || category === "m2_heatsink") {
    rows.push({ label: "인터페이스", value: (item) => item.specs.interface ?? "정보 부족" });
    if (category === "storage_accessory") {
      rows.push(
        { label: "어댑터 동시 장착 최대", value: (item) => item.specs.adapterStorageDeviceCount === undefined ? "정보 부족" : `${item.specs.adapterStorageDeviceCount}개` },
        { label: "PCIe 요구 슬롯 폭", value: (item) => item.specs.adapterPcieSlotWidth === undefined ? "정보 부족" : `x${item.specs.adapterPcieSlotWidth}` }
      );
    }
    rows.push(
      { label: "지원 규격", value: (item) => item.specs.supportedFormFactors?.join(" · ") || item.specs.formFactor || "정보 부족" },
      { label: "폼팩터", value: (item) => item.specs.formFactor ?? "정보 부족" }
    );
  } else if (category === "thermal_grease") {
    rows.push({ label: "열전도율", value: (item) => item.specs.thermalConductivityWmK === undefined ? "정보 부족" : `${item.specs.thermalConductivityWmK.toFixed(1)} W/mK` });
    ...
    rows.push({ label: "패드 두께", value: (item) => item.specs.thicknessMm === undefined ? "정보 부족" : `${item.specs.thicknessMm}mm` });
  } else if (category === "fan_hub") {
    rows.push(
      { label: "팬 출력 포트", value: (item) => item.specs.fanPortCount === undefined ? "정보 부족" : `${item.specs.fanPortCount}개` },
      { label: "RGB 출력 포트", value: (item) => item.specs.rgbPortCount === undefined ? "정보 부족" : `${item.specs.rgbPortCount}개` },
      { label: "RGB 전압", value: (item) => item.specs.rgbDeviceVoltage ?? "정보 부족" },
      { label: "외부 출력", value: (item) => item.specs.outputW === undefined ? "정보 부족" : `${item.specs.outputW}W` }
    );
  } else if (category === "ups") {
    rows.push(
      { label: "출력", value: (item) => item.specs.outputW === undefined ? "정보 부족" : `${item.specs.outputW}W` },
      { label: "용량", value: (item) => item.specs.capacityVa === undefined ? "정보 부족" : `${item.specs.capacityVa}VA` },
      { label: "콘센트", value: (item) => item.specs.outletCount === undefined ? "정보 부족" : `${item.specs.outletCount}개` }
    );
  } else {
    rows.push(
      { label: "규격", value: (item) => item.specs.formFactor ?? "정보 부족" },
...
export function AccessoryRecommendationPanel({ recommendations, selectedAccessories, onAddAccessory, onWatchAccessory, isAccessoryWatched }: { recommendations: AccessoryRecommendation[]; selectedAccessories: AccessorySelection[]; onAddAccessory: (item: AccessoryItem) => void; onWatchAccessory?: (item: AccessoryItem, targetPriceWon?: number) => boolean; isAccessoryWatched?: (item: AccessoryItem) => boolean }) {
  ...
  return <section className="peripheral-recommendation-panel" data-testid="peripheral-recommendation-panel">
    <div className="peripheral-recommendation-heading"><div><h2>현재 구성에 맞는 주변 부품</h2><p>현재 견적에 더할 수 있는 주변 부품을 추천해요.</p></div><span className="peripheral-recommendation-icon"><FiTool /></span></div>
...
export function CompatibilityMap({ links, findings = [], onFocusFinding }: { links: CompatibilityLink[]; findings?: Finding[]; onFocusFinding?: (ruleId: string) => void }) {
  ...
  return <section className="compatibility-map" data-testid="compatibility-map" tabIndex={-1}>
    ...
    <p className="compatibility-map-note"><FiInfo /> “문제 있음”은 차단 오류 또는 주의 항목이 연결된 상태이며, “정보 부족”는 현재 카탈로그 정보만으로는 알 수 없는 상태예요.</p>
  </section>;
}
