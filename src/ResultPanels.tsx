...
import { type RefreshTarget, uniqueRefreshTargets } from "../shared/refresh-targets";
...
import { type AccessoryCategory, type AccessoryItem, type AccessoryRecommendation, type AccessorySelection, type BuildAnalysis, type BuildAnalysisBalance, type BuildAnalysisInsight, type BuildDataHealth, type BuildSelection, type BuildMetrics, type CompatibilityLink, type CompatibilityResult, type Finding, type M2SlotAssignment, type Part, type PartCategory, type RecommendationPlan, type RecommendationPreferences, type RecommendationTrustEvidence, type UpgradeCompatibilityEvidence, type UpgradeExpansionEvidence, type UpgradeBudgetEvidence, type UpgradeRecommendation, ACCESSORY_CATEGORIES, ACCESSORY_CATEGORY_LABELS, BENCHMARK_SOURCE_KIND_LABELS, CATEGORY_LABELS, DATA_FRESHNESS_LABELS, DATA_QUALITY_LABELS, GAMING_REFRESH_RATE_LABELS, GAMING_RESOLUTION_LABELS, GAMING_RESOLUTION_VRAM_TARGETS, isKnownPrice, LISTING_TYPE_LABELS, PART_CATEGORIES } from "../shared/types";
...
export function DataHealthPanel({ health, partMap, accessoryMap, onRefresh, onRefreshAll, refreshingPartId }: { health: BuildDataHealth; partMap: ReadonlyMap<string, Part>; accessoryMap: ReadonlyMap<string, AccessoryItem>; onRefresh: (target: RefreshTarget) => void; onRefreshAll: (targets: RefreshTarget[]) => void; refreshingPartId: string | null }) {
...
export function BuildAnalysisPanel({ analysis }: { analysis: BuildAnalysis }) {
...
export function BuildBalancePanel({ balance }: { balance: BuildAnalysisBalance }) {
...
export function BuildAnalysisInsights({ strengths, focusAreas }: { strengths: BuildAnalysisInsight[]; focusAreas: BuildAnalysisInsight[] }) {
...
export const recommendationTrustLabels: Record<RecommendationTrustEvidence["level"], string> = { high: "높음", medium: "보통", low: "낮음" };

export const recommendationFreshnessLabels: Record<RecommendationTrustEvidence["freshness"], string> = { fresh: "최근 갱신", aging: "갱신 권장", stale: "오래된 데이터", unknown: "갱신 시점 불명" };

export function recommendationTrustText(trust: RecommendationTrustEvidence | undefined) {
  return trust ? `${recommendationTrustLabels[trust.level]} ${trust.score}점` : "산정 불가";
}

export function recommendationTrustDetail(trust: RecommendationTrustEvidence) {
