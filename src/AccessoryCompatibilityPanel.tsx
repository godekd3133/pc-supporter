...
export function AccessoryCompatibilityPanel({ result, onEdit, onAssignHubTarget }: { result: AccessoryCompatibilityResult; onEdit?: () => void; onAssignHubTarget?: (fanId: string, hubId: string) => void }) {
  ...
  return <section className={`accessory-compatibility-panel ${result.status}`} aria-label="주변 부품 호환 점검" data-testid="accessory-compatibility-panel">
    ...
    {result.fanHubTargetRecommendations && result.fanHubTargetRecommendations.length > 0 && <section className="accessory-connection-plans accessory-hub-target-recommendations" aria-label="팬 허브 연결 대상 추천"><div className="accessory-connection-plans-heading"><div><strong>팬 허브 연결 대상 추천</strong><small>포트·커넥터·전류·외부 전원 정보를 비교한 후보입니다. 추천이 있어도 실제 케이블 규격은 원문에서 확인하세요.</small></div><span>{result.fanHubTargetRecommendations.length}개 팬</span></div><div className="accessory-connection-plans-list">{result.fanHubTargetRecommendations.map((recommendation) => <FanHubTargetRecommendationCard key={recommendation.fanId} recommendation={recommendation} onAssign={onAssignHubTarget} />)}</div></section>}
