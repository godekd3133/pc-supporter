...
function statusLabel(status: AccessoryCompatibilityResult["status"]) {
  return status === "compatible" ? "호환 가능" : status === "incompatible" ? "주변 부품 호환 불가" : "주변 부품 정보 부족";
}

function planStatusLabel(status: AccessoryConnectivityPlan["status"]) {
  return status === "pass" ? "연결 가능" : status === "blocked" ? "연결 불가" : "연결 정보 부족";
}
...
function planCurrentText(plan: AccessoryConnectivityPlan) {
  ...
  return plan.maxFanCurrentA !== undefined ? `허브 최대 ${plan.maxFanCurrentA.toFixed(2)}A · 팬 전류 정보 없음` : "전류 정보 없음";
}

function powerRailRoleLabel(role: AccessoryPowerRail["role"]) {
  return role === "fan" ? "팬" : role === "rgb" ? "RGB" : role === "shared" ? "공용" : "역할 정보 없음";
}

function powerRailText(powerRails?: AccessoryPowerRail[]) {
  if (!powerRails || powerRails.length === 0) return "전원 레일 정보 없음";
  return powerRails.map((rail) => {
    ...
    return `${rail.voltage} ${powerRailRoleLabel(rail.role)}${measurements ? ` · ${measurements}` : " · 용량 정보 없음"}`;
  }).join(" / ");
}

function planPortText(plan: AccessoryConnectivityPlan) {
  if (plan.portIssue === "unknown") return "허브 포트 정보 없음";
...
function rgbLoadText(plan: AccessoryRgbConnectionPlan) {
  if (plan.rgbLoadStatus === "unknown") return plan.rgbPerDeviceCurrentA !== undefined || plan.rgbPerDevicePowerW !== undefined ? "컨트롤러 레일 용량 정보 없음" : "장치당 부하 정보 없음";
...
function rgbLoadEvidenceText(plan: AccessoryRgbConnectionPlan) {
  ...
  return "RGB 장치 소비전력 정보가 없으면 5V/12V 레일 여유를 계산하지 않습니다.";
}
...
function FanHubTargetRecommendationCard({ recommendation, onAssign }: { recommendation: AccessoryFanHubTargetRecommendation; onAssign?: (fanId: string, hubId: string) => void }) {
  return <article className="accessory-hub-target-recommendation"><div className="accessory-hub-target-recommendation-heading"><div><strong>{recommendation.fanName}</strong><small>추가 팬 {recommendation.fanCount}개 · 연결 대상 미지정</small></div><span>{recommendation.recommendedHubId ? "추천 부품 있음" : recommendation.suggestedHubId ? "우선 부품 있음" : "허브 비교 필요"}</span></div><p>{recommendation.summary}</p><div className="accessory-hub-target-candidates">{recommendation.candidates.length === 0 ? <small>현재 선택한 허브가 없습니다.</small> : recommendation.candidates.map((candidate) => <div key={candidate.hubId}><div><strong>{candidate.hubName}</strong><span className={candidate.status}>{targetCandidateStatusLabel(candidate.status)}</span></div><small>{candidate.reason}</small>{onAssign && candidate.status !== "blocked" && <button className="text-button" type="button" onClick={() => onAssign(recommendation.fanId, candidate.hubId)}>{candidate.hubId === recommendation.recommendedHubId ? "추천 허브 지정" : candidate.hubId === recommendation.suggestedHubId ? "우선 부품 지정" : "이 허브 지정"}</button>}</div>)}</div></article>;
}
...
function AccessoryConnectionPlanCard({ plan }: { plan: AccessoryConnectivityPlan }) {
  return <article className={`accessory-connection-plan ${plan.status}`}>
    <div className="accessory-connection-plan-heading"><div><strong>{plan.hubName}</strong><small>허브 출력 · {plan.hubFanOutputs.join(" · ") || "정보 부족"}</small></div><span>{planStatusLabel(plan.status)}</span></div>
    <div className="accessory-connection-plan-facts"><div><span>팬 연결 수</span><strong>{plan.fanCount}개</strong></div><div><span>포트 배치</span><strong>{planPortText(plan)}</strong></div><div><span>외부 전원</span><strong>{plan.externalPower ?? "정보 부족"}</strong></div><div><span>전류 계획</span><strong>{planCurrentText(plan)}</strong></div><div><span>전원 레일</span><strong title={powerRailText(plan.powerRails)}>{powerRailText(plan.powerRails)}</strong></div></div>
    ...
    <div className="accessory-connection-plan-fans">{plan.fans.map((fan) => <div key={fan.accessoryId}><span>{fan.name}{fan.quantity > 1 ? ` ×${fan.quantity}` : ""}{fan.unitCount > 1 ? ` · ${fan.unitCount}개/상품` : ""}</span><small>{fan.connectorTypes.join(" · ")}{fan.currentA !== undefined ? ` · ${fan.currentA.toFixed(2)}A/팬` : " · 소비전류 미등록"}</small></div>)}</div>
    <p>{plan.summary}</p>
  </article>;
}

function AccessoryRgbConnectionPlanCard({ plan }: { plan: AccessoryRgbConnectionPlan }) {
  return <article className={`accessory-connection-plan ${plan.status}`}><div className="accessory-connection-plan-heading"><div><strong>{plan.controllerName}</strong><small>RGB 출력 · {plan.controllerOutputs.join(" · ")}</small></div><span>{planStatusLabel(plan.status)}</span></div><div className="accessory-connection-plan-facts"><div><span>RGB 연결 장치</span><strong>{plan.deviceCount}개</strong></div><div><span>연결 구성</span><strong>{plan.additionalFanDeviceCount ? `케이스 ${plan.caseDeviceCount ?? 0} + 추가 팬 ${plan.additionalFanDeviceCount}` : "케이스 기본 RGB"}</strong></div><div><span>필요 전압</span><strong>{plan.requiredVoltages.join(" + ") || "정보 부족"}</strong></div><div><span>외부 전원</span><strong>{plan.externalPower ?? "정보 부족"}</strong></div><div><span>출력 포트</span><strong>{plan.outputCount === undefined ? "정보 부족" : `${plan.outputCount}개`}</strong></div><div><span>RGB 부하 계획</span><strong>{rgbLoadText(plan)}</strong></div><div><span>전원 레일</span><strong title={powerRailText(plan.powerRails)}>{powerRailText(plan.powerRails)}</strong></div></div>{plan.devices && plan.devices.length > 0 && <div className="accessory-connection-plan-rgb-devices"><span>RGB 부하 대상</span>{plan.devices.map((device) => <div key={device.id}><strong>{device.kind === "case" ? "케이스" : "추가 팬"}</strong><span>{device.name} · {device.count}개{device.voltage ? ` · ${device.voltage}` : " · 전압 정보 없음"}</span></div>)}</div>}<p>{plan.summary}</p></article>;
}
