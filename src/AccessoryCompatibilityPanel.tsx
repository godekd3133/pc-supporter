...
function planCurrentText(plan: AccessoryConnectivityPlan) {
  ...
  return plan.maxFanCurrentA !== undefined ? `허브 최대 ${plan.maxFanCurrentA.toFixed(2)}A · 팬별 전류 확인 필요` : "전류 정보 확인 필요";
}
...
function AccessoryConnectionPlanCard({ plan }: { plan: AccessoryConnectivityPlan }) {
  return <article className={`accessory-connection-plan ${plan.status}`}>
    ...
    <div className="accessory-connection-plan-facts"><div><span>팬 연결 수</span><strong>{plan.fanCount}개</strong></div><div><span>포트 배치</span><strong>{planPortText(plan)}</strong></div><div><span>외부 전원</span><strong>{plan.externalPower ?? "확인 필요"}</strong></div><div><span>전류 계획</span><strong>{planCurrentText(plan)}</strong></div><div><span>전원 레일 (페이지 표기)</span><strong title={powerRailText(plan.powerRails)}>{powerRailText(plan.powerRails)}</strong></div></div>
...
export function AccessoryCompatibilityPanel({ result, onEdit, onAssignHubTarget }: { result: AccessoryCompatibilityResult; onEdit?: () => void; onAssignHubTarget?: (fanId: string, hubId: string) => void }) {
  ...
  return <section className={`accessory-compatibility-panel ${result.status}`} aria-label="주변 부품 호환 점검" data-testid="accessory-compatibility-panel">
    ...
    {result.fanHubTargetRecommendations && result.fanHubTargetRecommendations.length > 0 && <section className="accessory-connection-plans accessory-hub-target-recommendations" aria-label="팬 허브 연결 대상 추천"><div className="accessory-connection-plans-heading"><div><strong>팬 허브 연결 대상 추천</strong><small>포트·커넥터·전류·외부 전원 정보를 비교한 부품이에요. 추천이 있어도 실제 케이블 규격은 제조사 페이지에서 확인하세요.</small></div><span>{result.fanHubTargetRecommendations.length}개 팬</span></div><div className="accessory-connection-plans-list">{result.fanHubTargetRecommendations.map((recommendation) => <FanHubTargetRecommendationCard key={recommendation.fanId} recommendation={recommendation} onAssign={onAssignHubTarget} />)}</div></section>}
    ...
    {!hasFindings && <div className="accessory-compatibility-empty"><FiCheckCircle /><div><strong>현재 선택한 주변 부품에서 확인된 충돌이 없습니다.</strong><p>표시 가능한 주변 부품 규격 기준을 통과했습니다. 실제 장착 위치·나사 간격·케이블 경로는 제조사 페이지와 실물 조립 전에 다시 확인해 주세요.</p></div></div>}
    ...
    <p className="accessory-compatibility-note"><FiInfo /> 이 결과는 핵심 부품 호환 결과와 별도의 주변 부품 점검입니다. 표시되지 않은 커넥터·허브 전류·케이블 경로는 임의로 호환된다고 추정하지 않습니다.</p>
  </section>;
}
