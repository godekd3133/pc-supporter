...
return (
  <div className="result-page">
    <div className="mobile-result-toolbar"><button className="mobile-result-back" type="button" onClick={onBack} aria-label="홈으로"><FiArrowLeft /></button><div><strong>호환 결과</strong></div><button className="mobile-result-share" type="button" onClick={onCopyResultLink} aria-label="견적 링크 복사"><FiShare2 /></button></div>
      ...
      <div className="result-actions">
        ...
        <details className="result-more-tools" ref={moreToolsRef}>
          ...
          <div onClick={() => { if (moreToolsRef.current) moreToolsRef.current.open = false; }}>
            ...
            <button className="button button-light result-link-copy-button" type="button" onClick={onCopyResultLink}><FiShare2 /> 견적 링크 복사</button>
            <button className="button button-light" onClick={onCopyReport}><FiCopy /> 견적 정보 복사</button>
            <button className="button button-light" onClick={onDownloadReport}><FiDownload /> 견적 정보 JSON 저장</button>
    ...
    <section className={`result-hero ${displayStatus}`}>
      <div className="result-hero-main"><span className="result-status-icon">{displayStatus === "compatible" ? <FiCheckCircle /> : displayStatus === "needs_review" ? <FiAlertTriangle /> : <FiXCircle />}</span><div><h1>{statusCopy}</h1><p>{statusDescription}</p>{decisionNote && <div className="result-decision-note" data-testid="result-decision-note"><FiInfo /><div><span>선택 메모</span><strong>{decisionNote}</strong></div></div>}</div></div>
    </section>
    {savedVersionContext && <section className="result-version-context" data-testid="result-version-context" aria-label="저장 견적 버전 정보"><FiGitBranch /><div><span>저장 견적 버전</span><strong>v{savedVersionContext.versionNumber} · {savedVersionContext.parentName ? `${savedVersionContext.parentName}에서 파생` : "첫 저장 견적"}</strong><small>저장 당시의 호환 결과는 히스토리에서 다른 버전과 비교할 수 있어요.</small></div>{onOpenHistory && <button className="text-button" type="button" data-testid="result-version-context-open-history" onClick={onOpenHistory}>버전 비교 열기 <FiExternalLink /></button>}</section>}
