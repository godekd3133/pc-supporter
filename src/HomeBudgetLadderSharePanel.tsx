...
export function HomeBudgetLadderSharePanel({ entries, onCopy, onRemove, onToast }: { entries: BudgetLadderLocalShareEntry[]; onCopy: (entry: BudgetLadderLocalShareEntry) => void; onRemove: (id: string) => void; onToast: (message: string) => void }) {
  ...
  <p className="home-budget-ladder-shares-note"><FiInfo /> <strong>상태 새로 확인</strong>은 서버의 공유 상태를 다시 조회합니다. <strong>이력 삭제</strong>는 이 브라우저의 기록만 지웁니다. 링크를 닫으려면 <strong>공유 취소</strong>를 선택하세요.{entries.length > visibleEntries.length && !showingAll ? " 최근 5개를 표시합니다. 전체 이력에서 나머지를 확인하세요." : ""}</p>
  </section>
