...
export function HomeBudgetLadderSharePanel({ entries, onCopy, onRemove, onToast }: { entries: BudgetLadderLocalShareEntry[]; onCopy: (entry: BudgetLadderLocalShareEntry) => void; onRemove: (id: string) => void; onToast: (message: string) => void }) {
  ...
  <p className="home-budget-ladder-shares-note"><FiInfo /> <strong>서버 상태를 확인해요</strong>는 현재 표시되는 링크를 다시 조회해요. <strong>이력에서 지워요</strong>는 이 브라우저 목록만 정리하고, 내가 만든 링크를 서버에서 막으려면 <strong>공유를 취소해요</strong>를 사용하세요.{entries.length > visibleEntries.length && !showingAll ? " 최근 5개만 보여드려요. 전체 이력 보기로 나머지를 확인할 수 있어요." : ""}</p>
  </section>
