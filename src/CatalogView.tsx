...
const CANDIDATE_SCOPE_LABELS: Record<CatalogCandidateScope, string> = { safe: "안전 부품", no_blocker: "차단 없음", precision: "전체 정밀" };
...
function candidateScopeDescription(scope: CatalogCandidateScope) {
  if (scope === "safe") return "새 문제를 만들지 않는 부품만 보여줘요.";
  if (scope === "no_blocker") return "막히는 오류는 없지만 확인할 항목이 남을 수 있는 부품까지 보여줘요.";
  return "안전·확인 필요·차단 부품을 모두 평가해 보여줘요. 차단 부품은 적용하지 않아요.";
}

function candidateScopeResultLabel(scope: CatalogCandidateScope) {
  return scope === "safe" ? "안전 부품" : scope === "no_blocker" ? "차단 없음 부품" : "전체 검사 부품";
}
...
function catalogComparisonCandidatesFor(parts: CatalogPart[]): AlternativeComparisonCandidate[] {
  return parts.map((part) => {
    ...
    return {
      ...
      performance: "현재 견적 적용 전 관찰 가능한 핵심 스펙 비교",
      compatibility: "현재 견적 적용 전 · 호환 검사 필요",
...
function CatalogSimilarityEvidencePanel({ part }: { part: CatalogPart }) {
  ...
  return <section className="catalog-similarity-evidence" aria-label={`${part.name} 성능 비교 정보`} data-testid="catalog-similarity-evidence"><div className="catalog-similarity-evidence-heading"><div><strong>성능 비교 정보</strong><small>{part.similarityLabel ?? "비교"} {part.similarityScore}점 · {similarityBasisLabelFor(evidence)} · {evidence.comparedDimensions} / {evidence.totalDimensions}개 지표</small></div><span>{evidence.confidence === "high" ? "정보 충분" : evidence.confidence === "limited" ? "정보 부족" : "정보 확인 필요"}</span></div>{dimensions.length > 0 ? <div className="catalog-similarity-evidence-table-wrap"><table><caption>현재 선택 부품과 비교 부품의 성능·확인 스펙 비교</caption><thead><tr><th scope="col">지표</th><th scope="col">현재</th><th scope="col">비교 부품</th><th scope="col">일치 점수</th></tr></thead><tbody>{dimensions.map((dimension) => <tr key={dimension.key}><th scope="row">{dimension.label}</th><td>{dimension.currentValue}{dimension.source === "model_reference" && <small className="similarity-dimension-source">모델 참조</small>}</td><td>{dimension.candidateValue}</td><td>{dimension.score}점</td></tr>)}</tbody></table></div> : <p className="catalog-similarity-evidence-empty">현재 부품과 비교 부품에서 함께 확인된 지표가 없어요. 호환 조건과 실제 스펙을 먼저 확인하세요.</p>}{part.performanceSummary && <p className="catalog-similarity-evidence-summary">{part.performanceSummary}</p>}{evidence.reference && <p className="catalog-similarity-evidence-reference" data-testid="catalog-similarity-reference"><FiInfo /> <span>{similarityReferenceTextFor(evidence)}. 선택 부품에 직접 확인된 값은 유지하고, 비어 있던 모델 공통 지표만 참조값으로 보완했습니다.</span></p>}{evidence.notes?.filter((note) => !(evidence.reference && note.startsWith("현재 선택 부품에 없는"))).map((note) => <small className="catalog-similarity-evidence-note" key={note}><FiInfo /> {note}</small>)}<small className="catalog-similarity-evidence-disclaimer"><FiInfo /> 점수는 저장된 카탈로그 정보의 상대 비교이며 실제 FPS·작업 시간·게임 성능을 보장하지 않습니다.</small></section>;
}

function catalogBenchmarkStatusLabel(status: BenchmarkEvidencePart["status"]) {
  return status === "complete" ? "점수 전부 있음" : status === "partial" ? "일부 점수 있음" : "점수 없음";
}
...
function catalogBenchmarkSourceCheckClass(evidence: BenchmarkEvidencePart) {
  ...
  return benchmarkSourceCheckLabelFor(evidence.sourceCheck) === "출처 확인됨" ? "verified" : "review";
}
...
function CatalogBenchmarkEvidenceSummary({ part }: { part: CatalogPart }) {
  ...
  const sourceSummary = evidence.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 없음";
  return <><em className={`catalog-benchmark-evidence-summary ${evidence.status}`} data-testid="catalog-benchmark-evidence-summary"><FiActivity /> 벤치마크 · {catalogBenchmarkStatusLabel(evidence.status)} {evidence.presentCount}/{evidence.totalCount}</em><em className={`catalog-benchmark-evidence-points ${evidence.status}`} title={`${scoreSummary} · ${sourceSummary} · ${benchmarkSourceCheckLabelFor(evidence.sourceCheck)}`}>{scoreSummary} · {sourceSummary} · {benchmarkSourceCheckLabelFor(evidence.sourceCheck)}</em></>;
}
...
function CatalogBenchmarkEvidence({ part }: { part: CatalogPart }) {
  ...
  const sourceNote = evidence.provenance?.sourceNote?.trim() || "출처 메모 없음 · 페이지 확인 필요";
  return <section className={`catalog-benchmark-evidence ${evidence.status}`} aria-label={`${part.name} 벤치마크 정보`} data-testid="catalog-benchmark-evidence">
    <div className="catalog-benchmark-evidence-heading"><div><strong><FiActivity /> 벤치마크 상세</strong><small>{catalogBenchmarkStatusLabel(evidence.status)} · {evidence.presentCount}/{evidence.totalCount}개 점수 · 자료 {benchmarkFreshnessLabelFor(evidence.benchmarkFreshness)}</small></div><span>{evidence.category === "cpu" ? "CPU" : "GPU"}</span></div>
    ...
    <div className="catalog-benchmark-evidence-source"><div><span>점수 출처</span><strong>{evidence.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 없음"}</strong><small>{sourceNote}{evidence.provenance?.updatedAt ? ` · 자료 갱신 ${catalogBenchmarkDateLabel(evidence.provenance.updatedAt)}` : ""}</small></div>{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer"><FiExternalLink /> 점수 출처</a>}</div>
    <div className="catalog-benchmark-evidence-meta"><span className={catalogBenchmarkSourceCheckClass(evidence)}>출처 확인 · {sourceCheckLabel}{evidence.sourceCheck?.detail ? ` · ${evidence.sourceCheck.detail}` : ""}</span><small>부품 데이터 갱신 {catalogBenchmarkDateLabel(evidence.dataUpdatedAt)}</small></div>
    <p className="catalog-benchmark-evidence-note"><FiInfo /> 점수는 측정 환경에 따라 달라지는 참고값입니다. 출처가 없거나 점수가 일부만 있으면 비교 전에 실제 출처를 확인해 주세요.</p>
  </section>;
}
...
function CatalogPartCard({ part, selected, compareSelected, showCompare, onSelect, onAdd, onToggleCompare, onWatchPart, isPartWatched }: { part: CatalogPart; selected: boolean; compareSelected: boolean; showCompare: boolean; onSelect: () => void; onAdd: () => void; onToggleCompare: () => void; onWatchPart?: (part: Part) => boolean; isPartWatched?: (part: Part) => boolean }) {
  ...
  return <article className={selected ? "catalog-part-card selected" : "catalog-part-card"}>
    <button className="catalog-part-card-main" type="button" data-testid={`catalog-part-${part.id}`} onClick={onSelect}>
      ...
      <span className="catalog-part-card-copy"><strong>{part.name}</strong><small>{part.brand ?? part.model ?? sourceLabel(part.source)}</small><span>{compactSummary(part)}</span><em>{QUALITY_LABELS[part.dataQuality]} · {freshnessLabel(part)}</em><em className={`catalog-price-evidence ${catalogPriceEvidenceFor(part)}`} data-testid={`catalog-price-evidence-${part.id}`} title={catalogPriceEvidenceDescriptionFor(part)}>{catalogPriceEvidenceLabelFor(part)}</em><CatalogBenchmarkEvidenceSummary part={part} />{part.candidateRisk && <em className={`catalog-candidate-risk ${part.candidateRisk}`}>{candidateRiskLabel(part.candidateRisk)}{part.similarityScore !== undefined ? ` · 유사도 ${part.similarityScore}점` : ""}</em>}{part.decision && <em className={`catalog-candidate-decision ${part.decision.status}`}>호환 기준 · {part.decision.label}</em>}</span>
      <span className="catalog-part-card-price">{priceLabel(part.priceWon)}</span>
    </button>
    <div className="catalog-part-card-actions">{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer" aria-label={`${part.name} 상품 페이지 보기`}>상품 페이지 <FiExternalLink /></a>}{onWatchPart && <CatalogWatchButton part={part} onWatch={onWatchPart} isWatched={isPartWatched} />}{showCompare && <button className={compareSelected ? "button button-small catalog-part-compare selected" : "button button-small catalog-part-compare"} type="button" aria-pressed={compareSelected} data-testid={`catalog-compare-${part.id}`} onClick={onToggleCompare}>{compareSelected ? <><FiCheck /> 비교 중</> : <><FiLayers /> 비교</>}</button>}<button className="button button-small catalog-part-add" type="button" onClick={onAdd} disabled={selected || blocked}>{blocked ? "차단 위험 · 적용 불가" : selected ? <><FiCheck /> 현재 선택</> : <><FiPlus /> 견적에 추가</>}</button></div>
  </article>;
}
...
function CatalogPartDetail({ part, priceHistory, priceHistoryLoading, priceHistoryError, onAdd, selected, compareSelected, showCompare, onToggleCompare, onWatchPart, isPartWatched, onOpenWatchlist, onOpenBuild, onRefresh, refreshing, refreshMessage, refreshError, refreshDiffs }: { part: CatalogPart; priceHistory?: CatalogPriceHistory; priceHistoryLoading: boolean; priceHistoryError: string | null; onAdd: () => void; selected: boolean; compareSelected: boolean; showCompare: boolean; onToggleCompare: () => void; onWatchPart?: (part: Part) => boolean; isPartWatched?: (part: Part) => boolean; onOpenWatchlist?: () => void; onOpenBuild: () => void; onRefresh?: () => void; refreshing?: boolean; refreshMessage?: string | null; refreshError?: string | null; refreshDiffs?: CatalogChangeValueDiff[] | null }) {
  ...
  return <section className="catalog-detail" aria-label="선택한 부품 상세" data-testid="catalog-part-detail">
    ...
    {part.missingFields.length > 0 && <p className="catalog-detail-missing"><FiInfo /> 확인되지 않은 스펙 {part.missingFields.slice(0, 5).map((field) => catalogMissingFieldLabelFor(field)).join(", ")}{part.missingFields.length > 5 ? ` 외 ${part.missingFields.length - 5}개` : ""}</p>}
    {part.rawSpecText && <details className="catalog-detail-raw"><summary>저장된 스펙 보기</summary><p>{part.rawSpecText}</p></details>}
    <div className="catalog-detail-actions">{onWatchPart && <CatalogWatchButton part={part} onWatch={onWatchPart} isWatched={isPartWatched} />}{onOpenWatchlist && <button className="button button-light" type="button" onClick={onOpenWatchlist}><FiClock /> 가격 추적 화면</button>}{showCompare && <button className={compareSelected ? "button button-light catalog-detail-compare selected" : "button button-light catalog-detail-compare"} type="button" aria-pressed={compareSelected} onClick={onToggleCompare}>{compareSelected ? <><FiCheck /> 비교에서 제외</> : <><FiLayers /> 비교에 추가</>}</button>}<button className="button button-primary" type="button" onClick={onAdd} disabled={selected || blocked}>{blocked ? "차단 위험 · 적용 불가" : selected ? <><FiCheck /> 현재 견적에 선택됨</> : <><FiPlus /> 현재 견적에 추가</>}</button><button className="button button-light" type="button" onClick={onOpenBuild}><FiActivity /> 견적 검사로 이동</button>{part.source === "danawa" && part.sourceProductCode && part.danawaUrl && <button className="button button-light catalog-detail-refresh" type="button" data-testid="catalog-refresh-part" onClick={onRefresh} disabled={!onRefresh || refreshing}>{refreshing ? <><FiLoader className="spin" /> 정보 확인 중...</> : <><FiRefreshCw /> 정보 다시 확인</>}</button>}{sourceUrl && <a className="button button-light" href={sourceUrl} target="_blank" rel="noreferrer"><FiExternalLink /> 상품 페이지 열기</a>}</div>
    ...
    {blocked && <p className="catalog-detail-blocked-note"><FiInfo /> 전체 검사에서 차단 위험이 확인된 부품은 현재 견적에 자동 적용하지 않아요.</p>}
    <p className="catalog-detail-note"><FiInfo /> 스펙·가격·정보 상태를 보여주는 화면입니다. 견적에 추가한 뒤 호환성 검사를 실행하면 이 조합이 맞는지 확인할 수 있어요.</p>
  </section>;
}
...
function CatalogIncompleteNotice({ count, category, missingFields }: { count: number; category: PartCategory; missingFields: CatalogSpecCoverageMissingField[] }) {
  if (count <= 0) return null;
  return <div className="catalog-incomplete-notice" role="status" data-testid="catalog-incomplete-notice"><FiInfo /><div><strong>정보가 부족한 부품 {count.toLocaleString("ko-KR")}개는 안전 부품에서 제외했어요.</strong><p>필수 스펙이 부족한 부품을 호환 가능하다고 가정하지 않습니다. 정보를 채운 뒤 다시 부품 평가에 포함할 수 있어요.</p><div className="catalog-incomplete-fields">{missingFields.slice(0, 3).map((field) => <a className="catalog-incomplete-field-link" data-testid="catalog-open-missing-field" href={`/catalog?category=${encodeURIComponent(category)}&quality=incomplete&missingField=${encodeURIComponent(field.field)}`} key={field.field}>{catalogMissingFieldLabelFor(field.field)} {field.count}개</a>)}</div></div><a className="button button-small button-light" data-testid="catalog-open-incomplete" href={`/catalog?category=${encodeURIComponent(category)}&quality=incomplete`}>불완전 데이터 보기</a></div>;
}
...
function catalogBenchmarkComparisonRowsFor(parts: CatalogPart[]) {
  ...
  addRow("benchmark-status", "벤치마크", (evidence) => evidence ? `${catalogBenchmarkStatusLabel(evidence.status)} · ${evidence.presentCount}/${evidence.totalCount}` : "해당 없음");
  ...
  addRow("benchmark-source", "벤치마크 출처", (evidence) => evidence?.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 없음");
  addRow("benchmark-source-check", "출처 확인", (evidence) => evidence ? benchmarkSourceCheckLabelFor(evidence.sourceCheck) : "해당 없음");
  addRow("benchmark-freshness", "자료 갱신", (evidence) => evidence ? benchmarkFreshnessLabelFor(evidence.benchmarkFreshness) : "해당 없음");
...
function CatalogSpecComparison({ category, parts, selectedIds, compareReady, comparisonContext, onAdd, onRemove, onCompareScenarios, onShareComparison, onRevokeComparison, onToast, onClear }: { category: PartCategory; parts: CatalogPart[]; selectedIds: ReadonlySet<string>; compareReady: boolean; comparisonContext: CatalogComparisonContext; onAdd: (part: CatalogPart) => void; onRemove: (partId: string) => void; onCompareScenarios?: (category: PartCategory, parts: CatalogPart[]) => void; onShareComparison?: CatalogComparisonShareHandler; onRevokeComparison?: CatalogComparisonRevokeHandler; onToast: (message: string) => void; onClear: () => void }) {
  ...
  return <section className="catalog-spec-comparison" aria-label="카탈로그 부품 비교" data-testid="catalog-spec-comparison">
    ...
    <div className="catalog-spec-comparison-table-wrap"><table><caption>{CATEGORY_LABELS[category]} 부품의 확인 가능한 가격·스펙을 비교해요. 페이지를 이동해도 선택한 부품은 유지됩니다.</caption><thead><tr><th scope="col">비교 항목</th>{parts.map((part) => <th scope="col" key={part.id}><div className="catalog-spec-comparison-part-heading"><span>{part.name}</span><button className="text-button" type="button" data-testid={`catalog-comparison-remove-${part.id}`} onClick={() => onRemove(part.id)}>제외</button></div></th>)}</tr></thead><tbody>
      {rows.map((row) => { const changed = new Set(row.values).size > 1; return <tr className={changed ? "changed" : undefined} key={row.key}><th scope="row">{row.label}{changed && <small> 차이</small>}</th>{row.values.map((value, index) => <td className={value === "미확인" || value === "가격 확인 필요" || value === "확인 필요" || value === "출처 없음" || value === "출처 확인 필요" ? "unknown" : undefined} key={`${row.key}-${parts[index]?.id ?? index}`}>{value}</td>)}</tr>; })}
...
export function CatalogView({ meta, build, partMap, profile, gamingResolution, gamingRefreshRate, onAddPart, onCompareParts, compareReady = false, onWatchPart, isPartWatched, onOpenWatchlist, onOpenBuild, onOpenAccessories, onShareComparison, onRevokeComparison, onToast, onBack }: { meta: ServiceMeta | null; build: BuildSelection; partMap: ReadonlyMap<string, Part>; profile: RecommendationProfile; gamingResolution?: GamingResolution; gamingRefreshRate?: GamingRefreshRate; onAddPart: (part: Part) => void; onCompareParts?: (category: PartCategory, parts: CatalogPart[]) => void; compareReady?: boolean; onWatchPart?: (part: Part) => boolean; isPartWatched?: (part: Part) => boolean; onOpenWatchlist?: () => void; onOpenBuild: () => void; onOpenAccessories?: (query?: string) => void; onShareComparison?: CatalogComparisonShareHandler; onRevokeComparison?: CatalogComparisonRevokeHandler; onToast: (message: string) => void; onBack: () => void }) {
  ...
  async function refreshCatalogPart(part: CatalogPart) {
    ...
    try {
      ...
      setRefreshPartMessage(`${refreshedPart.name} 정보 확인 완료 · ${payload.changedFields.length > 0 ? `${payload.changedFields.length}개 영역 갱신` : "변경된 영역 없음"}`);
  ...
  const candidateEmptyMessage = mode !== "compatible"
    ? query.trim()
      ...
      : "조건에 맞는 부품이 없습니다."
    : candidateScope === "safe"
      ? "현재 견적에 새 위험을 만들지 않는 안전한 부품이 없어요."
      : candidateScope === "no_blocker"
        ? "현재 견적에 새 차단 오류가 없는 부품이 없어요. 안전 부품 또는 확인 필요 부품 범위를 조정해 보세요."
        : "전체 검사 결과에 맞는 부품이 없어요.";
