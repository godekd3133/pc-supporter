...
function candidateScopeDescription(scope: CatalogCandidateScope) {
  if (scope === "safe") return "새 문제를 만들지 않는 후보만 보여줍니다.";
  if (scope === "no_blocker") return "막히는 오류는 없지만 확인할 항목이 남을 수 있는 후보까지 보여줍니다.";
  return "안전·확인 필요·차단 후보를 모두 평가해 보여줍니다. 차단 후보는 적용하지 않아요.";
}
...
function catalogBenchmarkSourceCheckClass(evidence: BenchmarkEvidencePart) {
  ...
  return benchmarkSourceCheckLabelFor(evidence.sourceCheck) === "원문 확인됨" ? "verified" : "review";
}
...
function CatalogBenchmarkEvidenceSummary({ part }: { part: CatalogPart }) {
  ...
  const sourceSummary = evidence.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 미등록";
...
function CatalogBenchmarkEvidence({ part }: { part: CatalogPart }) {
  ...
  const sourceNote = evidence.provenance?.sourceNote?.trim() || "출처 메모 없음 · 원문 확인 필요";
  return <section className={`catalog-benchmark-evidence ${evidence.status}`} aria-label={`${part.name} 벤치마크 정보`} data-testid="catalog-benchmark-evidence">
    ...
    <div className="catalog-benchmark-evidence-source"><div><span>점수 출처</span><strong>{evidence.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 미등록"}</strong><small>{sourceNote}{evidence.provenance?.updatedAt ? ` · 자료 갱신 ${catalogBenchmarkDateLabel(evidence.provenance.updatedAt)}` : ""}</small></div>{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer"><FiExternalLink /> 원문</a>}</div>
    <div className="catalog-benchmark-evidence-meta"><span className={catalogBenchmarkSourceCheckClass(evidence)}>원문 확인 · {sourceCheckLabel}{evidence.sourceCheck?.detail ? ` · ${evidence.sourceCheck.detail}` : ""}</span><small>부품 데이터 갱신 {catalogBenchmarkDateLabel(evidence.dataUpdatedAt)}</small></div>
    <p className="catalog-benchmark-evidence-note"><FiInfo /> 점수는 측정 환경에 따라 달라지는 참고값입니다. 출처가 없거나 점수가 일부만 있으면 비교 전에 원문을 확인해 주세요.</p>
  </section>;
}
...
function catalogBenchmarkComparisonRowsFor(parts: CatalogPart[]) {
  ...
  addRow("benchmark-source", "벤치마크 출처", (evidence) => evidence?.provenance ? BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind] : "출처 미등록");
  addRow("benchmark-source-check", "원문 확인", (evidence) => evidence ? benchmarkSourceCheckLabelFor(evidence.sourceCheck) : "해당 없음");
...
function CatalogSpecComparison({ category, parts, selectedIds, compareReady, comparisonContext, onAdd, onRemove, onCompareScenarios, onShareComparison, onRevokeComparison, onToast, onClear }: { category: PartCategory; parts: CatalogPart[]; selectedIds: ReadonlySet<string>; compareReady: boolean; comparisonContext: CatalogComparisonContext; onAdd: (part: CatalogPart) => void; onRemove: (partId: string) => void; onCompareScenarios?: (category: PartCategory, parts: CatalogPart[]) => void; onShareComparison?: CatalogComparisonShareHandler; onRevokeComparison?: CatalogComparisonRevokeHandler; onToast: (message: string) => void; onClear: () => void }) {
  ...
  return <section className="catalog-spec-comparison" aria-label="카탈로그 부품 비교" data-testid="catalog-spec-comparison">
    ...
    <div className="catalog-spec-comparison-table-wrap"><table><caption>{CATEGORY_LABELS[category]} 후보의 관찰 가능한 가격·스펙을 비교합니다. 페이지를 이동해도 선택한 부품은 유지됩니다.</caption><thead><tr><th scope="col">비교 항목</th>{parts.map((part) => <th scope="col" key={part.id}><div className="catalog-spec-comparison-part-heading"><span>{part.name}</span><button className="text-button" type="button" data-testid={`catalog-comparison-remove-${part.id}`} onClick={() => onRemove(part.id)}>제외</button></div></th>)}</tr></thead><tbody>
      {rows.map((row) => { const changed = new Set(row.values).size > 1; return <tr className={changed ? "changed" : undefined} key={row.key}><th scope="row">{row.label}{changed && <small> 차이</small>}</th>{row.values.map((value, index) => <td className={value === "미확인" || value === "가격 확인 필요" || value === "확인 필요" || value === "출처 미등록" || value === "원문 미확인" ? "unknown" : undefined} key={`${row.key}-${parts[index]?.id ?? index}`}>{value}</td>)}</tr>; })}
