...
const ACTION_LABELS: Record<CatalogSpecReviewAction, string> = { refresh_source: "자동 원문 재확인", review_source: "수동 원문 확인", inspect_catalog: "카탈로그 상세 확인" };
const EVIDENCE_LABELS: Record<CatalogSpecReviewEvidence, string> = { all: "전체 근거", spec: "일반 스펙", pcie: "PCIe 슬롯 evidence" };
...
function refreshOutcomeText(item: CatalogSpecReviewItem) {
  if (item.refreshOutcome === "repeated_failure") return `원문 자동 확인 연속 ${item.refreshFailureStreakCount ?? item.refreshFailureCount ?? 0}회 실패 · 수동 확인 필요`;
  if (item.refreshOutcome === "retryable_failure") return `원문 자동 확인 연속 ${item.refreshFailureStreakCount ?? item.refreshFailureCount ?? 0}회 실패 · 재시도 권장`;
  if (item.refreshOutcome === "succeeded") return `원문 자동 확인 ${item.refreshAttemptCount ?? 0}회 · 최근 반영`;
  return "자동 원문 확인 전";
}
...
function CatalogSpecRefreshCategoryCoverageList({ progress }: { progress: ReturnType<typeof catalogSpecRefreshProgressSummaryFor> }) {
  ...
  if (!categories || categories.length === 0) return <div className="catalog-spec-refresh-progress-category-coverage" data-testid="catalog-spec-refresh-category-coverage"><strong>카테고리 coverage</strong><small>coverage snapshot이 기록된 실행이 생기면 카테고리별 완성도가 표시됩니다.</small></div>;
...
function CatalogSpecRefreshPcieProgressPanel({ progress }: { progress: ReturnType<typeof catalogSpecRefreshProgressSummaryFor> }) {
  ...
  return <div className="catalog-spec-refresh-pcie-progress" data-testid="catalog-spec-refresh-pcie-progress" aria-label="PCIe evidence 보강 추이">
    <div className="catalog-spec-refresh-pcie-progress-heading"><div><strong>PCIe EVIDENCE LANE</strong><small>일반 스펙 completeness와 분리한 PCIe 슬롯 원문 보강 누적입니다.</small></div><span>{latestCoverage ? "snapshot 연결" : "기록 대기"}</span></div>
...
function CatalogSpecReviewItemCard({ item, selected, onToggle, disabled }: { item: CatalogSpecReviewItem; selected: boolean; onToggle: () => void; disabled: boolean }) {
  ...
  return <article className={selected ? "catalog-spec-review-item selected" : "catalog-spec-review-item"} data-testid="catalog-spec-review-item">
    <div className="catalog-spec-review-selection"><input type="checkbox" aria-label={`${item.partName} 원문 재확인 대상`} checked={selected} disabled={!selectable || disabled} onChange={onToggle} /><span>{selectable ? "일괄 확인 가능" : "수동 확인 필요"}</span></div>
    <div className="catalog-spec-review-item-main">
      ...
      <div className="catalog-spec-review-focus-fields">{item.focusFields.length > 0 ? item.focusFields.map((field) => <span key={`${field.category}:${field.field}`} title={field.instruction}>{field.label}</span>) : <span>누락 필드 미기록 · 원문 확인 필요</span>}</div>
    </div>
    <div className="catalog-spec-review-item-actions"><a className="button button-small button-secondary" data-testid="catalog-spec-review-open" href={item.catalogUrl}><FiSearch /> {item.nextActionLabel}</a>{sourceUrl && <a className="text-button" href={sourceUrl} target="_blank" rel="noreferrer"><FiExternalLink /> 원문</a>}</div>
  </article>;
}
...
export function AdminCatalogSpecReviewPanel({ onToast }: { onToast: (message: string) => void }) {
    ...
    void api<CatalogSpecReviewWorkPackage>(`/api/admin/catalog-spec/review-package?${params.toString()}`)
      .then((value) => {
        ...
        if (value.queueChanged && offset > 0) {
          ...
          setSelectedIds([]);
          setBatchResult(null);
          onToast("카탈로그 스펙 보강 큐가 갱신되어 첫 묶음부터 다시 표시합니다.");
          return;
        }
        queueFingerprintRef.current = value.queueFingerprint;
        setReviewPackage(value);
      })
      .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "카탈로그 스펙 보강 큐를 불러오지 못했습니다."); })
  ...
  async function refreshSelected() {
    ...
    if (!window.confirm(`선택한 ${selectedIds.length}개 부품의 다나와 원문을 순차적으로 다시 확인할까요? 각 항목은 개별적으로 성공·건너뜀·실패가 기록됩니다.`)) return;
    ...
    try {
      ...
      onToast(`${result.refreshedCount}개 원문 반영 · 건너뜀 ${result.skippedCount}개 · 실패 ${result.failedCount}개`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "선택한 부품 원문 일괄 재확인에 실패했습니다.");
    } finally {
  ...
  return <section className="admin-card catalog-spec-review-card" data-testid="admin-catalog-spec-review" aria-label="카탈로그 스펙 보강 작업 큐">
    <div className="admin-card-heading catalog-spec-review-heading"><div><p className="eyebrow">SPEC ENRICHMENT QUEUE</p><h3>카탈로그 스펙 보강 작업 패키지</h3><p>누락률과 호환 영향도가 높은 필드부터 원문 확인 작업을 묶어 내보냅니다. 이 큐는 안전 후보 판정을 우회하지 않습니다.</p></div><FiToolIcon /></div>
    <form className="catalog-spec-review-controls" onSubmit={submitFilters}>
      <label><span>근거 범위</span><select aria-label="스펙 보강 큐 근거 범위" value={evidence} onChange={(event) => { queueFingerprintRef.current = null; setEvidence(event.target.value as CatalogSpecReviewEvidence); setOffset(0); setSelectedIds([]); setBatchResult(null); }}><option value="all">전체 근거</option><option value="spec">일반 스펙</option><option value="pcie">PCIe 슬롯 evidence</option></select></label>
      <label><span>범주</span><select aria-label="스펙 보강 큐 범주" value={category} onChange={(event) => { queueFingerprintRef.current = null; setCategory(event.target.value as PartCategory | "all"); setOffset(0); setSelectedIds([]); setBatchResult(null); }}><option value="all">전체 카테고리</option>{PART_CATEGORIES.map((item) => <option value={item} key={item}>{CATEGORY_LABELS[item]}</option>)}</select></label>
      <label><span>우선순위</span><select aria-label="스펙 보강 큐 우선순위" value={priority} onChange={(event) => { queueFingerprintRef.current = null; setPriority(event.target.value as CatalogSpecReviewPriority | "all"); setOffset(0); setSelectedIds([]); setBatchResult(null); }}><option value="all">전체 우선순위</option><option value="high">우선 보강</option><option value="medium">보강 권장</option><option value="low">일반 보강</option></select></label>
      <label><span>작업 유형</span><select aria-label="스펙 보강 큐 작업 유형" value={action} onChange={(event) => { queueFingerprintRef.current = null; setAction(event.target.value as CatalogSpecReviewAction | "all"); setOffset(0); setSelectedIds([]); setBatchResult(null); }}><option value="all">전체 작업</option><option value="refresh_source">자동 원문 재확인</option><option value="review_source">수동 원문 확인</option><option value="inspect_catalog">카탈로그 상세 확인</option></select></label>
      <label className="catalog-spec-review-query"><span>검색</span><input aria-label="스펙 보강 큐 검색" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="부품명·브랜드·상품코드" /></label>
      <label className="catalog-spec-review-query"><span>누락 필드</span><input aria-label="스펙 보강 큐 누락 필드" value={missingField} onChange={(event) => setMissingField(event.target.value)} placeholder="예: maxGpuLengthMm" /></label>
    ...
    {loading && !reviewPackage ? <div className="catalog-spec-review-state" role="status"><FiLoader className="spin" /> 보강 큐를 계산하는 중...</div> : error && !reviewPackage ? <div className="catalog-spec-review-state error" role="alert"><FiAlertTriangle /> <span>{error}</span><button className="text-button" type="button" onClick={() => setRetryNonce((current) => current + 1)}>다시 시도</button></div> : reviewPackage && <>
      ...
      <p className="catalog-spec-review-note"><FiInfo /> 작업 패키지는 현재 catalog snapshot의 누락 정보를 읽기 전용으로 묶습니다. 실제 값 반영은 부품 상세의 원문 재확인 또는 관리자 수집 작업 후 coverage를 다시 확인해야 합니다.</p>
