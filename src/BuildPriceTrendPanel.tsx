...
export function BuildPriceTrendPanel({ build, partMap, accessoryMap, snapshot }: { build: BuildSelection; partMap: ReadonlyMap<string, Part>; accessoryMap: ReadonlyMap<string, AccessoryItem>; snapshot: BuildPriceSnapshot }) {
  ...
  return <section className="build-price-trend-panel" aria-label="전체 견적 가격 변동 추이" data-testid="build-price-trend-panel">
    <div className="build-price-trend-heading"><div><p className="eyebrow"><FiTrendingUp /> QUOTE / SUMMARY</p><h2>견적 가격 흐름</h2><p>선택한 부품의 카탈로그 가격 변경 기록을 합쳐 현재 견적의 흐름을 보여줘요.</p></div><span>{days}일</span></div>
    <div className="build-price-trend-total"><div><span>{snapshot.priceComplete ? "현재 총견적" : "확인된 총견적"}</span><strong>{formatWon(snapshot.totalPriceWon || trend.latestPriceWon)}</strong></div>{trend.netDeltaWon !== undefined && <em className={trend.netDeltaWon > 0 ? "increased" : trend.netDeltaWon < 0 ? "decreased" : "same"}>{formatDelta(trend.netDeltaWon)}{netPercent !== undefined ? ` · ${netPercent > 0 ? "+" : ""}${netPercent.toFixed(1)}%` : ""}</em>}</div>
    ...
    {loading && !historyLoaded ? <div className="build-price-trend-state" role="status"><FiLoader className="spin" /> 견적 가격 이력을 확인하는 중...</div> : error && !historyLoaded ? <div className="build-price-trend-state error" role="status"><FiInfo /> 가격 이력을 불러오지 못했어요. 현재 합계만 표시합니다.</div> : !chartReady ? <div className="build-price-trend-state"><FiInfo /> 이 기간의 가격 변경 기록이 아직 부족해요. 기록이 쌓이면 견적 흐름을 보여드릴게요.</div> : <PriceTrendChart points={trend.points} ariaLabel={`전체 견적 ${days}일 가격 추이`} testId="build-price-trend-chart" />}
    ...
    <p className="build-price-trend-note"><FiInfo /> 카탈로그 변경 기록 기반 참고값이에요. 기록이 없는 부품은 현재 확인값을 유지해 계산하며, 실제 결제 가격·재고·배송비를 보장하지 않습니다.{trend.coveredItemCount < trend.totalItemCount ? ` 가격 기록을 확인한 부품 ${trend.coveredItemCount}/${trend.totalItemCount}개.` : ""}</p>
  </section>;
}
