...
export function BuildPriceSummaryPanel({ snapshot, budgetWon, compact = false, testId = "build-price-summary", unknownItems = [], onRefresh, refreshingItemId = null }: BuildPriceSummaryPanelProps) {
  ...
  const statusLabel = state === "complete" ? "가격 확인 완료" : state === "partial" ? "가격 일부 확인" : "선택 대기";
  const totalLabel = state === "empty" ? "구성 선택 후 계산" : state === "complete" ? "카탈로그 기준 전체 합계" : "현재 확인된 부품 소계";
  ...
  return <section className={`build-price-summary-panel ${state}${compact ? " compact" : ""}`} aria-label="구매 금액 요약" data-testid={testId}>
    ...
    {state === "partial" && <div className="build-price-summary-warning"><FiAlertTriangle /><div><strong>결제 전 가격 확인 필요</strong></div></div>}
    {state === "complete" && priceEvidenceReviewCount > 0 && <div className="build-price-summary-warning evidence-review" data-testid={`${testId}-price-evidence-review`}><FiInfo /><div><strong>실제 판매가 재확인 필요 · {priceEvidenceReviewCount}종</strong></div></div>}
    {state === "partial" && unknownItems.length > 0 && <div className="build-price-summary-unknown" data-testid={`${testId}-unknown-items`}><div className="build-price-summary-unknown-heading"><strong>가격 확인이 필요한 항목</strong><span>{unknownPriceCount}종</span></div><div className="build-price-summary-unknown-list">{unknownItems.slice(0, 4).map((item) => { const sourceUrl = safeExternalUrl(item.sourceUrl); const refreshing = refreshingItemId === item.id; return <div className="build-price-summary-unknown-item" key={`${item.kind}-${item.id}`}><div><strong>{item.name}</strong><small>{item.kind === "accessory" ? "주변 부품" : "핵심 부품"} · 가격 미확인</small></div><div className="build-price-summary-unknown-actions">{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer">상품 페이지 <FiExternalLink /></a>}{item.refreshTarget && onRefresh && <button className="text-button" type="button" onClick={() => onRefresh(item.refreshTarget!)} disabled={refreshingItemId !== null}>{refreshing ? <><FiRefreshCw className="spin" /> 확인 중...</> : <><FiRefreshCw /> 정보 다시 확인</>}</button>}</div></div>; })}</div>{unknownItems.length > 4 && <small className="build-price-summary-unknown-more">그 외 {unknownItems.length - 4}종은 데이터 상태 패널에서 확인할 수 있습니다.</small>}</div>}
