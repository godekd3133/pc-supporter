...
function sectionPriceText(value: number, complete: boolean) {
  ...
  return value > 0 ? `${formatWon(value)} · 일부 가격 정보 없음` : "가격 정보 없음";
}
...
export function BuildPriceSummaryPanel({ snapshot, budgetWon, compact = false, testId = "build-price-summary", unknownItems = [], onRefresh, refreshingItemId = null }: BuildPriceSummaryPanelProps) {
  ...
  const totalLabel = state === "empty" ? "예상 금액" : state === "complete" ? "전체 예상 금액" : "가격 정보가 있는 부품 합계";
  const totalText = state === "empty" ? "부품을 선택해 주세요" : state === "complete" ? formatWon(snapshot.totalPriceWon) : snapshot.totalPriceWon > 0 ? formatWon(snapshot.totalPriceWon) : "가격 정보 없음";
  ...
  return <section className={`build-price-summary-panel ${state}${compact ? " compact" : ""}`} aria-label="구매 금액 요약" data-testid={testId}>
    ...
    {state === "partial" && unknownItems.length > 0 && <div className="build-price-summary-unknown" data-testid={`${testId}-unknown-items`}><div className="build-price-summary-unknown-heading"><strong>가격 정보가 없는 부품</strong><span>{unknownPriceCount}종</span></div><div className="build-price-summary-unknown-list">{unknownItems.slice(0, 4).map((item) => { const sourceUrl = safeExternalUrl(item.sourceUrl); const refreshing = refreshingItemId === item.id; return <div className="build-price-summary-unknown-item" key={`${item.kind}-${item.id}`}><div><strong>{item.name}</strong><small>{item.kind === "accessory" ? "주변 부품" : "핵심 부품"} · 가격 정보 없음</small></div><div className="build-price-summary-unknown-actions">{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer">상품 페이지 <FiExternalLink /></a>}{item.refreshTarget && onRefresh && <button className="text-button" type="button" onClick={() => onRefresh(item.refreshTarget!)} disabled={refreshingItemId !== null}>{refreshing ? <><FiRefreshCw className="spin" /> 가격 불러오는 중...</> : <><FiRefreshCw /> 가격 다시 불러오기</>}</button>}</div></div>; })}</div>{unknownItems.length > 4 && <small className="build-price-summary-unknown-more">그 외 {unknownItems.length - 4}종도 가격 정보가 없어요.</small>}</div>}
