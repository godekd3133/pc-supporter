...
function SharedComparisonBenchmarkEvidence({ evidence }: { evidence: AlternativeComparisonBenchmarkEvidence }) {
  ...
  return <div className={`shared-comparison-benchmark-evidence ${evidence.status}`} data-testid="shared-comparison-benchmark-evidence" aria-label={`${evidence.name} 원본 성능 근거`}>
    ...
    <small>출처 · {evidence.provenance ? `${BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind]} · ${evidence.provenance.sourceNote}` : "출처 유형 미등록"}</small>
    <small>원문 점검 · {benchmarkSourceCheckLabelFor(evidence.sourceCheck)}{evidence.sourceCheck?.detail ? ` · ${evidence.sourceCheck.detail}` : ""}</small>
    <small>자료 · {benchmarkFreshnessLabelFor(evidence.benchmarkFreshness)} · 데이터 갱신 {evidence.dataUpdatedAt}</small>
    {sourceUrl && <a className="shared-comparison-source" href={sourceUrl} target="_blank" rel="noreferrer">성능 원문 보기 <FiExternalLink /></a>}
  </div>;
}
...
function sharedBenchmarkRecheckDetail(recheck: AlternativeComparisonBenchmarkRecheck) {
  ...
  if (recheck.status === "current_unavailable") return "현재 후보를 확인할 수 없어 공유 snapshot과 대조하지 못했습니다.";
  if (recheck.status === "current_missing") return "현재 카탈로그에 benchmark 점수가 없어 공유 당시 값과 대조가 필요합니다.";
  if (recheck.status === "current_incomplete") return "현재 카탈로그에 일부 점수만 있어 완전한 benchmark 세트로 확인하지 못했습니다.";
  const facts = [
    ...
    recheck.sourceCheckNeedsReview ? "원문 재확인 필요" : undefined,
    recheck.freshnessNeedsReview ? "자료 갱신 권장" : undefined
  ].filter((value): value is string => Boolean(value));
  return facts.length > 0 ? facts.join(" · ") : "현재 benchmark 근거를 다시 확인해야 합니다.";
}
...
function SharedComparisonBenchmarkRecheck({ recheck }: { recheck: AlternativeComparisonBenchmarkRecheck }) {
  ...
  return <div className={`shared-comparison-live-benchmark-recheck ${recheck.status}`} data-testid="shared-comparison-live-benchmark-recheck" aria-label="현재 benchmark 재확인 결과">
...
function sharedScenarioRiskText(scenario: AlternativeComparisonCandidate["scenario"]) {
  return scenario ? `차단 ${scenario.blockerCount}개 · 주의 ${scenario.warningCount}개 · 확인 필요 ${scenario.unknownCount}개` : "가상 적용 기록 없음";
}
...
function sharedScenarioTradeoffStatus(tradeoff: NonNullable<NonNullable<AlternativeComparisonCandidate["scenario"]>["tradeoff"]>) {
  return tradeoff.eligible === false ? "비교 제외" : tradeoff.frontier ? "효율 경계" : "열세";
}
...
function sharedScenarioTradeoffFacts(tradeoff: NonNullable<NonNullable<AlternativeComparisonCandidate["scenario"]>["tradeoff"]>) {
  return [
    ...
    tradeoff.evidenceScore !== undefined ? `근거 ${tradeoff.evidenceScore}점` : "근거 확인 필요"
  ].join(" · ");
}
...
function SharedLiveWatchControl({ part, onToast }: { part: Part; onToast: (message: string) => void }) {
  ...
  function register() {
    ...
    try {
      ...
      onToast("가격 추적 목록에 후보를 등록하지 못했습니다.");
    }
  }
...
function SharedComparisonBaseline({ snapshot }: { snapshot: AlternativeComparisonSnapshot }) {
  ...
  return <>
    {hasBaseline && <section className="shared-comparison-baseline" data-testid="shared-comparison-baseline" aria-label="공유 후보 비교 현재 기준선"><div><p className="eyebrow">COMPARISON BASELINE</p><strong>현재 기준선</strong><span>{snapshot.currentPartName ?? "기준 부품 정보 없음"}</span></div><div>{snapshot.currentPartSummary && <small>{snapshot.currentPartSummary}</small>}{snapshot.currentPartPrice && <em>{snapshot.currentPartPrice}</em>}</div></section>}
    {hasSimilarityEvidence && <section className="shared-comparison-similarity-panel" data-testid="shared-comparison-similarity-evidence" aria-label="공유된 성능 비교 근거"><div className="shared-comparison-similarity-panel-heading"><div><p className="eyebrow">EVIDENCE PRESERVED</p><h3>성능 비교 근거 보존</h3><small>공유 당시 유사도에 사용한 공통 지표와 모델 참조를 함께 남깁니다.</small></div><span>{snapshot.candidates.filter((candidate) => candidate.similarityEvidence).length}개 후보</span></div><div className="shared-comparison-similarity-panel-grid">{snapshot.candidates.map((candidate) => <article key={`${candidate.name}-similarity-panel`}><strong>{candidate.name}</strong>{candidate.similarityEvidence ? <SharedComparisonSimilarityEvidence evidence={candidate.similarityEvidence} /> : <small>구조화된 비교 근거 기록 없음</small>}</article>)}</div><p className="shared-comparison-similarity-panel-note"><FiInfo /> 모델 참조가 포함된 지표는 선택 부품의 직접 측정값이 아니라 같은 모델 계열 카탈로그 참조값입니다. 실제 성능·FPS는 보장하지 않습니다.</p></section>}
    {hasBenchmarkEvidence && <section className="shared-comparison-benchmark-panel" data-testid="shared-comparison-benchmark-evidence-panel" aria-label="공유된 원본 성능 근거"><div className="shared-comparison-benchmark-panel-heading"><div><p className="eyebrow">BENCHMARK PRESERVED</p><h3>원본 성능 근거 보존</h3><small>공유 당시 카드와 상세 화면에서 확인한 CPU·GPU 점수, 출처, 원문 점검 상태를 함께 남깁니다.</small></div><span>{snapshot.candidates.filter((candidate) => candidate.benchmarkEvidence).length}개 후보</span></div><div className="shared-comparison-benchmark-panel-grid">{snapshot.candidates.map((candidate) => <article key={`${candidate.name}-benchmark-panel`}><strong>{candidate.name}</strong>{candidate.benchmarkEvidence ? <SharedComparisonBenchmarkEvidence evidence={candidate.benchmarkEvidence} /> : <small>원본 성능 근거 기록 없음</small>}</article>)}</div><p className="shared-comparison-benchmark-panel-note"><FiInfo /> 성능 점수는 측정 설정·드라이버·시스템 조건에 따라 달라지는 참고값입니다. 공유 snapshot의 점수와 출처는 생성 당시 기록이며, 현재 카탈로그와 달라질 수 있습니다.</p></section>}
  </>;
}
...
export function SharedAlternativeComparisonView({ onBack, onToast }: { onBack: () => void; onToast: (message: string) => void }) {
  ...
  async function importChecklistFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > LOCAL_IMPORT_MAX_BYTES) {
      setChecklistActionMessage("후보 비교 체크리스트 JSON은 1MB 이하 파일만 가져올 수 있습니다.");
      setChecklistTransferPreview(null);
      return;
    }
    try {
      ...
      setChecklistActionMessage("후보 비교 체크리스트 JSON 파일을 읽지 못했습니다.");
      setChecklistTransferPreview(null);
    }
  }
  ...
  function applyChecklistTransferPreview() {
    ...
    if (!alternativeComparisonChecklistTransferMatchesCurrentFor(checklistEntries.map((entry) => entry.key), checklistTransferPreview.itemKeys)) {
      setChecklistTransferPreview(null);
      setChecklistActionMessage("공유 snapshot의 확인 항목이 달라져 체크리스트를 가져올 수 없습니다. 같은 공유 링크에서 다시 내보내 주세요.");
      return;
    }
    ...
    void api<{ items: Part[]; missingIds: string[] }>(`/api/parts/batch?ids=${encodeURIComponent(candidatePartIds.join(","))}`, { retry: 1, signal: controller.signal })
      ...
      .catch((reason: unknown) => {
        if (!cancelled) {
          setLiveCandidateParts({});
          setLiveMissingCandidateIds([]);
          setLiveCheckError(reason instanceof Error ? reason.message : "현재 카탈로그 후보를 확인하지 못했습니다.");
          setLiveCheckedAt(new Date().toISOString());
        }
      })
  ...
  async function copySnapshot() {
    ...
    try {
      ...
      if (mountedRef.current) onToast("공유 후보 비교표를 클립보드에 복사했습니다.");
    } catch {
      if (mountedRef.current) onToast("공유 후보 비교표 복사에 실패했습니다. 브라우저 클립보드 권한을 확인해 주세요.");
    }
  }
