...
function sharedBenchmarkRecheckDetail(recheck: AlternativeComparisonBenchmarkRecheck) {
  ...
  if (recheck.status === "current_unavailable") return "현재 후보를 확인할 수 없어 공유 저장본과 대조하지 못했습니다.";
...
function SharedComparisonBaseline({ snapshot }: { snapshot: AlternativeComparisonSnapshot }) {
  ...
  return <>
    ...
    {hasBenchmarkEvidence && <section className="shared-comparison-benchmark-panel" data-testid="shared-comparison-benchmark-evidence-panel" aria-label="공유된 원본 성능 정보"><div className="shared-comparison-benchmark-panel-heading"><div><p className="eyebrow">BENCHMARK PRESERVED</p><h3>원본 성능 정보 보존</h3><small>공유 당시 카드와 상세 화면에서 확인한 CPU·GPU 점수, 출처, 원문 점검 상태를 함께 남깁니다.</small></div><span>{snapshot.candidates.filter((candidate) => candidate.benchmarkEvidence).length}개 후보</span></div><div className="shared-comparison-benchmark-panel-grid">{snapshot.candidates.map((candidate) => <article key={`${candidate.name}-benchmark-panel`}><strong>{candidate.name}</strong>{candidate.benchmarkEvidence ? <SharedComparisonBenchmarkEvidence evidence={candidate.benchmarkEvidence} /> : <small>원본 성능 정보 기록 없음</small>}</article>)}</div><p className="shared-comparison-benchmark-panel-note"><FiInfo /> 성능 점수는 측정 설정·드라이버·시스템 조건에 따라 달라지는 참고값입니다. 공유 저장본의 점수와 출처는 생성 당시 기록이며, 현재 카탈로그와 달라질 수 있습니다.</p></section>}
  </>;
}
