...
function BenchmarkEvidenceCard({ evidence }: { evidence: BenchmarkEvidencePart }) {
  ...
  return <article className={`benchmark-evidence-card ${evidence.status}`} data-testid={`benchmark-evidence-${evidence.category}`}>
    ...
    <div className="benchmark-evidence-source"><span>점수 근거</span>{evidence.provenance ? <p><strong>{BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind]}</strong> · {evidence.provenance.sourceNote}{evidence.provenance.updatedAt ? ` · 근거 갱신 ${new Date(evidence.provenance.updatedAt).toLocaleDateString("ko-KR")}` : ""}{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer">원문 <FiExternalLink /></a>}</p> : <p>점수는 기록되어 있지만 출처 유형·근거 메모가 없어 원문 확인이 필요합니다.</p>}</div>
    <small className={`benchmark-evidence-source-check ${evidence.sourceCheck ? (benchmarkSourceCheckLabelFor(evidence.sourceCheck) === "원문 검증됨" ? "verified" : "review") : "not-checked"}`}>원문 점검 · {benchmarkSourceCheckLabelFor(evidence.sourceCheck)}{evidence.sourceCheck?.detail ? ` · ${evidence.sourceCheck.detail}` : ""}</small>
...
export function BenchmarkEvidencePanel({ cpu, gpu, snapshot }: { cpu?: Part; gpu?: Part; snapshot?: BuildBenchmarkSnapshot }) {
  ...
  const snapshotSummary = buildSnapshot.status === "complete"
    ? "선택된 CPU·GPU의 benchmark 점수를 모두 기록했습니다."
    : buildSnapshot.status === "partial"
      ? "일부 점수만 있어 성능 판단 전 누락된 근거를 확인해야 합니다."
      : "선택된 부품의 benchmark 점수가 없어 성능 판단 근거가 부족합니다.";
  return <section className="benchmark-evidence-panel" aria-label="원본 benchmark 근거" data-testid="benchmark-evidence-panel">
    <div className="benchmark-evidence-heading"><div><p className="eyebrow">BENCHMARK EVIDENCE</p><h2>원본 benchmark 근거</h2><p>선택한 CPU·GPU에 카탈로그로 저장된 Cinebench R23·3DMark 점수와 근거 상태를 표시합니다.</p></div><span><FiActivity /> {evidences.filter((item) => item.status === "complete").length}/{evidences.length} 완전</span></div>
    <div className={`benchmark-evidence-build-snapshot ${buildSnapshot.status}`} data-testid="build-benchmark-snapshot"><div><span>견적 성능 근거 상태</span><strong>{buildBenchmarkSnapshotStatusText(buildSnapshot.status)}</strong></div><span>{buildSnapshot.presentScoreCount}/{buildSnapshot.expectedScoreCount}개 점수 · {buildSnapshot.parts.length}개 부품</span><p>{snapshotSummary}</p></div>
    ...
    <p className="benchmark-evidence-note"><FiInfo /> 점수는 측정 설정·드라이버·시스템 조건에 따라 달라지는 카탈로그 참고값이며, 실제 FPS·프레임타임·작업 시간·절대 성능 순위를 보장하지 않습니다. 출처 유형이 없거나 점수가 일부만 있으면 그대로 확인 필요로 표시합니다.</p>
  </section>;
}
