...
function BenchmarkEvidenceCard({ evidence }: { evidence: BenchmarkEvidencePart }) {
  ...
  return <article className={`benchmark-evidence-card ${evidence.status}`} data-testid={`benchmark-evidence-${evidence.category}`}>
    ...
    <div className="benchmark-evidence-source"><span>점수 정보</span>{evidence.provenance ? <p><strong>{BENCHMARK_SOURCE_KIND_LABELS[evidence.provenance.sourceKind]}</strong> · {evidence.provenance.sourceNote}{evidence.provenance.updatedAt ? ` · 정보 갱신 ${new Date(evidence.provenance.updatedAt).toLocaleDateString("ko-KR")}` : ""}{sourceUrl && <a href={sourceUrl} target="_blank" rel="noreferrer">점수 출처 <FiExternalLink /></a>}</p> : <p>점수는 기록되어 있지만 출처·출처 메모가 없어 실제 출처 확인이 필요합니다.</p>}</div>
    <small className={`benchmark-evidence-source-check ${evidence.sourceCheck ? (benchmarkSourceCheckLabelFor(evidence.sourceCheck) === "출처 확인됨" ? "verified" : "review") : "not-checked"}`}>출처 확인 · {benchmarkSourceCheckLabelFor(evidence.sourceCheck)}{evidence.sourceCheck?.detail ? ` · ${evidence.sourceCheck.detail}` : ""}</small>
