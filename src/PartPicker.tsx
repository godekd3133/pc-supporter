...
function PickerIncompleteDataNotice({ category, count, fields }: { category: PartCategory; count: number; fields: Array<{ field: string; count: number }> }) {
  ...
  return <div className="picker-incomplete-data-notice" data-testid="picker-incomplete-data-notice" role="status"><FiInfo /><div><strong>사양 정보가 부족한 부품 {count.toLocaleString("ko-KR")}개가 있어요.</strong><small>누락된 사양이 추가되면 호환 여부를 비교할 수 있어요.</small><div className="picker-incomplete-field-links">{fields.slice(0, 3).map((field) => <a href={`/catalog?category=${encodeURIComponent(category)}&quality=incomplete&missingField=${encodeURIComponent(field.field)}`} data-testid="picker-open-missing-field" key={field.field}>{catalogMissingFieldLabelFor(field.field)} {field.count}개</a>)}</div></div></div>;
}
...
function PickerCachedCatalogNotice({ visibleCount, total }: { visibleCount: number; total: number }) {
  return <div className="picker-cached-fallback" data-testid="picker-cached-fallback" role="status"><span className="picker-cached-fallback-icon"><FiDatabase /></span><div><strong>부품 목록을 임시로 보여드려요.</strong><p>{visibleCount.toLocaleString("ko-KR")} / {total.toLocaleString("ko-KR")}개 부품</p><small>최신 가격과 호환 정보는 잠시 후 다시 이용해 주세요.</small></div></div>;
}

function pickerPriceProvenance(part: Part) {
  const evidence = catalogPriceEvidenceFor(part);
  const label = catalogPriceEvidenceLabelFor(part);
  const observedAt = typeof part.priceCheckedAt === "string" && part.priceCheckedAt.trim()
    ? part.priceCheckedAt
    : part.source === "danawa" && part.updatedAt ? part.updatedAt : undefined;
  const date = observedAt && Number.isFinite(Date.parse(observedAt))
    ? new Date(observedAt).toLocaleDateString("ko-KR")
    : undefined;
  const dateLabel = part.source === "danawa" && date
    ? part.priceCheckedAt ? `가격 확인 ${date}` : `수집 기록 ${date} · 가격 재확인 필요`
    : undefined;
  return { evidence, text: dateLabel ? `${label} · ${dateLabel}` : label };
}
...
function PickerCachedCatalogList({ items, selected, expandedPickerId, comparePickerIds, onSelect, onToggleCompare, onToggleExpanded, partSummary, formatWon, PartVisual, PartEvidence }: { items: PickerPart[]; selected: PartSelection[]; expandedPickerId: string | null; comparePickerIds: string[]; onSelect: (part: Part) => void; onToggleCompare: (part: PickerPart) => void; onToggleExpanded: (partId: string) => void; partSummary: (part: Part | undefined) => string; formatWon: PartPickerProps["formatWon"]; PartVisual: PartPickerProps["PartVisual"]; PartEvidence: PartPickerProps["PartEvidence"] }) {
  return <div className="picker-list picker-list-cached" data-testid="picker-cached-catalog-list">{items.map((part) => {
    ...
    const priceProvenance = pickerPriceProvenance(part);
    return <article className={expanded ? "picker-item-card expanded" : "picker-item-card"} key={part.id}><button className={alreadySelected ? "picker-item already" : "picker-item"} type="button" onClick={() => onSelect(part)}><span className="picker-item-icon"><PartVisual part={part} /></span><span className="picker-item-main"><strong>{part.name}</strong><small>{partSummary(part)}</small><span className="data-badges">{part.listingType && part.listingType !== "retail" && <em className="listing-badge">{LISTING_TYPE_LABELS[part.listingType]}</em>}{part.missingFields.length > 0 && <em className="missing-badge">사양 미등록 {part.missingFields.length}개</em>}</span></span><span className="picker-item-side"><strong>{formatWon(part.priceWon)}</strong><small className={`catalog-detail-price-evidence ${priceProvenance.evidence}`}>{priceProvenance.text}</small>{alreadySelected ? <span><FiCheck /> 선택됨</span> : <span>선택 <FiExternalLink /></span>}</span></button><div className="picker-item-actions"><button className={compared ? "text-button picker-compare-toggle selected" : "text-button picker-compare-toggle"} type="button" aria-pressed={compared} onClick={() => onToggleCompare(part)}>{compared ? "비교 중" : "비교"}</button><button className="text-button picker-item-detail-toggle" type="button" aria-expanded={expanded} onClick={() => onToggleExpanded(part.id)}>{expanded ? "상세 사양 닫기" : "상세 사양"} <FiChevronDown /></button></div>{expanded && <PickerPartDetail part={part} PartEvidence={PartEvidence} />}</article>;
  })}</div>;
}
...
function PickerEmptyState({ message, canRelaxFilters, canShowReviewCandidates, reviewCandidateCount, onRelaxFilters, onShowReviewCandidates }: { message: string; canRelaxFilters: boolean; canShowReviewCandidates: boolean; reviewCandidateCount: number; onRelaxFilters: () => void; onShowReviewCandidates: () => void }) {
  return <div className="picker-state picker-empty-state" data-testid="picker-empty-state"><FiSearch /><div><strong>조건에 맞는 부품이 없어요.</strong><span>{message}</span>{(canShowReviewCandidates || canRelaxFilters) && <div className="picker-empty-state-actions">{canShowReviewCandidates && <button className="button button-small button-light" type="button" data-testid="picker-empty-review-candidates" onClick={onShowReviewCandidates}>정보가 부족한 부품도 보기 · {reviewCandidateCount.toLocaleString("ko-KR")}개</button>}{canRelaxFilters && <button className="button button-small button-secondary" type="button" data-testid="picker-empty-relax-filters" onClick={onRelaxFilters}><FiRefreshCw /> 부품 필터 완화</button>}</div>}</div></div>;
}
...
function pickerCandidateRisk(part: PickerPart) {
  if (part.candidateRisk === "safe") return "호환 가능";
  if (part.candidateRisk === "unsafe") return part.candidateReasons && part.candidateReasons.length > 0 ? `호환되지 않아요 · ${part.candidateReasons[0]}` : "호환 불가 있음";
  if (part.candidateRisk === "review") return part.candidateReasons && part.candidateReasons.length > 0 ? `사양 부족 · ${part.candidateReasons[0]}` : "호환 정보를 알 수 없어요";
  return "카탈로그 부품";
}

function pickerCandidateFullRiskText(part: PickerPart) {
  if (part.remainingBlockers === undefined || part.remainingWarnings === undefined || part.remainingUnknown === undefined) return "전체 호환 상태 없음";
...
function pickerRecommendationTrustDetail(trust: NonNullable<PickerPart["recommendationTrust"]>) {
  const compatibility = trust.compatibility === "verified" ? "호환 가능" : "호환 추가 확인";
...
function PickerComparison({ parts, currentSelections, category, affectedPartIds, onSelect, onPreview, onCompareScenarios, onCopy, onDownload, onJsonDownload, onShare, onRevoke, partSummary, formatWon, similarityEvidenceText }: { parts: PickerPart[]; currentSelections: Array<{ part: Part; quantity: number }>; category: PartCategory; affectedPartIds?: string[]; onSelect: (part: Part) => void; onPreview?: (category: PartCategory, part: Part, quantity?: number, affectedPartIds?: string[]) => void; onCompareScenarios?: (category: PartCategory, parts: PickerPart[], affectedPartIds?: string[]) => void; onCopy: () => void; onDownload: () => void; onJsonDownload: () => void; onShare: ShareHandler; onRevoke: RevokeHandler; partSummary: PartPickerProps["partSummary"]; formatWon: PartPickerProps["formatWon"]; similarityEvidenceText: PartPickerProps["similarityEvidenceText"] }) {
  ...
  const currentSelectionPriceText = currentSelectionPrice !== undefined ? formatWon(currentSelectionPrice) : currentSelections.length > 0 ? "가격 정보 없음" : undefined;
  ...
  return <section className="picker-comparison" aria-label="부품 비교">
    ...
    <div className="picker-comparison-baseline" data-testid="picker-comparison-baseline"><div><span>현재 기준선</span><strong>{currentSelectionNames}</strong></div><div><small>{currentSelectionSummary}</small><em>{currentSelectionPrice !== undefined ? formatWon(currentSelectionPrice) : "가격 정보 없음"}</em></div></div>
...
export function PartPicker({ category, build, partMap, profile, recommendationListingPolicy = "retail_only", gamingResolution, gamingRefreshRate, benchmarkCoverage, brandOptions = [], findingRuleId, findingTitle, initialCandidateMode = findingRuleId ? "safe" : "all", affectedPartIds, selected, onClose, onSelect, onToast, onWatchPart, onShareComparison, onRevokeComparison, onPreview, onCompareScenarios, partSummary, formatWon, formatSpecValue, similarityEvidenceText, PartVisual, PartEvidence, PartWatchButton }: PartPickerProps) {
  ...
  const pickerComparisonExportContext = { category: CATEGORY_LABELS[category], ...(pickerCurrentSelectionNames ? { currentPartName: pickerCurrentSelectionNames, currentPartSummary: pickerCurrentSelectionSummary, currentPartPrice: pickerCurrentSelectionPrice !== undefined ? formatWon(pickerCurrentSelectionPrice) : "가격 정보 없음" } : {}) };
  ...
  function applyCompatibilityPreset() {
    if (compatibilityPreset.labels.length === 0) {
      setPresetMessage("현재 견적에서 적용할 수 있는 호환 조건이 없어요.");
      return;
    }
    setSpecFilter((current) => ({ ...current, ...compatibilityPreset.values } as PickerSpecFilter));
    setPresetMessage(`${compatibilityPreset.labels.length}개 조건을 적용했습니다. 부품 목록을 좁혔어요.`);
  }
  ...
  const riskFilterLabel = riskFilter === "safe" ? "안전 부품" : riskFilter === "review" ? "정보가 부족한 부품" : riskFilter === "unsafe" ? "차단 부품" : "전체";
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="picker-modal" role="dialog" aria-modal="true" aria-labelledby="picker-title">
      <div className="modal-header"><div><p className="eyebrow">부품 찾기</p><h2 id="picker-title">{CATEGORY_LABELS[category]} 선택</h2><p>부품 이름과 사양으로 검색해요.</p>{findingTitle && <p className="picker-intent-label">문제 해결 부품 · {findingTitle}</p>}</div><button className="icon-button" type="button" onClick={onClose} aria-label="부품 선택 닫기"><FiXCircle /></button></div>
      ...
      {canOfferReviewCandidates && <button className="button button-small picker-review-candidates-action" type="button" onClick={() => { setCandidateMode("no_blocker"); setPresetMessage("호환 여부를 알 수 없는 부품도 함께 보여드려요."); }}>정보가 부족한 부품도 보기 · {riskCounts?.review.toLocaleString("ko-KR")}개</button>}
      ...
      {loading ? <div className="picker-state"><FiLoader className="spin" /><span>{candidateLoadingMessage}</span></div> : error ? <PickerFetchErrorNotice subject={`${CATEGORY_LABELS[category]} ${candidateMode === "all" ? "" : `${candidateModeLabel} `}목록`} message={error} onRetry={() => setRetryNonce((current) => current + 1)} retrying={loading} /> : items.length === 0 ? <PickerEmptyState message={candidateEmptyMessage} canRelaxFilters={hasNarrowCandidateFilter} canShowReviewCandidates={canOfferReviewCandidatesInEmpty} reviewCandidateCount={riskCounts?.review ?? 0} onRelaxFilters={clearCandidateFilters} onShowReviewCandidates={() => { setCandidateMode("no_blocker"); setPresetMessage("호환 여부를 알 수 없는 부품도 함께 보여드려요."); }} /> : <div className="picker-list">{items.map((part) => { const alreadySelected = selected.some((selection) => selection.partId === part.id); const candidateTotalPrice = part.recommendedQuantity !== undefined && isKnownPrice(part.priceWon) ? part.priceWon * part.recommendedQuantity : part.priceWon; const expanded = expandedPickerId === part.id; const compared = comparePickerIds.includes(part.id); const physicalReview = part.physicalEvidence?.status === "review"; const sourceUrl = safeExternalUrl(part.danawaUrl); const priceProvenance = pickerPriceProvenance(part); return <article className={expanded ? "picker-item-card expanded" : "picker-item-card"} key={part.id}><button className={alreadySelected ? "picker-item already" : "picker-item"} type="button" onClick={() => onSelect(part)}><span className="picker-item-icon"><PartVisual part={part} /></span><span className="picker-item-main"><strong>{part.name}</strong><small>{part.recommendedQuantity !== undefined ? `추천 킷 ${part.recommendedQuantity}개 · ` : ""}{partSummary(part)}</small>{part.performanceSummary && <small className="picker-similarity">{part.performanceSummary}</small>}{candidateMode !== "all" && part.physicalEvidence && part.physicalEvidence.status !== "not_applicable" && <small className={`picker-physical-evidence-line ${part.physicalEvidence.status}`}>{pickerPhysicalEvidenceText(part.physicalEvidence)}</small>}{(part.candidateRisk === "review" || part.candidateRisk === "unsafe") && part.candidateReasons && part.candidateReasons.length > 0 && <small className="picker-review-reasons">{part.candidateRisk === "unsafe" ? "호환되지 않아요: " : "사양 부족: "}{part.candidateReasons.slice(0, 2).join(" · ")}</small>}<span className="data-badges">{part.listingType && part.listingType !== "retail" && <em className="listing-badge">{LISTING_TYPE_LABELS[part.listingType]}</em>}{part.missingFields.length > 0 && <em className="missing-badge">사양 미등록 {part.missingFields.length}개</em>}</span></span><span className="picker-item-side"><strong>{formatWon(candidateTotalPrice)}</strong><small className={`catalog-detail-price-evidence ${priceProvenance.evidence}`}>{priceProvenance.text}</small>{part.recommendedQuantity !== undefined && <small>1킷 {formatWon(part.priceWon)}</small>}{alreadySelected ? <span><FiCheck /> 선택됨</span> : <span>선택 <FiExternalLink /></span>}</span></button><div className="picker-item-actions"><PartWatchButton part={part} onWatch={onWatchPart} />{sourceUrl && <a className="picker-source-link" href={sourceUrl} target="_blank" rel="noreferrer" aria-label={`${part.name} 다나와 상품 페이지 보기`}>다나와 보기 <FiExternalLink /></a>}<button className={compared ? "text-button picker-compare-toggle selected" : "text-button picker-compare-toggle"} type="button" aria-pressed={compared} onClick={() => togglePickerCompare(part)}>{compared ? "비교 중" : "비교"}</button><button className="text-button picker-item-detail-toggle" type="button" aria-expanded={expanded} onClick={() => setExpandedPickerId(expanded ? null : part.id)}>{expanded ? "상세 사양 닫기" : "상세 사양"} <FiChevronDown /></button></div>{expanded && <PickerPartDetail part={part} PartEvidence={PartEvidence} />}</article>; })}</div>}
      {showCachedFallback && <PickerCachedCatalogNotice visibleCount={visibleItems.length} total={visibleTotal} />}
