...
function PickerIncompleteDataNotice({ category, count, fields }: { category: PartCategory; count: number; fields: Array<{ field: string; count: number }> }) {
  ...
  return <div className="picker-incomplete-data-notice" data-testid="picker-incomplete-data-notice" role="status"><FiInfo /><div><strong>정보가 부족한 부품 {count.toLocaleString("ko-KR")}개를 안전 부품에서 제외했어요.</strong><small>필수 스펙이 부족한 부품은 호환 가능하다고 가정하지 않습니다. 누락 필드를 보강한 뒤 다시 평가하세요.</small><div className="picker-incomplete-field-links">{fields.slice(0, 3).map((field) => <a href={`/catalog?category=${encodeURIComponent(category)}&quality=incomplete&missingField=${encodeURIComponent(field.field)}`} data-testid="picker-open-missing-field" key={field.field}>{catalogMissingFieldLabelFor(field.field)} {field.count}개</a>)}</div></div></div>;
}
...
function pickerPhysicalEvidenceText(evidence: NonNullable<PickerPart["physicalEvidence"]>) {
  ...
  return `장착 정보 · ${pickerPhysicalEvidenceLabel(evidence.status)}${sourceCount > 0 ? ` · 출처 ${sourceCount}건` : " · 출처 메모 없음"} · ${evidence.summary}`;
}
...
function PickerPhysicalEvidenceSources({ sources }: { sources: PhysicalEvidenceSource[] | undefined }) {
  ...
  return safeSources.length > 0
    ? <div className="picker-physical-evidence-sources" aria-label="장착 정보 출처">{safeSources.map((source) => <small key={`${source.category}-${source.note}-${source.url ?? ""}`}><b>{pickerPhysicalEvidenceSourceIdentity(source)}</b> {source.note}{source.updatedAt ? ` · 정보 갱신 ${new Date(source.updatedAt).toLocaleDateString("ko-KR")}` : ""}{source.url && <a href={source.url} target="_blank" rel="noreferrer">제조사 페이지 <FiExternalLink /></a>}</small>)}</div>
    : <small className="picker-physical-evidence-sources-empty">출처 메모가 없어요 · 제조사 페이지에서 확인해 주세요</small>;
}
...
function PickerPhysicalEvidence({ part, compact = false }: { part: PickerPart; compact?: boolean }) {
  ...
  return <div className={`picker-physical-evidence ${evidence.status}${compact ? " compact" : ""}`} aria-label={`${part.name} 장착 정보`}><div><strong>장착 정보</strong><span>{pickerPhysicalEvidenceLabel(evidence.status)}</span></div><p>{evidence.summary}</p><PickerPhysicalEvidenceSources sources={evidence.sources} /></div>;
}
...
function PickerSimilarityEvidence({ part, similarityEvidenceText }: { part: PickerPart; similarityEvidenceText: PartPickerProps["similarityEvidenceText"] }) {
  ...
  return <div className="picker-item-similarity-detail" aria-label={`${part.name} 성능 비교`} data-testid="picker-similarity-evidence"><div><strong>성능 비교</strong><span>{part.similarityLabel} {part.similarityScore}점 · {similarityEvidenceText(evidence)} · {evidence.comparedDimensions}/{evidence.totalDimensions}개 지표</span></div>{part.performanceSummary && <p>{part.performanceSummary}</p>}{dimensions.length > 0 && <div className="picker-similarity-evidence-table-wrap"><table><caption>현재 선택 부품과 비교 부품의 지표별 비교</caption><thead><tr><th scope="col">지표</th><th scope="col">현재</th><th scope="col">비교 부품</th><th scope="col">점수</th></tr></thead><tbody>{dimensions.map((dimension) => <tr key={dimension.key}><th scope="row">{dimension.label}</th><td>{dimension.currentValue}{dimension.source === "model_reference" && <small className="similarity-dimension-source">모델 참조</small>}</td><td>{dimension.candidateValue}</td><td>{dimension.score}점</td></tr>)}</tbody></table></div>}{evidence.notes?.map((note) => <small key={note}><FiInfo /> {note}</small>)}<small className="picker-similarity-disclaimer"><FiInfo /> 상대 비교일 뿐 실제 FPS·작업 시간·게임 성능을 보장하지는 않아요.</small></div>;
}
...
function PickerPartDetail({ part, PartEvidence, similarityEvidenceText }: { part: PickerPart; PartEvidence: PartEvidenceRenderer; similarityEvidenceText: PartPickerProps["similarityEvidenceText"] }) {
  return <div className="picker-item-detail" aria-label={`${part.name} 부품 상세`}>
    {part.candidateRisk === "unsafe" && <p className="picker-unsafe-note"><FiXCircle /> 전체 부품 검사에서 차단 오류가 확인된 부품이에요. 현재 견적에는 자동 적용하지 않아요{part.candidateReasons && part.candidateReasons.length > 0 ? ` · ${part.candidateReasons.join(" · ")}` : ""}.</p>}
    ...
    <p className={`picker-price-evidence-detail ${catalogPriceEvidenceFor(part)}`} data-testid="picker-price-evidence" title={catalogPriceEvidenceDescriptionFor(part)}><FiInfo /> 가격 출처 · <strong>{catalogPriceEvidenceLabelFor(part)}</strong> · {catalogPriceEvidenceDescriptionFor(part)}</p>
...
function PickerComparison({ parts, currentSelections, category, affectedPartIds, onSelect, onPreview, onCompareScenarios, onCopy, onDownload, onJsonDownload, onShare, onRevoke, partSummary, formatWon, similarityEvidenceText }: { parts: PickerPart[]; currentSelections: Array<{ part: Part; quantity: number }>; category: PartCategory; affectedPartIds?: string[]; onSelect: (part: Part) => void; onPreview?: (category: PartCategory, part: Part, quantity?: number, affectedPartIds?: string[]) => void; onCompareScenarios?: (category: PartCategory, parts: PickerPart[], affectedPartIds?: string[]) => void; onCopy: () => void; onDownload: () => void; onJsonDownload: () => void; onShare: ShareHandler; onRevoke: RevokeHandler; partSummary: PartPickerProps["partSummary"]; formatWon: PartPickerProps["formatWon"]; similarityEvidenceText: PartPickerProps["similarityEvidenceText"] }) {
  ...
  return <section className="picker-comparison" aria-label="부품 비교">
    ...
    <div className="picker-comparison-table-wrap"><table><caption>적용 전에 선택한 부품의 가격·성능·호환성을 비교해요.</caption><thead><tr><th scope="col">비교 항목</th>{parts.map((part) => <th scope="col" key={part.id}>{part.name}</th>)}</tr></thead><tbody>
      ...
      {parts.some((part) => part.recommendationTrust) && <tr><th scope="row">추천 점수</th>{parts.map((part) => <td key={`${part.id}-trust`}>{part.recommendationTrust ? <><strong>{pickerRecommendationTrustText(part.recommendationTrust)}</strong><small>{pickerRecommendationTrustDetail(part.recommendationTrust)}</small></> : "산정 불가"}</td>)}</tr>}
...
export function PartPicker({ category, build, partMap, profile, recommendationListingPolicy = "retail_only", gamingResolution, gamingRefreshRate, benchmarkCoverage, brandOptions = [], findingRuleId, findingTitle, initialCandidateMode = findingRuleId ? "safe" : "all", affectedPartIds, selected, onClose, onSelect, onToast, onWatchPart, onShareComparison, onRevokeComparison, onPreview, onCompareScenarios, partSummary, formatWon, formatSpecValue, similarityEvidenceText, PartVisual, PartEvidence, PartWatchButton }: PartPickerProps) {
  ...
  const candidateModeLabel = candidateMode === "safe" ? "확인된 안전 부품" : candidateMode === "no_blocker" ? "차단 오류 없는 부품" : candidateMode === "precision" ? "전체 부품" : "전체 카탈로그";
  ...
  const performanceFilterLabel = performanceFilter === "similar" ? "동급·유사 성능 부품" : performanceFilter === "verified" ? "성능 정보 충분 부품" : performanceFilter === "benchmark" ? "벤치마크 정보 포함 부품" : "전체 성능 부품";
  ...
  const candidateEmptyMessage = priceStatus !== "all" && total === 0 ? `${priceStatusLabel}에 해당하는 부품이 없습니다. 가격 상태를 전체로 바꾸면 다른 부품을 확인할 수 있어요.` : freshness !== "all" && total === 0 ? `${freshnessFilterLabel}에 해당하는 부품이 없습니다. 갱신 상태를 전체로 바꾸면 다른 부품을 확인할 수 있어요.` : candidateMode !== "all" && physicalEvidenceFilter !== "all" && total === 0 ? `${physicalEvidenceFilterLabelText} 부품이 없어요. 장착 정보 조건을 전체로 바꾸면 호환 우선 부품을 확인할 수 있어요.` : candidateMode !== "all" && trustFilter !== "all" && total === 0 ? `${trustFilterLabel}가 없습니다. 추천 점수 조건을 낮추면 호환 우선 대안을 확인할 수 있습니다.` : candidateMode !== "all" && performanceFilter !== "all" ? `${performanceFilterLabel}가 없습니다. 성능 기준을 전체로 바꾸면 호환 우선 대안을 확인할 수 있습니다.` : candidateMode === "safe" ? findingTitle ? `${findingTitle}를 해결하면서 새 호환 위험을 만들지 않는 부품이 없어요.` : "현재 구성에 새 호환 위험을 만들지 않는 부품이 없어요." : candidateMode === "no_blocker" ? "현재 구성에서 차단 오류가 없는 부품이 없어요." : candidateMode === "precision" ? findingTitle ? `${findingTitle}를 해결하는 부품을 모두 검사했지만 결과가 없어요.` : "전체 부품 검사 결과가 없어요." : query.trim() ? `"${query.trim().slice(0, 40)}" 검색 결과가 없습니다. 다른 모델명으로 검색해 보세요.` : "검색 결과가 없습니다.";
  const candidateLoadingMessage = candidateMode === "all" ? "부품 목록을 불러오는 중..." : candidateMode === "precision" ? "전체 부품을 검사하는 중..." : `${candidateModeLabel}를 계산하는 중...`;
  const candidateModeHelp = findingTitle && candidateMode === "precision" ? `${findingTitle}를 해결하는 전체 부품을 실제 구성에 적용해 안전·확인 필요·차단 위험을 모두 분류해요. 차단 부품은 적용하지 마세요.` : findingTitle && candidateMode === "no_blocker" ? `${findingTitle}를 해결하고 새 차단 오류는 없지만 스펙 확인이 필요한 부품도 함께 보여줘요.` : findingTitle && candidateMode !== "all" ? `${findingTitle}를 해결하고 새 차단 오류와 확인 필요 항목을 만들지 않는 부품을 먼저 보여줘요.` : candidateMode === "safe" ? "현재 구성에 적용해 봤을 때 새 차단 오류와 확인 필요 항목이 없는 부품만 보여줘요." : candidateMode === "no_blocker" ? "현재 구성에 적용해 봤을 때 새 차단 오류는 없지만 스펙 확인이 필요한 부품을 포함할 수 있어요." : candidateMode === "precision" ? "현재 구성에 모든 부품을 적용해 봐서 안전·확인 필요·차단 위험을 분류해요. 차단 부품은 적용하지 마세요." : "현재 구성과 관계없이 카탈로그 조건에 맞는 부품을 표시합니다.";
  ...
  const candidateTrustSummary = candidateMode !== "all" && trustFilter !== "all" ? `${trustFilterLabel} · ${total.toLocaleString("ko-KR")}개 표시${trustExcludedCount > 0 ? ` · 점수 기준 미충족 ${trustExcludedCount.toLocaleString("ko-KR")}개 제외` : ""}` : "";
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="picker-modal" role="dialog" aria-modal="true" aria-labelledby="picker-title">
      ...
      <div className="picker-filters"><label><span>데이터</span><select value={quality} onChange={(event) => setQuality(event.target.value as "all" | DataQuality)}><option value="all">전체 데이터</option><option value="live">다나와 최신</option><option value="seed">기본 정보</option><option value="manual">직접 확인</option><option value="incomplete">스펙 부족</option></select></label><label><span>구매 조건</span><select value={listingPolicy} onChange={(event) => setListingPolicy(event.target.value as ListingPolicy)}><option value="retail_only">{LISTING_POLICY_LABELS.retail_only}</option><option value="include_bulk">벌크 포함</option><option value="all">{LISTING_POLICY_LABELS.all}</option></select></label><label><span>정렬</span><select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)}><option value="price_asc">가격 낮은 순</option><option value="price_desc">가격 높은 순</option><option value="name">이름 순</option><option value="updated">최근 갱신</option>{candidateMode !== "all" && <><option value="similarity">유사도 높은 순</option><option value="value">가성비 높은 순</option></>}</select></label><label><span>부품</span><select value={candidateMode} onChange={(event) => { const next = event.target.value as typeof candidateMode; setCandidateMode(next); if (next === "all") { setPerformanceFilter("all"); setTrustFilter("all"); if (sort === "similarity" || sort === "value") setSort("price_asc"); } }}><option value="all">전체 카탈로그</option><option value="precision">전체 부품 검사</option><option value="no_blocker">차단 오류 없는 부품</option><option value="safe">확인된 안전 부품</option></select></label><label className="picker-risk-filter"><span>위험도</span><select aria-label="부품 위험도" value={riskFilter} disabled={candidateMode === "all"} onChange={(event) => setRiskFilter(event.target.value as PickerRiskFilter)}><option value="all">전체 위험도</option><option value="safe">안전</option><option value="review">확인 필요</option><option value="unsafe">차단</option></select></label><label className="picker-performance-filter"><span>성능 기준</span><select aria-label="대체 부품 성능 기준" value={performanceFilter} disabled={candidateMode === "all"} onChange={(event) => setPerformanceFilter(event.target.value as PickerPerformanceFilter)}><option value="all">전체 성능 부품</option><option value="similar">동급·유사만</option><option value="verified">정보 충분만</option><option value="benchmark">벤치마크 포함</option></select></label><label className="picker-trust-filter"><span>추천 점수</span><select aria-label="대체 부품 추천 점수" value={trustFilter} disabled={candidateMode === "all"} onChange={(event) => setTrustFilter(event.target.value as PickerTrustFilter)}><option value="all">전체 점수</option><option value="medium_plus">보통 이상</option><option value="high">높음만</option></select></label><label className="picker-budget-filter"><span>교체 예산 <em>선택</em></span><input type="number" min="1" step="10000" value={candidateBudget} disabled={candidateMode === "all"} onChange={(event) => setCandidateBudget(event.target.value)} placeholder="예: 300000" /></label>{category === "gpu" && <label className="picker-spec-filter"><span>최소 VRAM</span><PickerSpecSelect ariaLabel="부품 선택기 최소 VRAM" value={specFilter.minVramGb} options={GPU_VRAM_FILTER_OPTIONS} onChange={(value) => setSpecFilter((current) => ({ ...current, minVramGb: value }))} /></label>}{(category === "memory" || category === "ssd" || category === "hdd") && <label className="picker-spec-filter"><span>{category === "memory" ? "최소 모듈 용량" : "최소 용량"}</span><PickerSpecSelect ariaLabel="부품 선택기 최소 용량" value={specFilter.minCapacityGb} options={category === "memory" ? MEMORY_CAPACITY_FILTER_OPTIONS : STORAGE_CAPACITY_FILTER_OPTIONS} onChange={(value) => setSpecFilter((current) => ({ ...current, minCapacityGb: value }))} /></label>}{category === "memory" && <label className="picker-spec-filter"><span>최소 속도</span><PickerSpecSelect ariaLabel="부품 선택기 최소 메모리 속도" value={specFilter.minMemorySpeedMhz} options={MEMORY_SPEED_FILTER_OPTIONS} onChange={(value) => setSpecFilter((current) => ({ ...current, minMemorySpeedMhz: value }))} /></label>}{category === "psu" && <label className="picker-spec-filter"><span>최소 정격</span><PickerSpecSelect ariaLabel="부품 선택기 최소 정격 출력" value={specFilter.minWattageW} options={PSU_WATTAGE_FILTER_OPTIONS} onChange={(value) => setSpecFilter((current) => ({ ...current, minWattageW: value }))} /></label>}{(category === "ssd" || category === "hdd") && <label className="picker-spec-filter"><span>연결 방식</span><PickerSpecSelect ariaLabel="부품 선택기 연결 방식" value={specFilter.storageInterface} options={STORAGE_INTERFACE_FILTER_OPTIONS} onChange={(value) => setSpecFilter((current) => ({ ...current, storageInterface: value as PickerSpecFilter["storageInterface"] }))} /></label>}</div>
      ...
      {canOfferReviewCandidates && <button className="button button-small picker-review-candidates-action" type="button" onClick={() => { setCandidateMode("no_blocker"); setPresetMessage("안전한 부품과 함께 차단 오류가 없는 확인 필요 부품을 보여드려요. 부품별 확인 사유를 구매 전에 살펴봐 주세요."); }}>확인 필요 부품도 보기 · {riskCounts?.review.toLocaleString("ko-KR")}개</button>}
      ...
      {(category === "gpu" || category === "case" || category === "psu") && <label className="picker-physical-evidence-filter"><span>장착 정보</span><select aria-label="부품 장착 정보" value={physicalEvidenceFilter} disabled={candidateMode === "all"} onChange={(event) => setPhysicalEvidenceFilter(event.target.value as PickerPhysicalEvidenceFilter)}><option value="all">전체 장착 정보</option><option value="verified">장착 정보 확인됨</option><option value="review">장착 정보 확인 필요</option></select></label>}
      ...
      {loading ? <div className="picker-state"><FiLoader className="spin" /><span>{candidateLoadingMessage}</span></div> : error ? <PickerFetchErrorNotice subject={`${CATEGORY_LABELS[category]} ${candidateMode === "all" ? "" : `${candidateModeLabel} `}목록`} message={error} onRetry={() => setRetryNonce((current) => current + 1)} retrying={loading} /> : items.length === 0 ? <PickerEmptyState message={candidateEmptyMessage} canRelaxFilters={hasNarrowCandidateFilter} canShowReviewCandidates={canOfferReviewCandidatesInEmpty} reviewCandidateCount={riskCounts?.review ?? 0} onRelaxFilters={clearCandidateFilters} onShowReviewCandidates={() => { setCandidateMode("no_blocker"); setPresetMessage("안전한 부품과 함께 차단 오류가 없는 확인 필요 부품을 보여드려요. 부품별 확인 사유를 구매 전에 살펴봐 주세요."); }} /> : <div className="picker-list">{items.map((part) => { const alreadySelected = selected.some((selection) => selection.partId === part.id); const candidateTotalPrice = part.recommendedQuantity !== undefined && isKnownPrice(part.priceWon) ? part.priceWon * part.recommendedQuantity : part.priceWon; const expanded = expandedPickerId === part.id; const compared = comparePickerIds.includes(part.id); const physicalReview = part.physicalEvidence?.status === "review"; const sourceUrl = safeExternalUrl(part.danawaUrl); return <article className={expanded ? "picker-item-card expanded" : "picker-item-card"} key={part.id}><button className={alreadySelected ? "picker-item already" : "picker-item"} type="button" onClick={() => onSelect(part)}><span className="picker-item-icon"><PartVisual part={part} /></span><span className="picker-item-main"><strong>{part.name}</strong><small>{part.recommendedQuantity !== undefined ? `추천 킷 ${part.recommendedQuantity}개 · ` : ""}{partSummary(part)}</small>{candidateMode !== "all" && part.similarityScore !== undefined && part.similarityLabel && <small className="picker-similarity">{part.similarityLabel} {part.similarityScore}점 · {part.performanceSummary ?? "비교 정보 확인"}</small>}{candidateMode !== "all" && part.valueScore !== undefined && part.valueLabel && <small className="picker-value">{part.valueLabel} {valueScoreText(part.valueScore)} · 가격 대비 유사도</small>}{candidateMode !== "all" && part.recommendationTrust && <small className={`picker-trust ${part.recommendationTrust.level}`}>추천 점수 {pickerRecommendationTrustText(part.recommendationTrust)} · {pickerRecommendationTrustDetail(part.recommendationTrust)}</small>}{candidateMode !== "all" && part.physicalEvidence && part.physicalEvidence.status !== "not_applicable" && <small className={`picker-physical-evidence-line ${part.physicalEvidence.status}`}>{pickerPhysicalEvidenceText(part.physicalEvidence)}</small>}{(part.candidateRisk === "review" || part.candidateRisk === "unsafe") && part.candidateReasons && part.candidateReasons.length > 0 && <small className="picker-review-reasons">{part.candidateRisk === "unsafe" ? "차단 있음: " : "확인 필요: "}{part.candidateReasons.slice(0, 2).join(" · ")}</small>}<span className="data-badges">{part.decision && <em className={`decision-badge ${part.decision.status}`}>{part.decision.label}</em>}{pickerFreshnessFor(part) && <em className={`freshness-badge ${pickerFreshnessFor(part)}`}>{pickerFreshnessLabelFor(part)}</em>}<em className={`price-status-badge ${pickerPriceStatusFor(part)}`} title={catalogPriceEvidenceDescriptionFor(part)}>{pickerPriceStatusLabelFor(part)}</em><em className={`quality-badge ${part.dataQuality}`}>{DATA_QUALITY_LABELS[part.dataQuality]}</em>{candidateMode !== "all" && part.candidateRisk === "safe" && <em className={physicalReview ? "compatibility-badge review" : "compatibility-badge"}>{physicalReview ? "호환 확인 · 장착 확인 필요" : "호환 확인"}</em>}{(candidateMode === "no_blocker" || candidateMode === "precision") && part.candidateRisk === "review" && <em className="compatibility-badge review">차단 없음 · 확인 필요</em>}{candidateMode === "precision" && part.candidateRisk === "unsafe" && <em className="compatibility-badge unsafe">차단 있음</em>}{part.listingType && part.listingType !== "retail" && <em className="listing-badge">{LISTING_TYPE_LABELS[part.listingType]}</em>}{part.missingFields.length > 0 && <em className="missing-badge">누락 {part.missingFields.length}</em>}</span></span><span className="picker-item-side"><strong>{formatWon(candidateTotalPrice)}</strong>{part.recommendedQuantity !== undefined && <small>1킷 {formatWon(part.priceWon)}</small>}{alreadySelected ? <span><FiCheck /> 선택됨</span> : <span>선택 <FiExternalLink /></span>}</span></button><div className="picker-item-actions"><PartWatchButton part={part} onWatch={onWatchPart} />{sourceUrl && <a className="picker-source-link" href={sourceUrl} target="_blank" rel="noreferrer" aria-label={`${part.name} 다나와 상품 페이지 보기`}>다나와 보기 <FiExternalLink /></a>}<button className={compared ? "text-button picker-compare-toggle selected" : "text-button picker-compare-toggle"} type="button" aria-pressed={compared} onClick={() => togglePickerCompare(part)}>{compared ? "비교 중" : "비교"}</button><button className="text-button picker-item-detail-toggle" type="button" aria-expanded={expanded} onClick={() => setExpandedPickerId(expanded ? null : part.id)}>{expanded ? "상세·이유 닫기" : "상세·이유"} <FiChevronDown /></button></div>{expanded && <PickerPartDetail part={part} PartEvidence={PartEvidence} similarityEvidenceText={similarityEvidenceText} />}</article>; })}</div>}
