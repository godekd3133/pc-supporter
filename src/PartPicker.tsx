...
import type { AlternativeRiskCounts, BrandCountOption, BuildSelection, CatalogBenchmarkCoverage, CompatiblePartCandidate, DataFreshness, DataQuality, GamingRefreshRate, GamingResolution, Part, PartCategory, PartSelection, PriceAvailabilityFilter, RecommendationProfile, RecommendationTrustCounts, RecommendationTrustFilter, SimilarityEvidence, ListingPolicy } from "../shared/types";
...
function PickerCachedCatalogNotice({ visibleCount, total }: { visibleCount: number; total: number }) {
  return <div className="picker-cached-fallback" data-testid="picker-cached-fallback" role="status"><span className="picker-cached-fallback-icon"><FiDatabase /></span><div><strong>이전에 불러온 부품 목록을 표시합니다.</strong><p>{visibleCount.toLocaleString("ko-KR")} / {total.toLocaleString("ko-KR")}개 부품</p><small>최신 가격과 호환 정보는 잠시 후 다시 이용해 주세요.</small></div></div>;
}
...
export function PartPicker({ category, build, partMap, profile, recommendationListingPolicy = "retail_only", gamingResolution, gamingRefreshRate, benchmarkCoverage, brandOptions = [], findingRuleId, findingTitle, initialCandidateMode = findingRuleId ? "safe" : "all", affectedPartIds, selected, onClose, onSelect, onToast, onWatchPart, onShareComparison, onRevokeComparison, onPreview, onCompareScenarios, partSummary, formatWon, formatSpecValue, similarityEvidenceText, PartVisual, PartEvidence, PartWatchButton }: PartPickerProps) {
  ...
  const candidateModeHelp = candidateMode === "precision"
    ...
    : candidateMode === "no_blocker"
      ? "호환 문제가 없는 부품과 사양 확인이 필요한 부품을 표시합니다."
      : candidateMode === "safe"
        ? "현재 구성과 호환되는 부품입니다."
        : "카탈로그에서 부품을 찾아요.";
 ...
 const cachedCatalog = useMemo(() => mergeCatalogPickerCache(cachedParts, [...partMap.values()]), [cachedParts, partMap]);
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="picker-modal" role="dialog" aria-modal="true" aria-labelledby="picker-title">
      ...
      {loading ? <div className="picker-state"><FiLoader className="spin" /><span>{candidateLoadingMessage}</span></div> : error ? <PickerFetchErrorNotice subject={`${CATEGORY_LABELS[category]} ${candidateMode === "all" ? "" : `${candidateModeLabel} `}목록`} message={error} onRetry={() => setRetryNonce((current) => current + 1)} retrying={loading} /> : items.length === 0 ? <PickerEmptyState message={candidateEmptyMessage} canRelaxFilters={hasNarrowCandidateFilter} canShowReviewCandidates={canOfferReviewCandidatesInEmpty} reviewCandidateCount={riskCounts?.review ?? 0} onRelaxFilters={clearCandidateFilters} onShowReviewCandidates={() => { setCandidateMode("no_blocker"); setPresetMessage("호환 여부를 확인하지 못한 부품도 표시합니다."); }} /> : <div className="picker-list">{items.map((part) => { const alreadySelected = selected.some((selection) => selection.partId === part.id); const candidateTotalPrice = part.recommendedQuantity !== undefined && isKnownPrice(part.priceWon) ? part.priceWon * part.recommendedQuantity : part.priceWon; const expanded = expandedPickerId === part.id; const compared = comparePickerIds.includes(part.id); const physicalReview = part.physicalEvidence?.status === "review"; const sourceUrl = safeExternalUrl(part.danawaUrl); return <article className={expanded ? "picker-item-card expanded" : "picker-item-card"} key={part.id}><button className={alreadySelected ? "picker-item already" : "picker-item"} type="button" onClick={() => onSelect(part)}><span className="picker-item-icon"><PartVisual part={part} /></span><span className="picker-item-main"><strong>{part.name}</strong><small>{part.recommendedQuantity !== undefined ? `추천 킷 ${part.recommendedQuantity}개 · ` : ""}{partSummary(part)}</small>{part.performanceSummary && <small className="picker-similarity">{part.performanceSummary}</small>}{candidateMode !== "all" && part.physicalEvidence && part.physicalEvidence.status !== "not_applicable" && <small className={`picker-physical-evidence-line ${part.physicalEvidence.status}`}>{pickerPhysicalEvidenceText(part.physicalEvidence)}</small>}{(part.candidateRisk === "review" || part.candidateRisk === "unsafe") && part.candidateReasons && part.candidateReasons.length > 0 && <small className="picker-review-reasons">{part.candidateRisk === "unsafe" ? "호환되지 않아요: " : "사양 부족: "}{part.candidateReasons.slice(0, 2).join(" · ")}</small>}<span className="data-badges">{part.listingType && part.listingType !== "retail" && <em className="listing-badge">{LISTING_TYPE_LABELS[part.listingType]}</em>}{part.missingFields.length > 0 && <em className="missing-badge">사양 미등록 {part.missingFields.length}개</em>}</span></span><span className="picker-item-side"><strong>{formatWon(candidateTotalPrice)}</strong>{part.recommendedQuantity !== undefined && <small>1킷 {formatWon(part.priceWon)}</small>}{alreadySelected ? <span><FiCheck /> 선택됨</span> : <span>선택 <FiExternalLink /></span>}</span></button><div className="picker-item-actions"><PartWatchButton part={part} onWatch={onWatchPart} />{sourceUrl && <a className="picker-source-link" href={sourceUrl} target="_blank" rel="noreferrer" aria-label={`${part.name} 다나와 상품 페이지 보기`}>다나와 보기 <FiExternalLink /></a>}<button className={compared ? "text-button picker-compare-toggle selected" : "text-button picker-compare-toggle"} type="button" aria-pressed={compared} onClick={() => togglePickerCompare(part)}>{compared ? "비교 중" : "비교"}</button><button className="text-button picker-item-detail-toggle" type="button" aria-expanded={expanded} onClick={() => setExpandedPickerId(expanded ? null : part.id)}>{expanded ? "상세 사양 닫기" : "상세 사양"} <FiChevronDown /></button></div>{expanded && <PickerPartDetail part={part} PartEvidence={PartEvidence} />}</article>; })}</div>}
