...
export function SavedBuildComparisonDecisionSummary({ builds, liveChecks, formatWon, onOpenBuild, openingBuildId }: { builds: SavedBuild[]; liveChecks: Record<string, SavedBuildLiveCheck>; formatWon: (value: number | undefined) => string; onOpenBuild?: (build: SavedBuild, focus?: "purchase-list") => void; openingBuildId?: string | null }) {
  ...
  return <section className="history-comparison-decision" aria-label="견적 비교 결정 요약">
    ...
    {entries.length >= 2 && <section className="history-comparison-tradeoff" data-testid="saved-build-comparison-tradeoff" aria-label="저장 견적 버전 효율 경계"><div className="history-comparison-tradeoff-heading"><div><strong>호환·비용·분석·확장성의 효율 경계</strong><small>현재 카탈로그 재검사 결과에서 다른 버전이 모든 기준으로 앞서는 경우만 열세로 표시합니다.</small></div><span>{tradeoffs.filter((tradeoff) => tradeoff.frontier).length}개 경계</span></div><div className="history-comparison-tradeoff-list">{tradeoffs.map((tradeoff) => <article className={tradeoff.frontier ? "frontier" : "dominated"} key={tradeoff.id}><div><span>{tradeoff.frontier ? "효율 경계" : "열세"}</span><strong>{tradeoff.name}</strong></div><small>위험 {tradeoff.riskScore}점 · 총액 {tradeoff.totalPriceWon === undefined ? "확인 필요" : formatWon(tradeoff.totalPriceWon)} · 분석 {tradeoff.analysisScore === undefined ? "확인 필요" : `${tradeoff.analysisScore}점`} · 확장성 {tradeoff.expansionScore === undefined ? "확인 필요" : `${tradeoff.expansionScore}점`}</small><p>{tradeoff.reason}</p></article>)}</div><p className="history-comparison-tradeoff-note"><FiInfo /> 한쪽에만 존재하는 가격·분석·확장성 근거로는 버전을 열세로 단정하지 않습니다. 효율 경계는 읽기 전용 비교 기준입니다.</p></section>}
