...
export function SavedBuildPurchasePriceHistoryPanel({ builds }: { builds: SavedBuild[] }) {
  ...
  const options: Array<{ id: SavedBuildPurchasePriceHistoryFilter; label: string; count: number }> = [
    ...
    { id: "server-history", label: "이전 revision 있음", count: rows.filter(({ summary }) => savedBuildPurchasePriceHistoryMatchesFilter(summary, "server-history")).length },
    { id: "unrecorded", label: "가격 이력 미기록", count: rows.filter(({ summary }) => savedBuildPurchasePriceHistoryMatchesFilter(summary, "unrecorded")).length }
  ];
  ...
  return <section className="saved-build-purchase-price-history-panel" aria-label="저장 견적 가격 이력 관리" data-testid="saved-build-purchase-price-history-panel">
    <div className="saved-build-purchase-price-history-panel-heading"><div><p className="eyebrow">PRICE HISTORY</p><h2>가격 이력 보기</h2><p>결과 화면을 열지 않아도 저장 견적별 가격 확인 샘플과 서버 revision을 확인합니다.</p></div><span><FiClock /> {visibleRows.length} / {rows.length}개</span></div>
    ...
    {visibleRows.length === 0 ? <div className="saved-build-purchase-price-history-panel-empty" data-testid="saved-build-purchase-price-history-panel-empty"><FiInfo /><span>선택한 조건의 가격 이력 견적이 없습니다.</span><button className="text-button" type="button" onClick={() => setFilter("all")}>전체 견적 보기</button></div> : <div className="saved-build-purchase-price-history-panel-grid">{visibleRows.map(({ build, summary }) => {
      const label = summary.status === "multi-sample" ? "2회 이상 확인" : summary.status === "recorded" ? "가격 이력 있음" : "가격 이력 미기록";
      return <article className={`history-card-purchase-price-history ${summary.status}`} data-testid={`saved-build-purchase-price-history-card-${build.id}`} key={build.id}><div className="history-card-purchase-price-history-heading"><span>{build.name}</span><strong>{summary.status === "unrecorded" ? "미기록" : `${summary.recordedRowCount}개 행`}</strong></div><small>{label}{summary.status !== "unrecorded" ? ` · ${summary.sampleCount}개 샘플 · 최소 ${summary.minSampleCount}회 · 최대 ${summary.maxSampleCount}회` : ""}{summary.revision !== undefined ? ` · revision ${summary.revision}` : ""}{summary.historyCount > 0 ? ` · 이전 이력 ${summary.historyCount}개` : ""}{summary.updatedAt ? ` · ${new Date(summary.updatedAt).toLocaleString("ko-KR", { dateStyle: "short", timeStyle: "short" })}` : ""}</small><a className="history-card-purchase-price-history-open" href={`/share/${encodeURIComponent(build.id)}#purchase-list`} data-testid={`saved-build-purchase-price-history-open-${build.id}`}>구매 목록 보기</a></article>;
    })}</div>}
