...
function scenarioAnalysisSummary(current: CompatibilityResult, next: CompatibilityResult) {
  return `부품 적용 후 성능 분석 ${analysisScoreText(next.analysis.overallScore, next.analysis.scoreLabel)} · ${scenarioAnalysisDetail(current, next)}`;
}
...
function scenarioChecksFor(item: CandidateScenarioCompareItem, result: CompatibilityResult, decision: CandidatePurchaseDecision, history: CandidatePriceHistory | undefined, formatWon: (value: number | undefined) => string): AlternativeComparisonScenarioCheck[] {
  ...
  checks.push({
    ...
    detail: applicationStatus === "blocked"
      ...
      : "부품을 실제 견적에 적용할 때는 변경 예정 부품·가격·전체 재검사 결과를 확인한 뒤 진행하세요."
  });
  return checks;
}
...
function CandidateBenchmarkEvidence({ currentParts, candidate, similarityEvidence }: { currentParts: Part[]; candidate: Part; similarityEvidence?: CandidateScenarioPart["similarityEvidence"] }) {
  ...
  const currentStatus = currentEvidence ? `${currentEvidence.presentCount}/${currentEvidence.totalCount}개 · ${benchmarkFreshnessLabelFor(currentEvidence.benchmarkFreshness)}` : "점수 기록 없음";
  ...
  return <div className={`candidate-scenario-benchmark ${candidateEvidence.status}`} data-testid="candidate-scenario-benchmark"><div className="candidate-scenario-benchmark-heading"><strong>원본 벤치마크 비교</strong><small>{currentStatus} → 부품 {candidateStatus}</small></div><div className="candidate-scenario-benchmark-list">{comparisonRows.map((row) => { const delta = benchmarkDeltaText(row.delta, row.deltaPercent); return <div key={row.key}><span>{row.label}</span><em>{benchmarkScoreText(row.currentValue)}</em><b>→</b><em>{benchmarkScoreText(row.candidateValue)}</em><small className={delta === "비교 불가" ? "review" : row.delta !== undefined && row.delta >= 0 ? "positive" : "negative"}>{delta}</small></div>; })}</div><small className="candidate-scenario-benchmark-note">제조사 페이지 확인 · {sourceCheck} · 자료 {benchmarkFreshnessLabelFor(candidateEvidence.benchmarkFreshness)} · 유사도 {similarityBasisLabelFor(similarityEvidence)}. 원본 점수 변화는 성능 참고 정보이며 실제 FPS·작업 시간·절대 순위를 보장하지 않습니다.</small>{similarityEvidence?.reference && <small className="candidate-scenario-benchmark-reference" data-testid="candidate-scenario-benchmark-reference"><FiInfo /> {similarityReferenceTextFor(similarityEvidence)}. 부품 원본 점수와 동일한 측정값으로 간주하지 않습니다.</small>}</div>;
}
...
export function CandidateScenarioComparisonPanel({ state, currentResult, onApply, onSave, onRetry, onClose, onWatchPart, onShareComparison, onRevokeComparison, onToast, formatWon }: { state: CandidateScenarioCompareState; currentResult: CompatibilityResult; onApply: (item: CandidateScenarioCompareItem) => void; onSave?: (item: CandidateScenarioCompareItem) => void; onRetry: (itemId: string) => void; onClose: () => void; onWatchPart?: (part: Part) => boolean; onShareComparison?: CandidateScenarioShareHandler; onRevokeComparison?: CandidateScenarioRevokeHandler; onToast?: (message: string) => void; formatWon: (value: number | undefined) => string }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="candidate-scenario-dialog" role="dialog" aria-modal="true" aria-labelledby="candidate-scenario-title">
      <div className="modal-header"><div><p className="eyebrow">MULTI CANDIDATE WHAT-IF</p><h2 id="candidate-scenario-title">선택 부품 전체 미리 비교</h2><p>현재 견적은 바꾸지 않고 선택한 {state.items.length}개 부품을 각각 전체 검사 규칙에 적용합니다.</p></div><button className="icon-button" type="button" onClick={onClose} aria-label="부품 미리 비교 닫기"><FiXCircle /></button></div>
      ...
      <div className="candidate-scenario-list">
        {state.items.map((item) => {
          ...
          return <article className={`candidate-scenario-card ${item.status} ${item.comparison?.direction ?? ""}`} key={item.id}>
            ...
            {item.status === "ready" && result && <div className="candidate-scenario-analysis" data-testid="candidate-scenario-analysis"><div><span>부품 적용 후 성능 분석</span><strong>{analysisScoreText(currentResult.analysis.overallScore, currentResult.analysis.scoreLabel)} → {analysisScoreText(result.analysis.overallScore, result.analysis.scoreLabel)}</strong><small>{scenarioAnalysisDetail(currentResult, result)}</small></div></div>}
