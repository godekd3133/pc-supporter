...
export function AdminCatalogSpecOverridePanel({ onToast }: { onToast: (message: string) => void }) {
  ...
  async function validateSingle() {
    ...
    try {
      ...
      onToast(result.invalidCount === 0 ? "수동 스펙 override 저장 확인을 통과했습니다." : "저장할 수 없는 항목이 있습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "수동 스펙 override 확인에 실패했습니다.");
    }
  }
  ...
  async function validateBulk() {
    ...
    try {
      ...
      onToast(result.invalidCount === 0 ? "일괄 override 저장 확인을 통과했습니다." : "일괄 override에 수정할 항목이 있습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "일괄 override 확인에 실패했습니다.");
    }
  }
  ...
  return <section className="admin-card catalog-spec-override-card" data-testid="admin-catalog-spec-override" aria-label="수동 카탈로그 스펙 보강">
    <div className="admin-card-heading catalog-spec-override-heading"><div><p className="eyebrow">MANUAL SPEC EVIDENCE</p><h3>제조사 정보 수동 스펙 보강</h3><p>원문에 없는 호환 핵심 필드는 제조사 문서 정보와 함께 입력합니다. 누락 필드만 채울 수 있고 기존 값을 덮어쓰지 않습니다.</p></div><span className="catalog-spec-override-icon"><FiEdit3 /></span></div>
    ...
    <p className="catalog-spec-override-note"><FiInfo /> 저장된 override는 원본 catalog 파일과 별도이며, 적용한 필드만 runtime에서 채웁니다. 값의 정보가 확인되지 않으면 입력하지 마세요.</p>
  </section>;
}
