...
export function AdminCatalogSpecOverridePanel({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <section className="admin-card catalog-spec-override-card" data-testid="admin-catalog-spec-override" aria-label="수동 카탈로그 스펙 보강">
    <div className="admin-card-heading catalog-spec-override-heading"><div><p className="eyebrow">MANUAL SPEC EVIDENCE</p><h3>제조사 정보 수동 스펙 보강</h3><p>상품 페이지에 없는 호환 핵심 필드는 제조사 문서 정보와 함께 입력합니다. 누락 필드만 채울 수 있고 기존 값을 덮어쓰지 않습니다.</p></div><span className="catalog-spec-override-icon"><FiEdit3 /></span></div>
