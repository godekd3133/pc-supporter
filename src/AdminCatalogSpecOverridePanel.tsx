...
export function AdminCatalogSpecOverridePanel({ onToast }: { onToast: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    void api<{ items: OverrideListItem[] }>("/api/admin/catalog-spec-overrides")
      ...
      .catch((reason: unknown) => { if (!cancelled) setOverridesError(reason instanceof Error ? reason.message : "저장된 스펙 override를 불러오지 못했습니다."); })
  ...
  async function validateSingle() {
    ...
    if (!input) {
      onToast("먼저 override를 입력할 부품을 선택해 주세요.");
      return;
    }
    ...
    try {
      ...
      onToast(result.invalidCount === 0 ? "수동 스펙 override 저장 검증을 통과했습니다." : "저장할 수 없는 항목이 있습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "수동 스펙 override 검증에 실패했습니다.");
    }
  }
  ...
  async function saveSingle() {
    ...
    if (validatedInput !== serialized || !validation || validation.invalidCount > 0) {
      onToast("저장 전 최신 JSON 검증을 먼저 통과해 주세요.");
      return;
    }
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    ...
    try {
      ...
      onToast("수동 스펙 override를 저장했고 카탈로그를 다시 계산했습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "수동 스펙 override 저장에 실패했습니다.");
    } finally {
  ...
  async function validateBulk() {
    ...
    try {
      ...
      onToast("일괄 override JSON 문법을 확인해 주세요.");
      return;
    }
    ...
    try {
      ...
      onToast(result.invalidCount === 0 ? "일괄 override 저장 검증을 통과했습니다." : "일괄 override에 수정할 항목이 있습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "일괄 override 검증에 실패했습니다.");
    }
  }
  ...
  async function saveBulk() {
    ...
    if (bulkValidatedInput !== serialized || !bulkValidation || bulkValidation.invalidCount > 0) {
      onToast("일괄 저장 전 최신 JSON 검증을 먼저 통과해 주세요.");
      return;
    }
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    ...
    try {
      ...
      resetSourceBatchProgress();
      window.dispatchEvent(new Event("pc-supporter:catalog-meta-refresh"));
      onToast("일괄 수동 스펙 override를 저장했습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "일괄 override 저장에 실패했습니다.");
    } finally {
      if (isCurrent()) setSaving(false);
    }
  }
  ...
  async function removeOverride(partId: string) {
    if (!window.confirm("이 수동 스펙 override를 제거하고 원본 catalog 값으로 되돌릴까요?")) return;
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    setSaving(true);
    try {
      ...
      window.dispatchEvent(new Event("pc-supporter:catalog-meta-refresh"));
      onToast("수동 스펙 override를 제거하고 원본 기준으로 되돌렸습니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "수동 스펙 override 제거에 실패했습니다.");
    } finally {
      if (isCurrent()) setSaving(false);
    }
  }
  ...
  async function checkSource(partId: string) {
    ...
    try {
      ...
      resetSourceBatchProgress();
      window.dispatchEvent(new Event("pc-supporter:catalog-meta-refresh"));
      onToast(`수동 스펙 근거 점검 완료 · ${sourceCheckText(result.sourceCheck)}`);
      if (sourceHistoryPartId === partId) await loadSourceHistory(partId);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "수동 스펙 근거 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
  ...
  async function checkSourcesBatch() {
    ...
    if (targets.length === 0) {
      onToast("점검할 카탈로그 스펙 HTTPS 원문 URL이 없습니다.");
      return;
    }
    ...
    try {
      ...
      onToast(`카탈로그 스펙 근거 ${range} 일괄 점검 완료 · 확인됨 ${result.passedCount}개 · 재확인 ${result.reviewCount}개${result.persistFailureCount > 0 ? ` · 저장 실패 ${result.persistFailureCount}개` : ""}${result.nextOffset !== undefined ? ` · 다음 ${result.nextOffset}번부터 계속` : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "카탈로그 스펙 근거 일괄 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) {
        setSourceCheckingPartId(null);
        setSaving(false);
  ...
  async function loadSourceHistory(partId: string) {
    ...
    try {
      ...
      if (!isCurrent()) return;
      setSourceHistoryError(reason instanceof Error ? reason.message : "수동 스펙 근거 점검 이력을 불러오지 못했습니다.");
    } finally {
  ...
  return <section className="admin-card catalog-spec-override-card" data-testid="admin-catalog-spec-override" aria-label="수동 카탈로그 스펙 보강">
    <div className="admin-card-heading catalog-spec-override-heading"><div><p className="eyebrow">MANUAL SPEC EVIDENCE</p><h3>제조사 근거 수동 스펙 보강</h3><p>원문에 없는 호환 핵심 필드는 제조사 문서 근거와 함께 입력합니다. 누락 필드만 채울 수 있고 기존 값을 덮어쓰지 않습니다.</p></div><span className="catalog-spec-override-icon"><FiEdit3 /></span></div>
    ...
    <p className="catalog-spec-override-note"><FiInfo /> 저장된 override는 원본 catalog 파일과 별도이며, 적용한 필드만 runtime에서 채웁니다. 값의 근거가 확인되지 않으면 입력하지 마세요.</p>
  </section>;
}
