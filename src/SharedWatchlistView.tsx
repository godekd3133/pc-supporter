...
const SHARED_WATCHLIST_DECISION_LABELS: Record<PriceWatchDecisionState, string> = { target: "목표가 도달", buy: "구매 검토", wait: "하락 대기", observe: "재상승 관찰", tracking: "목표가 관찰 중", unavailable: "가격 정보 없음", error: "확인 오류" };
...
export function SharedWatchlistView({ onBack, onImport }: { onBack: () => void; onImport: (saved: SavedCatalogWatchlist) => void }) {
  ...
  useEffect(() => {
    ...
    void api<SavedCatalogWatchlist>("/api/watchlists/" + encodeURIComponent(watchlistId), { signal: controller.signal })
      ...
      .catch((reason: unknown) => { if (!cancelled) setError(reason instanceof Error ? reason.message : "공유 가격 목록을 불러오지 못했습니다."); })
  ...
  return <div className="shared-watchlist-page"><div className="workspace-heading"><div><button className="back-link" type="button" onClick={onBack}><FiArrowLeft /> 홈으로</button><p className="eyebrow">가격 목록 공유</p><h1>{saved?.name ?? (loading ? "관심 가격 목록을 불러오는 중" : "공유 가격 목록")}</h1><p>공유 당시 관심 항목과 목표가를 보고 내 목록에 추가할 수 있어요.</p></div><span className="admin-badge"><FiShare2 /> 공유 목록</span></div>{loading ? <div className="shared-watchlist-state"><FiLoader className="spin" /> 공유 가격 목록을 불러오는 중...</div> : error ? <div className="shared-watchlist-state error" role="alert"><FiXCircle /><span>{error}</span><div className="shared-watchlist-error-actions"><button className="button button-light" type="button" data-testid="shared-watchlist-retry" onClick={() => setRetryNonce((current) => current + 1)}><FiRefreshCw /> 다시 시도</button><button className="text-button" type="button" onClick={onBack}>홈으로</button></div></div> : saved && <section className="shared-watchlist-card"><div className="shared-watchlist-card-heading"><div><p className="eyebrow">저장한 가격 목록</p><h2>{saved.name}</h2><small>저장 {new Date(saved.createdAt).toLocaleString("ko-KR")} · 수정 {new Date(saved.updatedAt).toLocaleString("ko-KR")} · 최저가 근접 알림 {saved.nearLowThresholdPercent}% · 알림 {priceAlertPolicyText(saved.alertPreferences)} · {saved.expiresAt ? "만료 " + new Date(saved.expiresAt).toLocaleString("ko-KR") : "무기한"}</small></div><div className="shared-watchlist-card-actions"><button className="button button-secondary" type="button" onClick={() => void refreshLivePrices()} disabled={livePriceLoading}>{livePriceLoading ? <><FiLoader className="spin" /> 확인 중...</> : <><FiRefreshCw /> 가격 갱신</>}</button><button className="button button-primary" type="button" onClick={() => onImport(saved)}><FiDownload /> 내 관심 목록에 추가</button></div></div>{livePriceCheckedAt && <p className="shared-watchlist-live-status"><FiClock /> 현재 가격 확인 {new Date(livePriceCheckedAt).toLocaleString("ko-KR")}{priceHistoryLoading ? " · 가격 이력 확인 중..." : priceHistoryError ? " · 가격 이력 불러오기 실패" : " · 최근 " + priceHistoryDays + "일 가격 정보 반영"}</p>}{saved && <div className="shared-watchlist-history-controls"><label><span>가격 이력 기간</span><select aria-label="공유 가격 이력 기간" value={priceHistoryDays} onChange={(event) => setPriceHistoryDays(Number(event.target.value) as SharedWatchlistPriceHistoryWindow)}><option value={7}>7일</option><option value={30}>30일</option><option value={90}>90일</option></select></label><small>기간을 바꿔도 공유 목록이나 내 목록은 달라지지 않아요.</small></div>}{priceHistoryError && <p className="shared-watchlist-price-history-error" role="alert"><FiXCircle /> {priceHistoryError}</p>}{decisionCounts && <div className="price-watchlist-decision-overview shared-watchlist-decision-overview" aria-label="공유 목록 항목별 가격 상태" data-testid="shared-watchlist-decision-overview"><strong>항목별 가격 상태</strong><div>{(Object.keys(SHARED_WATCHLIST_DECISION_LABELS) as PriceWatchDecisionState[]).map((state) => <span className={state} key={state}>{SHARED_WATCHLIST_DECISION_LABELS[state]} <b>{decisionCounts[state]}</b></span>)}</div></div>}<div className="shared-watchlist-entries">{saved.entries.map((entry) => { const key = entry.kind + ":" + entry.itemId; const live = livePrices[key]; const history = priceHistories[key]; const decision = priceWatchDecisionFor({ currentStatus: live?.status ?? "unknown", currentPriceWon: live?.priceWon, targetPriceWon: entry.targetPriceWon, nearLowThresholdPercent: saved.nearLowThresholdPercent, history: history?.summary }); const targetGapWon = live?.status === "available" && live.priceWon !== undefined && entry.targetPriceWon !== undefined && live.priceWon > entry.targetPriceWon ? live.priceWon - entry.targetPriceWon : undefined; return <article className="shared-watchlist-entry" key={key}><div><span>{entry.kind === "accessory" ? "주변 부품" : "핵심 부품"} · {entry.category}</span><strong>{entry.itemName}</strong><small>{entry.itemId}</small></div><div className="shared-watchlist-entry-prices"><div>{entry.targetPriceWon !== undefined ? <><span>목표가</span><strong>{entry.targetPriceWon.toLocaleString("ko-KR")}원</strong></> : <span>목표가 미설정</span>}</div><div className="shared-watchlist-entry-current"><span>현재 가격</span>{live?.status === "available" ? <strong>{live.priceWon !== undefined ? live.priceWon.toLocaleString("ko-KR") + "원" : "가격 정보 없음"}</strong> : live?.status === "unavailable" ? <strong>가격 확인 불가</strong> : live?.status === "error" ? <strong>일시 확인 오류</strong> : <strong>미확인</strong>}{live?.sourceUrl && <a className="shared-watchlist-source-link" href={live.sourceUrl} target="_blank" rel="noreferrer">상품 페이지 <FiExternalLink /></a>}<em className={"shared-watchlist-decision " + decision.state} data-testid="shared-watchlist-decision">{decision.label}</em><small className="shared-watchlist-decision-summary">{decision.summary}</small>{targetGapWon !== undefined && <small className="shared-watchlist-target-gap">목표가까지 +{formatWon(targetGapWon)}</small>}{history && history.summary.sampleCount >= 2 && <div className="shared-watchlist-history" data-testid="shared-watchlist-price-history"><small className="shared-watchlist-history-summary">최근 {history.windowDays}일 {history.summary.sampleCount}회 · {history.summary.minPriceWon !== undefined ? "최저 " + formatWon(history.summary.minPriceWon) : "최저가 정보 없음"}{history.summary.currentPositionPercent !== undefined ? " · 현재 위치 " + history.summary.currentPositionPercent.toFixed(1) + "%" : ""}</small>{history.points.length > 0 && <div className="price-watchlist-sparkline" role="img" aria-label={entry.itemName + " 최근 " + history.windowDays + "일 공유 가격 추세"}>{history.points.map((point) => <span key={point.changeId} style={{ height: historyBarHeight(history, point.priceWon) + "%" }} title={point.priceWon.toLocaleString("ko-KR") + "원"} />)}</div>}</div>}</div></div></article>; })}</div></section>}</div>;
}
