...
export function AccessoryDetailPanel({ item, selected, isWatched, isCachedFallback = false, cachedAt, refreshing = false, onRefresh, refreshMessage, refreshError, refreshDiffs, onAdd, onWatch, onOpenWatchlist, onClose }: { item: AccessoryItem; selected: boolean; isWatched: boolean; isCachedFallback?: boolean; cachedAt?: string; refreshing?: boolean; onRefresh?: () => void; refreshMessage?: string; refreshError?: string; refreshDiffs?: CatalogChangeValueDiff[]; onAdd: () => void; onWatch: () => boolean; onOpenWatchlist: () => void; onClose: () => void }) {
  ...
  return <section className="accessory-detail-panel" aria-label="선택한 주변 부품 상세" data-testid="accessory-detail-panel">
    ...
    {isCachedFallback && <div className="accessory-detail-cache-state" data-testid="accessory-detail-cache-state" role="status"><span><FiDatabase /></span><div><strong>캐시 기반 상세</strong><p>서버 목록이 복구되기 전까지 브라우저에 저장된 기본 정보로 표시합니다. 이 상태는 최신 상품 정보·실시간 가격·호환 확인 결과가 아니에요.</p><small>캐시 저장 {cachedLabel}</small></div>{onRefresh && <button className="button button-small button-light" type="button" data-testid="accessory-detail-refresh" onClick={onRefresh} disabled={refreshing}>{refreshing ? <><FiLoader className="spin" /> 정보 확인 중...</> : <><FiRefreshCw /> 정보 다시 확인</>}</button>}</div>}
    ...
    {!isCachedFallback && onRefresh && <div className="accessory-detail-refresh" data-testid="accessory-detail-refresh-bar"><div><FiRefreshCw /><span><strong>상품 정보·가격 다시 확인</strong><small>저장된 다나와 상품 페이지를 다시 읽어 변경 이력을 남깁니다.</small></span></div><button className="button button-small button-light" type="button" data-testid="accessory-detail-refresh" onClick={onRefresh} disabled={refreshing}>{refreshing ? <><FiLoader className="spin" /> 확인 중...</> : <><FiRefreshCw /> 지금 확인</>}</button></div>}
    ...
    {rows.length > 0 ? <dl className="accessory-detail-specs">{rows.map(([label, value]) => <div key={`${label}-${value}`}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : <p className="accessory-detail-empty"><FiInfo /> 정리된 규격을 확인하지 못했어요. 수집된 스펙을 확인해 주세요.</p>}
    {item.missingFields.length > 0 && <p className="accessory-detail-missing"><FiInfo /> 확인되지 않은 정보 {item.missingFields.slice(0, 5).map((field) => catalogMissingFieldLabelFor(field)).join(", ")}{item.missingFields.length > 5 ? ` 외 ${item.missingFields.length - 5}개` : ""}</p>}
    {item.rawSpecText && <details className="accessory-detail-raw"><summary>저장된 스펙 보기</summary><p>{item.rawSpecText}</p></details>}
