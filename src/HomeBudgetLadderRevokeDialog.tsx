...
export function HomeBudgetLadderRevokeDialog({ entry, submitting, onClose, onConfirm }: { entry: BudgetLadderLocalShareEntry; submitting: boolean; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !submitting) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="home-budget-ladder-revoke-dialog" role="alertdialog" aria-modal="true" aria-labelledby="budget-ladder-revoke-title" aria-describedby="budget-ladder-revoke-description">
      <div className="modal-header"><div><h2 id="budget-ladder-revoke-title">공유 저장본을 취소할까요?</h2><p>서버에서 링크를 폐기하기 전에 대상을 확인하세요.</p></div><button className="icon-button" type="button" onClick={onClose} disabled={submitting} aria-label="공유 취소 확인창 닫기"><FiXCircle /></button></div>
