...
export function HomeBudgetLadderRevokeDialog({ entry, submitting, onClose, onConfirm }: { entry: BudgetLadderLocalShareEntry; submitting: boolean; onClose: () => void; onConfirm: () => void }) {
  const modalRef = useModalAccessibility({ onClose, closeOnEscape: !submitting });
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !submitting) onClose(); }}>
    <section ref={modalRef} tabIndex={-1} className="home-budget-ladder-revoke-dialog" role="alertdialog" aria-modal="true" aria-labelledby="budget-ladder-revoke-title" aria-describedby="budget-ladder-revoke-description">
      <div className="modal-header"><div><p className="eyebrow">REVOKE SHARE</p><h2 id="budget-ladder-revoke-title">공유 저장본을 취소할까요?</h2><p>서버에서 링크를 폐기하기 전에 대상을 확인하세요.</p></div><button className="icon-button" type="button" onClick={onClose} disabled={submitting} aria-label="공유 취소 확인창 닫기"><FiXCircle /></button></div>
      <div className="home-budget-ladder-revoke-body">
        <div className="home-budget-ladder-revoke-warning"><FiAlertTriangle /><div><strong>이 작업은 공유받은 사람에게도 영향을 줍니다.</strong><p id="budget-ladder-revoke-description">취소 후에는 이 링크로 예산 비교 저장본을 다시 열 수 없습니다. 현재 브라우저의 최근 공유 이력에서도 함께 제거됩니다.</p></div></div>
