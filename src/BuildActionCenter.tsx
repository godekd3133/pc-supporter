...
import { PURCHASE_CHECKLIST_ACTION_EVENT, PURCHASE_CHECKLIST_CHANGE_EVENT, PURCHASE_CHECKLIST_MAX_CHECKED_ITEMS, purchaseChecklistItemsFor } from "../shared/purchase-checklist";
...
function actionButtonText(action: BuildAction) {
  return action.ruleId ? "항목 보기" : action.targetId === "gpu-fit-summary-panel" ? "GPU 장착 보기" : action.targetId === "build-resource-summary" ? "전력·냉각 보기" : action.targetId === "data-health-panel" ? "데이터 보기" : action.targetId === "purchase-list-panel" ? "구매 목록 보기" : action.targetId === "repair-plan-panel" ? "수리 플랜 보기" : action.targetId === "build-connectivity-panel" ? "연결 자원 보기" : "체크리스트 보기";
}
...
export function BuildActionCenterPanel({ build, result, partMap, checklistStorageKey, onFocusFinding, onFocusSection, onFocusRepairPlans }: { build: BuildSelection; result: CompatibilityResult; partMap: ReadonlyMap<string, Part>; checklistStorageKey: string; onFocusFinding: (ruleId: string) => void; onFocusSection: (targetId: string) => void; onFocusRepairPlans: () => void }) {
  ...
  return <section className={`build-action-center ${center.state}`} aria-label="우선 할 일 목록" data-testid="build-action-center" tabIndex={-1}><div className="build-action-center-heading"><div><p className="eyebrow">NEXT ACTIONS</p><h2>지금 먼저 확인할 것</h2><p>{center.summary}</p></div><span className="build-action-center-status"><StatusIcon /> {center.state === "blocked" ? "구매 보류" : center.state === "review" ? "확인 후 진행" : "최종 확인"}</span></div>{actionProgress.total > 0 && <div className="build-action-center-progress"><div><span>우선 할 일 진행</span><b>{actionProgress.checked} / {actionProgress.total}개</b><em>{actionProgress.percent}%</em></div><div className="build-action-center-progress-track" role="progressbar" aria-label={`우선 할 일 ${actionProgress.percent}% 완료`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={actionProgress.percent}><span style={{ width: `${actionProgress.percent}%` }} /></div></div>}<div className="build-action-center-list">{visibleActions.map((action) => { const Icon = sourceIcons[action.source]; const checklistIds = actionChecklistIdsFor(action.id, checklistItemIdSet); const checked = actionChecklistCheckedFor(action.id, checklistItemIdSet, checkedIdSet); return <article className={`build-action-item ${action.priority}${checked ? " checked" : ""}`} key={action.id}><div className="build-action-item-icon"><Icon /></div><div className="build-action-item-copy"><div><span className={`build-action-priority ${action.priority}`}>{priorityLabels[action.priority]}</span><span className="build-action-source">{sourceLabels[action.source]}</span>{checked && <span className="build-action-complete">체크 완료</span>}</div><strong>{action.title}</strong><p>{action.summary}</p></div><div className="build-action-item-actions">{checklistIds.length > 0 && <button className="text-button build-action-item-complete" type="button" aria-label={`${action.title} ${checked ? "완료 취소" : "완료로 표시"}`} onClick={() => toggleChecklistAction(action.id, !checked)}>{checked ? "완료 취소" : "완료로 표시"}</button>}{(action.ruleId || action.targetId) && <button className="text-button build-action-item-button" type="button" onClick={() => action.ruleId ? onFocusFinding(action.ruleId) : action.targetId === "repair-plan-panel" ? onFocusRepairPlans() : onFocusSection(action.targetId!)}>{actionButtonText(action)} <FiSearch /></button>}</div></article>; })}</div>{center.hiddenCount > 0 && <button className="text-button build-action-center-more" type="button" onClick={() => onFocusSection("purchase-checklist")}><FiInfo /> 나머지 {center.hiddenCount}개 확인하기</button>}</section>;
}
