...
function analysisDeltaText(summary: ReturnType<typeof savedBuildCheckTransitionSummaryFor>, changed: boolean) {
  if (!changed) return "점수·라벨·근거 수준 동일";
  if (summary.analysisScoreDelta === undefined) return "점수·라벨·근거 수준 변화 확인";
  if (summary.analysisScoreDelta === 0) return "성능 지수 동일 · 라벨·근거 수준 변화 확인";
  return `성능 지수 ${summary.analysisScoreDelta > 0 ? "+" : ""}${summary.analysisScoreDelta}점 · 신뢰도 변화 확인`;
}
...
function resourceBudgetDeltaText(summary: ReturnType<typeof savedBuildCheckTransitionSummaryFor>, changed: boolean) {
  ...
  return values.length > 0 ? values.join(" · ") : "예산 상태·근거 수준 변화 확인";
}
...
function benchmarkImpactDetail(impact: ReturnType<typeof savedBuildCheckTransitionSummaryFor>["benchmarkImpact"]) {
  if (impact.status === "not_recorded") return "저장 당시와 현재 모두 benchmark snapshot이 없습니다.";
  if (impact.status === "unverified") return "저장 당시 또는 현재 근거가 부족해 성능 판단 영향을 확정할 수 없습니다.";
  ...
  if (impact.benchmarkDateChanged) return "benchmark 자료 시점이 변경되었습니다.";
...
function refreshNextActionsFor(item: CatalogRefreshReportItem) {
  ...
  if (item.changedFields.some((field) => field === "원문 스펙" || field === "정규화 스펙" || field === "데이터 품질" || field === "누락 필드")) actions.push({ id: "result-findings", label: "현재 판정 다시 확인" });
  return actions;
}
...
function findingPlanResolutionLabel(resolution: FindingPlanResolution) {
  return resolution === "resolved" ? "해결" : resolution === "remaining" ? "남음" : "근거 확인 필요";
}
...
function SavedBuildRecheckRefreshImpactPanel({ report, findingChanges, result, onFocusFinding, onFocusSection }: { report: CatalogRefreshReport; findingChanges: ReadonlyArray<SavedBuildCheckFindingDiff>; result: CompatibilityResult; onFocusFinding?: (ruleId: string) => void; onFocusSection?: (targetId: string) => void }) {
  ...
  return <section className={`saved-build-recheck-refresh-impact ${report.status}`} aria-label="원문 재확인과 현재 재검사 연결" data-testid="saved-build-recheck-refresh-impact">
    <div className="saved-build-recheck-refresh-impact-heading"><div><p className="eyebrow">REFRESH → CURRENT CHECK</p><strong>원문 재확인과 현재 판정 연결</strong><small>저장 당시 입력에 대해 확인한 부품과 현재 재검사 finding의 부품 ID가 겹치는 관측만 연결합니다.</small></div><span>{linkedCount}개 연결</span></div>
    {report.failures.length > 0 && <p className="saved-build-recheck-refresh-impact-failure"><FiAlertTriangle /> 원문 확인 실패 {report.failures.length}개는 현재 판정과 연결할 수 없습니다.</p>}
    {impacts.length > 0 ? <div className="saved-build-recheck-refresh-impact-list">{impacts.slice(0, 6).map(({ item, findingChanges: linkedChanges }) => {
      ...
      return <article className={linkedChanges.length > 0 ? "linked" : "unlinked"} key={`${item.target.kind}-${item.target.id}`}>
        <div className="saved-build-recheck-refresh-impact-item-heading"><strong>{item.name}</strong><span>{linkedChanges.length > 0 ? `판정 변화 ${linkedChanges.length}개` : "판정 연결 없음"}</span></div>
        ...
        {linkedChanges.length > 0 ? <ul>{linkedChanges.slice(0, 4).map((change) => <li key={`${change.key}-${change.change}`}><b>{findingChangeLabel(change.change)}</b><span>{(change.after ?? change.before)?.title ?? change.key}</span></li>)}</ul> : <p>현재 재검사에서 이 부품과 겹치는 finding 변화는 관측되지 않았습니다. 원문 변경이 판정 원인이라고 단정하지 않습니다.</p>}
        ...
        {focusableFinding && onFocusFinding && <button className="text-button" type="button" onClick={() => onFocusFinding(focusableFinding.ruleId)}><FiSearch /> 연결된 현재 finding 보기</button>}
      </article>;
    })}</div> : report.failures.length === 0 && <p className="saved-build-recheck-refresh-impact-empty"><FiInfo /> 성공한 원문 확인 항목이 없어 현재 판정 연결을 계산할 수 없습니다.</p>}
    ...
    <p className="saved-build-recheck-refresh-impact-note"><FiInfo /> 이 연결은 대상 부품 ID와 finding 영향 부품 ID가 겹친다는 참고 근거입니다. 원문 변경과 판정 사이의 인과관계를 확정하지 않습니다.</p>
  </section>;
}
...
export function SavedBuildRecheckDiffPanel({ snapshot, result, partMap, onFocusFinding, onPreviewSuggestion, onFocusRepairPlans, onFocusSection }: { snapshot: SavedBuildCheckSnapshot; result: CompatibilityResult; partMap: ReadonlyMap<string, Part>; onFocusFinding?: (ruleId: string) => void; onPreviewSuggestion?: (category: Part["category"], part: Part, quantity?: number, affectedPartIds?: string[]) => void; onFocusRepairPlans?: () => void; onFocusSection?: (targetId: string) => void }) {
  ...
  return <section className={`saved-build-recheck-diff-panel ${summary.direction}`} aria-label="저장 당시와 현재 재검사 비교" data-testid="saved-build-recheck-diff" tabIndex={-1}>
    <div className="saved-build-recheck-diff-heading"><div><p className="eyebrow">SAVED → CURRENT RECHECK</p><h2>저장 당시와 현재 재검사 비교</h2><p>저장 snapshot과 현재 카탈로그·규칙 엔진으로 다시 계산한 결과를 분리해 보여줍니다.</p></div><strong><DirectionIcon /> {directionLabel(summary.direction)}</strong></div>
    <div className="saved-build-recheck-diff-grid">
      <article className={diff.statusChanged ? "changed" : undefined}><span>판정</span><strong>{statusLabel(snapshot.status)} → {statusLabel(result.status)}</strong><small>{diff.statusChanged ? "판정이 달라졌습니다." : "저장 당시와 현재 판정이 같습니다."}</small></article>
      ...
      <article className={benchmarkChanged ? "changed" : undefined} data-testid="saved-build-recheck-benchmark"><span>CPU·GPU 근거</span><strong>{buildBenchmarkImpactStatusText(diff.benchmarkImpact.status)} · {buildBenchmarkDecisionImpactText(diff.benchmarkImpact.decisionImpact)}</strong><small>{benchmarkSnapshotText(snapshot.benchmarkSnapshot)} → {benchmarkSnapshotText(result.benchmarkSnapshot)} · {benchmarkImpactDetail(diff.benchmarkImpact)}</small></article>
      <article className={metadataChanged ? "changed" : undefined}><span>검사 기준</span><strong>{diff.catalogChanged ? "카탈로그 기준 변경" : "카탈로그 기준 동일"}</strong><small>{diff.engineChanged ? `엔진 ${snapshot.engineVersion} → ${result.engineVersion}` : `엔진 ${result.engineVersion}`}</small></article>
    </div>
    <div className="saved-build-recheck-diff-facts"><span>저장 당시 <b>{new Date(snapshot.checkedAt).toLocaleString("ko-KR", { dateStyle: "medium", timeStyle: "short" })}</b></span><span>현재 재검사 <b>{new Date(result.checkedAt).toLocaleString("ko-KR", { dateStyle: "medium", timeStyle: "short" })}</b></span><span>{summary.findingDiffAvailable ? `finding 해결 ${summary.resolvedFindingCount}개 · 신규 ${summary.newFindingCount}개` : "구버전 snapshot · finding 상세 비교 불가"}</span></div>
    {(riskChanged || priceChanged || analysisChanged || resourceBudgetChanged || benchmarkChanged || metadataChanged || diff.statusChanged) && <p className="saved-build-recheck-diff-warning"><FiInfo /> 저장 당시 결과와 현재 결과가 달라졌거나 benchmark 근거를 확정할 수 없습니다. 현재 카탈로그 기준 결과를 우선 참고하되, 구매 전 변경된 finding·성능 분석·전력·냉각 예산·benchmark 원문을 다시 확인하세요.</p>}
    {!diff.hasChanges && <p className="saved-build-recheck-diff-same"><FiCheckCircle /> 저장 당시 snapshot과 현재 재검사에서 판정·위험·가격·성능 분석·전력·냉각 예산·benchmark 근거·검사 기준의 변화가 확인되지 않았습니다.</p>}
    ...
    <div className="saved-build-recheck-diff-findings" data-testid="saved-build-recheck-diff-findings">
      <div className="saved-build-recheck-diff-findings-heading"><strong>변경된 finding</strong><span>{findingDiff.available ? `${changedFindings.length}개` : "비교 불가"}</span></div>
      {!findingDiff.available ? <p className="saved-build-recheck-diff-findings-empty"><FiInfo /> 구버전 snapshot에는 finding 상세가 없어 규칙별 비교를 제공하지 않습니다.</p> : changedFindings.length === 0 ? <p className="saved-build-recheck-diff-findings-empty"><FiCheckCircle /> 저장 당시와 현재 재검사 사이에 변경된 finding이 없습니다.</p> : <div className="saved-build-recheck-diff-findings-list">{changedFindings.slice(0, 4).map((change) => {
        ...
        return <article className={`saved-build-recheck-diff-finding ${findingChangeTone(change.change)}`} key={`${change.key}-${change.change}`}>
          ...
          {currentFinding && strategyPlans.length > 0 && <div className="saved-build-recheck-finding-strategies" data-testid={`saved-build-recheck-finding-strategies-${change.key}`}>
            <div className="saved-build-recheck-finding-strategies-heading"><strong>이 finding 기준 전략</strong><span>현재 플랜 연결</span></div>
            <div className="saved-build-recheck-finding-strategies-list">{strategyPlans.map((plan) => {
              ...
              return <article className={resolution} key={`${change.key}-${plan.label}`}><div><strong>{plan.label}</strong><span>{findingPlanResolutionLabel(resolution)}</span></div><small>잔여 차단 {plan.remainingBlockers}개 · 가격 {priceDeltaText(plan.priceDeltaWon, plan.priceComplete)}</small>{changes.length > 0 ? <p>{changes.slice(0, 2).map((item) => item.kind === "change_quantity" ? `${item.fromQuantity ?? "?"}개 → ${item.toQuantity ?? "?"}개` : `${item.fromPartName ?? "현재 선택"} → ${item.toPart.name}`).join(" · ")}{changes.length > 2 ? ` 외 ${changes.length - 2}개` : ""}</p> : <p>{resolution === "resolved" ? "이 플랜의 해결 범위에 포함됩니다." : resolution === "remaining" ? "플랜 적용 후에도 이 finding이 남습니다." : "플랜의 finding 연결 근거를 확인하세요."}</p>}</article>;
            })}</div>
          </div>}
          {currentFinding && !recommendation && onFocusFinding && <button className="text-button" type="button" onClick={() => onFocusFinding(currentFinding.ruleId)}><FiSearch /> 현재 finding 보기</button>}
        </article>;
      })}</div>}
      {changedFindings.length > 4 && <small className="saved-build-recheck-diff-findings-more">그 외 변경된 finding {changedFindings.length - 4}개는 현재 상세 판정에서 확인하세요.</small>}
    </div>
    {strategyPlans.length > 0 && <div className="saved-build-recheck-strategies" data-testid="saved-build-recheck-strategies">
      <div className="saved-build-recheck-strategies-heading"><div><strong>현재 결과 해결 전략 비교</strong><small>변경된 finding을 줄이는 세 가지 방향을 같은 현재 재검사 기준으로 비교합니다.</small></div><span>{strategyPlans.length}안</span></div>
    ...
    {onFocusSection && <div className="saved-build-recheck-diff-actions"><button className="button button-light" type="button" onClick={() => onFocusSection("result-findings")}><FiSearch /> 현재 상세 판정 보기</button><button className="text-button" type="button" onClick={() => onFocusSection("saved-build-check-timeline")}><FiRefreshCw /> 검사 타임라인 보기</button></div>}
