...
function directionLabel(direction: ReturnType<typeof savedBuildCheckTransitionSummaryFor>["direction"]) {
  return direction === "improved" ? "확인 항목 감소" : direction === "regressed" ? "확인 항목 증가" : direction === "changed" ? "일부 변경" : "변화 없음";
}
...
function findingPlanResolutionLabel(resolution: FindingPlanResolution) {
  return resolution === "resolved" ? "해결" : resolution === "remaining" ? "남음" : "정보 부족";
}
...
function SavedBuildRecheckRefreshImpactPanel({ report, findingChanges, result, onFocusFinding, onFocusSection }: { report: CatalogRefreshReport; findingChanges: ReadonlyArray<SavedBuildCheckFindingDiff>; result: CompatibilityResult; onFocusFinding?: (ruleId: string) => void; onFocusSection?: (targetId: string) => void }) {
  ...
  return <section className={`saved-build-recheck-refresh-impact ${report.status}`} aria-label="부품 가격·사양 변경" data-testid="saved-build-recheck-refresh-impact">
    <div className="saved-build-recheck-refresh-impact-heading"><div><strong>변경된 부품 정보</strong><small>가격·사양 변경과 호환 결과 변화를 표시합니다.</small></div><span>결과 변화 {linkedCount}개</span></div>
    {report.failures.length > 0 && <p className="saved-build-recheck-refresh-impact-failure"><FiAlertTriangle /> 부품 정보를 불러오지 못한 항목 {report.failures.length}개</p>}
...
export function SavedBuildRecheckDiffPanel({ snapshot, result, partMap, onFocusFinding, onPreviewSuggestion, onFocusRepairPlans, onFocusSection }: { snapshot: SavedBuildCheckSnapshot; result: CompatibilityResult; partMap: ReadonlyMap<string, Part>; onFocusFinding?: (ruleId: string) => void; onPreviewSuggestion?: (category: Part["category"], part: Part, quantity?: number, affectedPartIds?: string[]) => void; onFocusRepairPlans?: () => void; onFocusSection?: (targetId: string) => void }) {
  ...
  return <section className={`saved-build-recheck-diff-panel ${summary.direction}`} aria-label="저장 당시와 현재 비교" data-testid="saved-build-recheck-diff" tabIndex={-1}>
    <div className="saved-build-recheck-diff-heading"><div><p className="eyebrow">저장 견적 변경</p><h2>저장 당시와 현재 비교</h2><p>저장 당시와 현재의 가격·호환 결과를 비교해요.</p></div><strong><DirectionIcon /> {directionLabel(summary.direction)}</strong></div>
    ...
    <div className="saved-build-recheck-diff-findings" data-testid="saved-build-recheck-diff-findings">
      ...
      {!findingDiff.available ? <p className="saved-build-recheck-diff-findings-empty"><FiInfo /> 구버전 저장본에는 항목 상세가 없어 규칙별 비교를 제공하지 않습니다.</p> : changedFindings.length === 0 ? <p className="saved-build-recheck-diff-findings-empty"><FiCheckCircle /> 저장 당시와 현재 결과가 같아요.</p> : <div className="saved-build-recheck-diff-findings-list">{changedFindings.slice(0, 4).map((change) => {
        ...
        return <article className={`saved-build-recheck-diff-finding ${findingChangeTone(change.change)}`} key={`${change.key}-${change.change}`}>
          ...
          <div className="saved-build-recheck-diff-finding-versions"><div><small>저장 당시</small><p>{beforeFinding ? beforeFinding.message : "이 결과에는 기록되지 않았어요."}</p></div><b aria-hidden="true">→</b><div><small>현재 결과</small><p>{afterFinding ? afterFinding.message : "현재 결과에서 해결되었습니다."}</p></div></div>
    ...
    {strategyPlans.length > 0 && <div className="saved-build-recheck-strategies" data-testid="saved-build-recheck-strategies">
      <div className="saved-build-recheck-strategies-heading"><div><strong>바꿔볼 조합 비교</strong><small>현재 결과와 비교해 바뀌는 항목을 줄인 세 가지 안을 보여줘요.</small></div><span>{strategyPlans.length}안</span></div>
      <div className="saved-build-recheck-strategies-list">{strategyPlans.map((plan, index) => <article className={index === 0 ? "recommended" : undefined} key={`${plan.label}-${plan.title}`}>
        ...
        <div className="saved-build-recheck-strategy-stats"><span>변경 <b>{plan.changes.length}개</b></span><span>차단 해결 <b>{plan.resolvedBlockers}개</b></span><span>남은 위험 <b>{plan.remainingBlockers}/{plan.remainingWarnings}/{plan.remainingUnknown}</b></span><span>가격 <b>{priceDeltaText(plan.priceDeltaWon, plan.priceComplete)}</b></span><span>적용 후 <b>{plan.priceComplete ? `${plan.afterTotalPriceWon.toLocaleString("ko-KR")}원` : "정보 부족"}</b></span></div>
        <p>{plan.reason}</p>
    ...
    {onFocusSection && <div className="saved-build-recheck-diff-actions"><button className="button button-light" type="button" onClick={() => onFocusSection("result-findings")}><FiSearch /> 현재 상세 결과 보기</button><button className="text-button" type="button" onClick={() => onFocusSection("saved-build-check-timeline")}><FiRefreshCw /> 가격·호환 기록 보기</button></div>}
    <p className="saved-build-recheck-diff-note"><FiInfo /> 저장 당시와 현재 견적의 차이예요.</p>
  </section>;
}
