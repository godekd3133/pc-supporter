  ...
  return <section className={`saved-build-recheck-refresh-impact ${report.status}`} aria-label="정보 다시 확인과 이번 검사 연결" data-testid="saved-build-recheck-refresh-impact">
    ...
    {impacts.length > 0 ? <div className="saved-build-recheck-refresh-impact-list">{impacts.slice(0, 6).map(({ item, findingChanges: linkedChanges }) => {
      ...
      return <article className={linkedChanges.length > 0 ? "linked" : "unlinked"} key={`${item.target.kind}-${item.target.id}`}>
        ...
        <small>{item.target.kind === "part" ? "핵심 부품" : "주변 부품"} · 확인 {refreshDateText(item.refreshedAt)} · 변경 {item.changedFields.length > 0 ? item.changedFields.map((field) => catalogChangeFieldLabelFor(field)).join(" · ") : "변경된 영역 없음"}</small>
        ...
        {linkedChanges.length > 0 ? <ul>{linkedChanges.slice(0, 4).map((change) => <li key={`${change.key}-${change.change}`}><b>{findingChangeLabel(change.change)}</b><span>{(change.after ?? change.before)?.title ?? change.key}</span></li>)}</ul> : <p>현재 재검사에서 이 부품과 겹치는 항목 변화는 관측되지 않았습니다. 정보 변경이 결과의 원인이라고 단정하지 않습니다.</p>}
    ...
    })}</div> : report.failures.length === 0 && <p className="saved-build-recheck-refresh-impact-empty"><FiInfo /> 성공한 정보 확인 항목이 없어 현재 결과 연결을 계산할 수 없습니다.</p>}
    {report.items.length > 6 && <small className="saved-build-recheck-refresh-impact-more">그 외 확인 항목 {report.items.length - 6}건은 구매 목록의 정보 다시 확인 결과에서 확인할 수 있습니다.</small>}
    <p className="saved-build-recheck-refresh-impact-note"><FiInfo /> 이 연결은 대상 부품 ID와 항목 영향 부품 ID가 겹친다는 참고 정보입니다. 정보 변경과 결과 사이의 인과관계를 정하지 않아요.</p>
  </section>;
}
