...
function statusText(status: SavedBuildVersionShareCheck["status"] | undefined) {
  return status === "compatible" ? "호환 가능" : status === "needs_review" ? "정보 부족" : status === "incompatible" ? "호환 불가" : "검사 저장본 없음";
}
...
function formatPrice(value: number | undefined, complete: boolean | undefined) {
  return complete && value !== undefined ? `${value.toLocaleString("ko-KR")}원` : "가격 정보 없음";
}

function signed(value: number | undefined) {
  if (value === undefined) return "정보 부족";
...
function savedToCurrentDirection(saved: SavedBuildVersionShareCheck | undefined, current: CurrentVersionCheck["current"]) {
  ...
  return currentRank > savedRank ? "현재 기준 개선" : currentRank < savedRank ? "현재 기준 위험 증가" : saved.blockerCount !== current.blockerCount || saved.warningCount !== current.warningCount || saved.unknownCount !== current.unknownCount ? "호환 항목 변경" : "결과 변화 없음";
}
...
export function SharedSavedBuildVersionView({ onBack, onToast }: Props) {
  ...
  return <div className="shared-version-comparison-page">
    <div className="workspace-heading"><div><button className="back-link" type="button" onClick={onBack}><FiArrowLeft /> 홈으로</button><h1>공유된 견적 버전 비교</h1><p>두 견적의 부품·가격·호환 결과를 비교해요.</p></div><span className="admin-badge"><FiShare2 /> 읽기 전용</span></div>
...
function SharedVersionComparisonCard({ snapshot, onCopy, onDownload }: { snapshot: SavedBuildVersionComparisonShareSnapshot; onCopy: () => void; onDownload: () => void }) {
  ...
  return <section className={`shared-version-comparison-card ${directionTone(payload.summary.direction)}`} aria-label="공유된 저장 견적 버전 비교" data-testid="shared-version-comparison-card">
    ...
    <section className="shared-version-comparison-summary" aria-label="버전 비교 요약"><div><span>결과 방향</span><strong>{directionText(payload.summary.direction)}</strong></div><div><span>구성</span><strong>{payload.summary.selectionChangedCategoryCount}개 범주 변경</strong></div><div><span>위험 변화</span><strong>{transition ? `차단 ${signed(transition.blockerDelta)} · 주의 ${signed(transition.warningDelta)} · 확인 ${signed(transition.unknownDelta)}` : "비교 기준 없음"}</strong></div><div><span>총액</span><strong>{transition?.priceDeltaWon !== undefined ? `${transition.priceDeltaWon > 0 ? "+" : ""}${transition.priceDeltaWon.toLocaleString("ko-KR")}원` : beforeCheck && afterCheck ? `${formatPrice(beforeCheck.totalPriceWon, beforeCheck.priceComplete)} → ${formatPrice(afterCheck.totalPriceWon, afterCheck.priceComplete)}` : "가격 정보 없음"}</strong></div><div><span>결과 항목</span><strong>{findingCount > 0 ? `${findingCount}개 변경` : "변화 없음"}</strong></div></section>
...
function SharedVersionCurrentCheck({ payload, onToast }: { payload: SavedBuildVersionSharePayload; onToast: (message: string) => void }) {
  ...
  function downloadCurrentRecheck() {
    ...
    anchor.remove();
    window.setTimeout(() => window.URL.revokeObjectURL(url), 0);
    onToast("현재 견적 비교 정보를 JSON으로 저장했어요.");
  }
  ...
  async function copyCurrentRecheck() {
    ...
    try {
      ...
      if (mountedRef.current) onToast("현재 견적 비교 정보를 복사했어요.");
    } catch {
      if (mountedRef.current) onToast("현재 견적 비교 정보를 복사하지 못했어요. 브라우저 클립보드 설정을 확인해 주세요.");
    }
  }
