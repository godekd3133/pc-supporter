...
import { FiAlertTriangle, FiArrowLeft, FiCheckCircle, FiCopy, FiDownload, FiExternalLink, FiGitBranch, FiInfo, FiLoader, FiRefreshCw, FiShare2, FiXCircle } from "react-icons/fi";
...
function statusText(status: SavedBuildVersionShareCheck["status"] | undefined) {
  return status === "compatible" ? "호환 가능" : status === "needs_review" ? "확인 필요" : status === "incompatible" ? "호환 불가" : "검사 snapshot 없음";
}
...
function savedToCurrentDirection(saved: SavedBuildVersionShareCheck | undefined, current: CurrentVersionCheck["current"]) {
  ...
  return currentRank > savedRank ? "현재 기준 개선" : currentRank < savedRank ? "현재 기준 위험 증가" : saved.blockerCount !== current.blockerCount || saved.warningCount !== current.warningCount || saved.unknownCount !== current.unknownCount ? "현재 위험 수치 변경" : "판정 변화 없음";
}
...
export function SharedSavedBuildVersionView({ onBack, onToast }: Props) {
  ...
  return <div className="shared-version-comparison-page">
    <div className="workspace-heading"><div><button className="back-link" type="button" onClick={onBack}><FiArrowLeft /> 홈으로</button><p className="eyebrow">VERSION PAIR SHARE</p><h1>공유된 견적 버전 비교</h1><p>저장 당시 두 버전의 구성·검사 결과·선택 이유를 읽기 전용으로 확인합니다.</p></div><span className="admin-badge"><FiShare2 /> 읽기 전용</span></div>
...
function SharedVersionComparisonCard({ snapshot, onCopy, onDownload }: { snapshot: SavedBuildVersionComparisonShareSnapshot; onCopy: () => void; onDownload: () => void }) {
  ...
  return <section className={`shared-version-comparison-card ${directionTone(payload.summary.direction)}`} aria-label="공유된 저장 견적 버전 비교" data-testid="shared-version-comparison-card">
    ...
    <section className="shared-version-comparison-summary" aria-label="버전 비교 요약"><div><span>판정 방향</span><strong>{directionText(payload.summary.direction)}</strong></div><div><span>구성</span><strong>{payload.summary.selectionChangedCategoryCount}개 범주 변경</strong></div><div><span>위험 변화</span><strong>{transition ? `차단 ${signed(transition.blockerDelta)} · 주의 ${signed(transition.warningDelta)} · 확인 ${signed(transition.unknownDelta)}` : "비교 기준 없음"}</strong></div><div><span>총액</span><strong>{transition?.priceDeltaWon !== undefined ? `${transition.priceDeltaWon > 0 ? "+" : ""}${transition.priceDeltaWon.toLocaleString("ko-KR")}원` : beforeCheck && afterCheck ? `${formatPrice(beforeCheck.totalPriceWon, beforeCheck.priceComplete)} → ${formatPrice(afterCheck.totalPriceWon, afterCheck.priceComplete)}` : "가격 확인 필요"}</strong></div><div><span>분석 점수</span><strong>{transition?.analysisScoreDelta !== undefined ? `${transition.analysisScoreDelta > 0 ? "+" : ""}${transition.analysisScoreDelta}점` : afterCheck?.analysisScore !== undefined ? `${afterCheck.analysisScore}점` : "확인 필요"}</strong></div><div><span>finding</span><strong>{findingCount > 0 ? `${findingCount}개 변경` : "변화 없음"}</strong></div></section>
    <div className="shared-version-comparison-columns"><section aria-label="변경된 부품"><div className="shared-version-comparison-subheading"><strong>변경된 구성</strong><span>{payload.changes.length}개</span></div>{payload.changes.length > 0 ? <div className="shared-version-comparison-change-list">{payload.changes.map((change) => <div key={change.id}><span>{change.label}</span><small><em>{change.before}</em><b>→</b><em>{change.after}</em></small></div>)}</div> : <p className="shared-version-comparison-empty"><FiCheckCircle /> 선택 구성 변화 없음</p>}</section><section aria-label="변경된 호환성 판정"><div className="shared-version-comparison-subheading"><strong>변경된 호환성 판정</strong><span>{findingCount}개</span></div>{findingCount > 0 ? <ul className="shared-version-comparison-findings">{payload.findingChanges.map((finding) => <li className={finding.change}><span>{finding.change === "resolved" ? "해결됨" : finding.change === "new" ? "신규" : finding.change === "severity_changed" ? "심각도 변경" : "내용 변경"}</span><strong>{finding.title}</strong></li>)}</ul> : <p className="shared-version-comparison-empty"><FiCheckCircle /> finding 변화 없음</p>}</section></div>
    <p className="shared-version-comparison-boundary"><FiInfo /> {payload.dataBoundary} 저장된 snapshot은 공유 당시 기준이며, 현재 판매가·재고·FPS·제조사 QVL·실제 장착·케이블 배선의 최신 상태를 보장하지 않습니다.</p>
    <p className="shared-version-comparison-source"><FiGitBranch /> 이 링크는 원본 저장 견적을 변경하지 않으며, 공유받은 사용자는 버전을 자동 적용할 수 없습니다.</p>
  </section>;
}
...
function SharedVersionCurrentCheck({ payload, onToast }: { payload: SavedBuildVersionSharePayload; onToast: (message: string) => void }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (!cancelled) setState({ status: "error", message: error instanceof Error ? error.message : "현재 카탈로그 기준 재검사에 실패했습니다." });
      }
    };
    void run();
    return () => { cancelled = true; controller.abort(); };
  ...
  return <section className="shared-version-current-check" aria-label="현재 카탈로그 기준 재검사" data-testid="shared-version-comparison-current-check"><div className="shared-version-current-check-heading"><div><p className="eyebrow">CURRENT CATALOG RECHECK</p><h3>현재 카탈로그 기준 재검사</h3><small>공유 당시 snapshot과 현재 공개된 저장 견적을 다시 검사해 시점 차이를 확인합니다. 결과는 저장 견적이나 공유 snapshot을 변경하지 않습니다.</small></div><div className="shared-version-current-check-actions"><button className="button button-light" type="button" data-testid="shared-version-comparison-current-retry" onClick={() => setRetryNonce((current) => current + 1)} disabled={state.status === "loading"}><FiRefreshCw className={state.status === "loading" ? "spin" : undefined} /> {state.status === "loading" ? "재검사 중..." : "현재 기준 다시 확인"}</button>{state.status === "ready" && <><button className="button button-light" type="button" data-testid="shared-version-comparison-current-copy" onClick={() => void copyCurrentRecheck()}><FiCopy /> 현재 재검사 복사</button><button className="button button-light" type="button" data-testid="shared-version-comparison-current-download" onClick={downloadCurrentRecheck}><FiDownload /> 현재 재검사 JSON</button></>}</div></div>{state.status === "loading" && <div className="shared-version-current-check-state" role="status"><FiLoader className="spin" /> 두 버전을 현재 catalog 기준으로 검사하는 중...</div>}{state.status === "error" && <div className="shared-version-current-check-state error" role="alert"><FiAlertTriangle /><span>{state.message}</span><button className="text-button" type="button" onClick={() => setRetryNonce((current) => current + 1)}>다시 시도</button></div>}{state.status === "ready" && <div className="shared-version-current-check-grid" data-testid="shared-version-current-check-grid">{state.entries.map((entry) => { const saved = entry.savedCheck; const priceDelta = saved?.priceComplete && entry.current.priceComplete ? entry.current.totalPriceWon - saved.totalPriceWon : undefined; const analysisDelta = saved?.analysisScore !== undefined && entry.current.analysisScore !== undefined ? entry.current.analysisScore - saved.analysisScore : undefined; const findingDelta = findingDeltaFor(saved, entry.current); const catalogChanged = Boolean(saved?.catalogSnapshotAt && saved.catalogSnapshotAt !== entry.current.catalogSnapshotAt); return <article key={entry.label} className={entry.current.status}><div className="shared-version-current-check-item-heading"><div><span>{entry.label}</span><strong>{entry.name}</strong></div><em>{currentStatusText(entry.current.status)}</em></div><small>현재 위험 · 차단 {entry.current.blockerCount} · 주의 {entry.current.warningCount} · 확인 {entry.current.unknownCount}</small><div className="shared-version-current-check-evidence"><span>전력 {entry.current.resources.power}</span><span>냉각 {entry.current.resources.cooling}</span>{entry.current.benchmark && <><span>benchmark {entry.current.benchmark.status} · {entry.current.benchmark.presentScoreCount}/{entry.current.benchmark.expectedScoreCount}</span>{entry.current.benchmark.rows.length > 0 && <span>점수 {entry.current.benchmark.rows.map((row) => `${row.label} ${row.value ?? "-"}`).join(" · ")}</span>}</>}</div><div className="shared-version-current-check-deltas"><span>snapshot 대비 <b>{savedToCurrentDirection(saved, entry.current)}</b></span><span>가격 <b>{signedDelta(priceDelta)}{priceDelta !== undefined ? "원" : ""}</b></span><span>분석 <b>{signedDelta(analysisDelta)}{analysisDelta !== undefined ? "점" : ""}</b></span><span>finding <b>{findingDelta ? `해결 ${findingDelta.resolved} · 신규 ${findingDelta.added} · 변경 ${findingDelta.changed}` : "snapshot 없음"}</b></span></div>{findingDelta && (findingDelta.resolvedTitles.length + findingDelta.addedTitles.length + findingDelta.changedTitles.length > 0) && <div className="shared-version-current-check-finding-preview" data-testid={`shared-version-current-finding-preview-${entry.label}`}><strong>변경 finding</strong>{findingDelta.resolvedTitles.length > 0 && <span><em>해결</em> {findingDelta.resolvedTitles.join(" · ")}</span>}{findingDelta.addedTitles.length > 0 && <span><em>신규</em> {findingDelta.addedTitles.join(" · ")}</span>}{findingDelta.changedTitles.length > 0 && <span><em>변경</em> {findingDelta.changedTitles.join(" · ")}</span>}</div>}<small>현재 catalog {new Date(entry.current.catalogSnapshotAt).toLocaleString("ko-KR")} · engine {entry.current.engineVersion} · 검사 {new Date(entry.current.checkedAt).toLocaleString("ko-KR")}</small>{catalogChanged && <p className="shared-version-current-check-warning"><FiAlertTriangle /> 공유 당시 catalog 기준과 현재 기준이 다릅니다.</p>}</article>; })}</div>}<p className="shared-version-current-check-note"><FiInfo /> 현재 기준 재검사는 공개 snapshot의 판단을 갱신하지 않습니다. 실제 구매·장착 전에는 가격·재고·제조사 원문·물리 조건을 별도로 확인하세요.</p></section>;
}
