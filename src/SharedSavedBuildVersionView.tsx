...
function SharedVersionComparisonCard({ snapshot, onCopy, onDownload }: { snapshot: SavedBuildVersionComparisonShareSnapshot; onCopy: () => void; onDownload: () => void }) {
  ...
  return <section className={`shared-version-comparison-card ${directionTone(payload.summary.direction)}`} aria-label="공유된 저장 견적 버전 비교" data-testid="shared-version-comparison-card">
    ...
    <section className="shared-version-comparison-summary" aria-label="버전 비교 요약"><div><span>결과 방향</span><strong>{directionText(payload.summary.direction)}</strong></div><div><span>구성</span><strong>{payload.summary.selectionChangedCategoryCount}개 범주 변경</strong></div><div><span>위험 변화</span><strong>{transition ? `차단 ${signed(transition.blockerDelta)} · 주의 ${signed(transition.warningDelta)} · 확인 ${signed(transition.unknownDelta)}` : "비교 기준 없음"}</strong></div><div><span>총액</span><strong>{transition?.priceDeltaWon !== undefined ? `${transition.priceDeltaWon > 0 ? "+" : ""}${transition.priceDeltaWon.toLocaleString("ko-KR")}원` : beforeCheck && afterCheck ? `${formatPrice(beforeCheck.totalPriceWon, beforeCheck.priceComplete)} → ${formatPrice(afterCheck.totalPriceWon, afterCheck.priceComplete)}` : "가격 확인 필요"}</strong></div><div><span>분석 점수</span><strong>{transition?.analysisScoreDelta !== undefined ? `${transition.analysisScoreDelta > 0 ? "+" : ""}${transition.analysisScoreDelta}점` : afterCheck?.analysisScore !== undefined ? `${afterCheck.analysisScore}점` : "확인 필요"}</strong></div><div><span>결과 항목</span><strong>{findingCount > 0 ? `${findingCount}개 변경` : "변화 없음"}</strong></div></section>
    ...
    <p className="shared-version-comparison-boundary"><FiInfo /> {payload.dataBoundary} 저장된 저장본은 공유 당시 기준이며, 현재 판매가·재고·FPS·제조사 QVL·실제 장착·케이블 배선의 최신 상태를 보장하지 않습니다.</p>
