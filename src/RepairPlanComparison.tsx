...
function RepairPlanComparisonPanel({ plan, state, currentBuild, currentResult, partMap, onApply, onSavePlan, onRetry, onClose }: { plan: RecommendationPlan; state: RepairPlanComparisonViewState; currentBuild: BuildSelection; currentResult: CompatibilityResult; partMap: ReadonlyMap<string, Part>; onApply: () => void; onSavePlan: (build: BuildSelection, preferences: RecommendationPreferences, label: string) => void; onRetry: () => void; onClose: () => void }) {
  if (state.status === "loading") return <section className="repair-plan-comparison loading" aria-label="수리 플랜 전체 비교" data-testid="repair-plan-comparison" role="status"><div className="repair-plan-comparison-heading"><div><h2>플랜 적용 후 전체 견적을 계산 중...</h2><p>{plan.title}</p></div><FiLoader className="spin" /></div><p className="repair-plan-comparison-loading"><FiActivity /> 현재 부품·수량을 바꾸지 않고 플랜 적용 후 전체 규칙을 다시 검사합니다.</p></section>;
  ...
  return <section className={`repair-plan-comparison ${comparison.direction}`} aria-label="수리 플랜 전체 비교" data-testid="repair-plan-comparison">
    <div className="repair-plan-comparison-heading"><div><h2>현재 견적과 플랜 적용 후 비교</h2><p>{plan.title}을 전체 부품 조합에 적용한 결과입니다. 실제 편집기 값은 아직 바뀌지 않았습니다.</p></div><div className="repair-plan-comparison-heading-actions"><span className={`repair-plan-comparison-direction ${comparison.direction}`}>{directionLabel}</span><button className="icon-button" type="button" onClick={onClose} aria-label="수리 플랜 비교 닫기"><FiXCircle /></button></div></div>
