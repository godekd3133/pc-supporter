...
export function AdminGpuPhysicalPanel({ onToast, onMetaRefresh }: { onToast: (message: string) => void; onMetaRefresh: () => void }) {
  ...
  async function remove(partId: string) {
    ...
    try {
      ...
      onMetaRefresh();
      onToast("물리 호환 확인값을 삭제했습니다. 원문 파싱값만 다시 사용합니다.");
    } catch (reason: unknown) {
  ...
  return <section className="admin-card gpu-physical-admin-card" data-testid="admin-gpu-physical-panel">
    <div className="admin-card-heading"><div><p className="eyebrow">PHYSICAL FIT REVIEW</p><h3>GPU·케이스·PSU 물리 호환 확인</h3><p className="admin-card-description">판매 목록만으로 확정하기 어려운 GPU 물리 슬롯·케이블 굽힘·케이스 측면 여유·PSU PCIe 케이블 분배 구조를 제조사 매뉴얼 정보로 별도 저장합니다.</p></div><FiShield /></div>
    ...
    {categoryItems.length === 0 ? <p className="gpu-physical-empty"><FiInfo /> 아직 {category === "gpu" ? "GPU" : category === "case" ? "케이스" : "PSU"} 물리 확인값이 없습니다.</p> : <div className="gpu-physical-list">{categoryItems.map((item) => <article key={item.partId}><div><strong>{item.partName}</strong><small>{overrideValueText(item)} · {new Date(item.updatedAt).toLocaleDateString("ko-KR")}</small><small>{item.sourceNote}</small><small className={`gpu-physical-source-check ${item.sourceCheck?.status ?? "not_checked"}`}>{physicalSourceCheckText(item.sourceCheck)}</small></div><div>{sourceUrlFor(item.sourceUrl ?? "") && <a href={sourceUrlFor(item.sourceUrl ?? "")} target="_blank" rel="noreferrer" aria-label={`${item.partName} 물리 확인 정보 원문`}><FiExternalLink /></a>}{item.sourceUrl && <button className="text-button" type="button" onClick={() => void checkSource(item.partId)} disabled={busy}>{sourceCheckingPartId === item.partId ? <><FiLoader className="spin" /> 점검 중...</> : <><FiSearch /> 정보 점검</>}</button>}{item.sourceUrl && <button className="text-button" type="button" onClick={() => void toggleSourceHistory(item.partId)} disabled={busy}>{sourceHistoryPartId === item.partId ? "이력 닫기" : "이력 보기"}</button>}<button className="text-button danger-text-button" type="button" onClick={() => void remove(item.partId)} disabled={busy}><FiTrash2 /> 삭제</button></div>{sourceHistoryPartId === item.partId && <PhysicalSourceHistoryPanel entries={sourceHistory} loading={sourceHistoryLoading} error={sourceHistoryError} />}</article>)}</div>}
