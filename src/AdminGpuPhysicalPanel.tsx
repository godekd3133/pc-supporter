...
const REVIEW_PRIORITY_LABELS = { high: "우선 확인", medium: "확인 권장", low: "일반 확인" } as const;
const REVIEW_STATUS_LABELS = { pending: "미확인", partial: "부분 확인", stale: "정보 재확인", reviewed: "확인 완료" } as const;
...
export function AdminGpuPhysicalPanel({ onToast, onMetaRefresh }: { onToast: (message: string) => void; onMetaRefresh: () => void }) {
  ...
  async function loadOverrides() {
    ...
    try {
      ...
      if (!mountedRef.current || overrideListRequestVersionRef.current !== requestVersion) return;
      setError(reason instanceof Error ? reason.message : "GPU 물리 호환 확인 데이터를 불러오지 못했습니다.");
    } finally {
  ...
  useEffect(() => {
    ...
    void api<PhysicalReviewQueue>(`/api/admin/gpu-physical-overrides/review-queue?${params.toString()}`)
      ...
      .catch((reason: unknown) => { if (!cancelled) setReviewQueueError(reason instanceof Error ? reason.message : "물리 확인 우선순위를 불러오지 못했습니다."); })
  ...
  useEffect(() => {
    ...
    void api<PhysicalReviewCoverage>("/api/admin/gpu-physical-overrides/coverage")
      ...
      .catch((reason: unknown) => { if (!cancelled) setPhysicalCoverageError(reason instanceof Error ? reason.message : "장착 정보 coverage를 불러오지 못했습니다."); })
  ...
  async function searchParts(event: FormEvent) {
    ...
    try {
      ...
      if (partSearchRequestVersionRef.current === requestVersion) onToast(reason instanceof Error ? reason.message : "확인 대상 부품을 찾지 못했습니다.");
    } finally {
  ...
  async function save() {
    ...
    try {
      ...
      onMetaRefresh();
      onToast(`${part.name}의 물리 호환 확인값을 저장했습니다.`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 호환 확인값을 저장하지 못했습니다.");
    } finally {
  ...
  async function remove(partId: string) {
    ...
    try {
      ...
      onMetaRefresh();
      onToast("물리 호환 확인값을 삭제했습니다. 수집된 파싱값만 다시 사용합니다.");
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 호환 확인값을 삭제하지 못했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function checkSource(partId: string) {
    ...
    try {
      ...
      setReviewQueueRefreshNonce((current) => current + 1);
      onMetaRefresh();
      onToast(`정보 URL 점검 완료: ${physicalSourceCheckText(result.sourceCheck)}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "정보 URL을 점검하지 못했습니다.");
    } finally {
      if (isCurrent()) {
  ...
  async function checkSourcesBatch() {
    ...
    try {
      ...
      setReviewQueueRefreshNonce((current) => current + 1);
      onMetaRefresh();
      onToast(`${result.checkedCount}개 ${CATEGORY_LABELS[category]} 정보 URL을 점검했습니다. 통과 ${result.passedCount}개 · 재확인 ${result.reviewCount}개 · ${result.persisted ? `저장 ${result.persistedCount}개${result.persistFailureCount > 0 ? ` · 저장 실패 ${result.persistFailureCount}개` : ""}` : "미리보기(저장 안 함)"}${result.totalCandidates > result.checkedCount ? ` · 남은 URL ${result.totalCandidates - result.checkedCount}개` : ""}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "정보 URL 일괄 점검에 실패했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function toggleSourceHistory(partId: string) {
    ...
    try {
      ...
      if (sourceHistoryRequestVersionRef.current !== requestVersion) return;
      setSourceHistory([]);
      setSourceHistoryError(reason instanceof Error ? reason.message : "정보 URL 점검 이력을 불러오지 못했습니다.");
    } finally {
  ...
  function importBulkCsvText(value: string) {
    ...
    if (parsed.errors.length > 0) {
      onToast(`물리 확인 CSV를 반영하지 못했습니다: ${parsed.errors.slice(0, 3).join(" · ")}`);
      return;
    }
    updateBulkJson(JSON.stringify({ items: parsed.items }, null, 2));
    setBulkOpen(true);
    onToast(`${parsed.items.length}개 물리 확인 CSV 행을 JSON에 반영했습니다. 서버 확인 후 저장해 주세요.`);
  }

  function importBulkCsv() {
    if (!bulkCsvText.trim()) {
      onToast("반영할 물리 확인 CSV를 붙여 넣어 주세요.");
      return;
    }
  ...
  async function importBulkCsvFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      onToast(reason instanceof Error ? reason.message : "물리 확인 CSV 파일을 읽지 못했습니다.");
    }
  }
  ...
  async function importBulkJsonFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setBulkOpen(true);
      onToast(`${items.length}개 JSON 확인 행을 반영했습니다. 서버 확인 후 저장해 주세요.`);
    } catch (reason: unknown) {
      onToast(reason instanceof Error ? reason.message : "물리 확인 JSON 파일을 읽지 못했습니다.");
    }
  }

  async function validateBulk() {
    if (!bulkJson.trim()) {
      onToast("확인할 물리 확인 JSON을 입력해 주세요.");
      return;
    }
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    ...
    try {
      ...
      onToast(result.invalidCount === 0 ? `${result.validCount}개 물리 확인값을 저장할 수 있습니다.` : `${result.invalidCount}개 물리 확인 행을 수정해야 합니다.`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 확인 일괄 확인에 실패했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function saveBulk() {
    if (!bulkValidation || bulkValidation.invalidCount > 0 || validatedBulkInput !== bulkJson) {
      onToast("입력 내용을 바꿨다면 먼저 물리 확인 JSON 확인을 다시 실행해 주세요.");
      return;
    }
    const requestVersion = ++mutationRequestVersionRef.current;
    const isCurrent = () => mountedRef.current && mutationRequestVersionRef.current === requestVersion;
    ...
    try {
      ...
      setBulkValidation(null);
      setValidatedBulkInput("");
      onToast(`${result.count}개 GPU·케이스·PSU 물리 확인값을 원자적으로 저장했습니다.`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 확인 일괄 저장에 실패했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function exportBulkCsv() {
    ...
    try {
      ...
      onToast(`${result.items.length}개 저장된 물리 확인값을 CSV로 내보냈습니다.`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 확인값을 내보내지 못했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function exportReviewPackage() {
    ...
    try {
      ...
      onToast(`${result.items.length}개 ${CATEGORY_LABELS[category]} 우선 확인 작업 패키지를 저장했습니다. ${result.nextOffset !== undefined ? `다음 묶음은 ${result.nextOffset}번부터 재개합니다.` : "현재 목록을 모두 담았습니다."}`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 확인 작업 패키지를 내보내지 못했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function exportBulkTemplate() {
    ...
    try {
      ...
      onToast(`${result.items.length}개 ${CATEGORY_LABELS[category]} 확인 템플릿을 저장했습니다. 전체 ${result.total}개 중 첫 500개입니다.`);
    } catch (reason: unknown) {
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "물리 확인 템플릿을 내보내지 못했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  async function openReviewItem(item: PhysicalReviewQueueItem) {
    ...
    try {
      ...
      if (isCurrent()) onToast(reason instanceof Error ? reason.message : "확인 대상 부품을 불러오지 못했습니다.");
    } finally {
      if (isCurrent()) setBusy(false);
    }
  }
  ...
  return <section className="admin-card gpu-physical-admin-card" data-testid="admin-gpu-physical-panel">
    ...
    <div className="gpu-physical-list-heading"><strong>저장된 물리 확인 목록</strong><span>{categoryItems.length}개</span></div>
    {categoryItems.length === 0 ? <p className="gpu-physical-empty"><FiInfo /> 아직 {category === "gpu" ? "GPU" : category === "case" ? "케이스" : "PSU"} 물리 확인값이 없습니다.</p> : <div className="gpu-physical-list">{categoryItems.map((item) => <article key={item.partId}><div><strong>{item.partName}</strong><small>{overrideValueText(item)} · {new Date(item.updatedAt).toLocaleDateString("ko-KR")}</small><small>{item.sourceNote}</small><small className={`gpu-physical-source-check ${item.sourceCheck?.status ?? "not_checked"}`}>{physicalSourceCheckText(item.sourceCheck)}</small></div><div>{sourceUrlFor(item.sourceUrl ?? "") && <a href={sourceUrlFor(item.sourceUrl ?? "")} target="_blank" rel="noreferrer" aria-label={`${item.partName} 물리 확인 정보 페이지`}><FiExternalLink /></a>}{item.sourceUrl && <button className="text-button" type="button" onClick={() => void checkSource(item.partId)} disabled={busy}>{sourceCheckingPartId === item.partId ? <><FiLoader className="spin" /> 점검 중...</> : <><FiSearch /> 정보 점검</>}</button>}{item.sourceUrl && <button className="text-button" type="button" onClick={() => void toggleSourceHistory(item.partId)} disabled={busy}>{sourceHistoryPartId === item.partId ? "이력 닫기" : "이력 보기"}</button>}<button className="text-button danger-text-button" type="button" onClick={() => void remove(item.partId)} disabled={busy}><FiTrash2 /> 삭제</button></div>{sourceHistoryPartId === item.partId && <PhysicalSourceHistoryPanel entries={sourceHistory} loading={sourceHistoryLoading} error={sourceHistoryError} />}</article>)}</div>}
