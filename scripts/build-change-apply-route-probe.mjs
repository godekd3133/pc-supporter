...
const profileDir = await mkdtemp(join(tmpdir(), "pc-supporter-build-change-apply-route-probe-"));
...
try {
  ...
  const result = await client.evaluate(`(async () => {
    ...
    try {
      const currentBuildText = document.querySelector('.build-mini-list')?.textContent ?? "";
      const card = [...document.querySelectorAll('.suggestion-card')].find((candidate) => {
        const button = candidate.querySelector('.suggestion-apply:not([disabled])');
        const name = button?.getAttribute('aria-label')?.replace(/ 적용$/, "") ?? "";
        return !(candidate.textContent ?? "").includes("적용하지 않음") && name.length > 0 && !currentBuildText.includes(name) && button instanceof HTMLButtonElement;
      });
      const apply = card?.querySelector('.suggestion-apply:not([disabled])');
      if (!(apply instanceof HTMLButtonElement)) return { stage: "missing-apply", compatibilityCalls };
      apply.click();
      for (let index = 0; index < 160 && !document.querySelector('.build-change-dialog'); index += 1) await wait(25);
      const dialog = document.querySelector('.build-change-dialog');
      if (!dialog) return { stage: "missing-dialog", compatibilityCalls, body: (document.body?.innerText ?? "").slice(-1800) };
      const confirm = dialog.querySelector('.build-change-actions .button-primary');
      if (!(confirm instanceof HTMLButtonElement)) return { stage: "missing-confirm", compatibilityCalls };
      confirm.click();
      ...
      return { stage: "checked", compatibilityCalls, path: location.pathname, home: document.querySelector('.home-page') !== null, editor: document.querySelector('.workspace-page') !== null, result: document.querySelector('.result-page') !== null, staleToast: body.includes("적용 후 다시 검사") };
    } finally {
