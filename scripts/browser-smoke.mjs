...
export async function selectLabel(client, labelText, value) {
  return client.evaluate(`(() => { const label = [...document.querySelectorAll("label")].find((candidate) => (candidate.textContent ?? "").includes(${JSON.stringify(labelText)})); const select = label?.querySelector("select"); if (!select) return false; const setter = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, "value")?.set; setter?.call(select, ${JSON.stringify(value)}); select.dispatchEvent(new Event("change", { bubbles: true })); return select.value === ${JSON.stringify(value)}; })()`);
}
...
async function main() {
  ...
  try {
    ...
    await waitForValue(client, "(document.body?.innerText ?? '').includes('오류 시연 견적')", "홈 화면");
    ...
    await waitForValue(client, "(document.body?.innerText ?? '').includes('오류 시연 견적')", "홈 데이터 신뢰도 성능 근거 링크 준비");
    ...
    assert(await clickText(client, "요구사항 해석"), "한 줄 요구사항 해석 버튼을 찾지 못했습니다.");
    ...
    await waitForValue(client, "document.querySelector('[data-testid=\"benchmark-3dmark-package-progress\"]')?.textContent?.includes('1 / 24') === true", "3DMark GPU 검수 작업 진행률");
    assert((await clickSelector(client, '[data-testid="benchmark-3dmark-package-next"]', 1)) === 1, "3DMark GPU 검수 다음 작업 패키지 버튼을 클릭하지 못했습니다.");
    await waitForValue(client, "document.querySelector('[data-testid=\"benchmark-3dmark-package-range\"]')?.textContent?.startsWith('25–48 /') === true", "3DMark GPU 검수 다음 작업 패키지");
    ...
    try {
      await waitForValue(client, "document.querySelector('[data-testid=\"benchmark-3dmark-package-range\"]')?.textContent?.startsWith('25–48 /') === true && document.querySelector('[data-testid=\"benchmark-3dmark-package-previous\"]')?.disabled === false", "3DMark GPU 검수 작업 패키지 재개");
    } catch (error) {
    ...
    assert(await client.evaluate("(() => { try { const value = JSON.parse(localStorage.getItem('pc-supporter-3dmark-review-work-progress-v1') ?? 'null'); return value?.offset === 24 && Array.isArray(value?.completedIds) && value.completedIds.length === 1 && typeof value.queueFingerprint === 'string'; } catch { return false; } })()"), "3DMark GPU 검수 작업 위치·완료 체크가 새로고침 후 보존되지 않았습니다.");
    ...
    const unknownMotherboardPayload = await waitForJson(`${baseUrl}/api/parts?category=motherboard&priceStatus=unknown&limit=1`, (payload) => Array.isArray(payload?.items) && Boolean(payload.items[0]?.id), "가격 미확인 메인보드");
    ...
    assert(await client.evaluate("(() => { const node = document.querySelector('[data-testid=\"build-price-summary-unknown-items\"]'); const summary = document.querySelector('[data-testid=\"build-price-summary\"]'); const text = node?.textContent ?? ''; return text.includes('가격 확인이 필요한 항목') && text.includes('원문 재확인') && (summary?.textContent ?? '').includes('예산 판정 보류'); })()"), "가격 미확인 항목 목록과 원문 재확인·예산 보류 상태가 표시되지 않았습니다.");
    ...
    await waitForValue(client, "location.pathname === '/build' && document.querySelector('.build-preflight-refresh') !== null", "지연 refresh 경합 probe 편집기");
    await waitForValue(client, "document.querySelector('button.button-primary.full-width')?.disabled === false", "지연 refresh 경합 probe 검사 준비");
    ...
    await waitForValue(client, "document.querySelector('[data-testid=\"accessory-recommendation-details\"]')?.open === true && (document.body?.innerText ?? '').includes('저장된 원문 스펙')", "주변 부품 추천 스펙 근거 펼치기");
    ...
    await waitForValue(client, "document.querySelector('[data-testid=\"accessory-recommendation-comparison\"]') !== null && (document.body?.innerText ?? '').includes('후보 비교')", "주변 부품 추천 후보 비교표");
    ...
    await waitForValue(client, "(document.body?.innerText ?? '').includes('대체 부품') || (document.body?.innerText ?? '').includes('안전한 대체 후보')", "대체 후보 영역");
    ...
    await client.evaluate("(() => { history.pushState({}, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')); return true; })()");
    await waitForValue(client, "location.pathname === '/' && (document.body?.innerText ?? '').includes('오류 시연 견적')", "변경 미리보기 route transition 홈 복귀");
    ...
    assert(resultText.includes("대체 부품") || resultText.includes("안전한 대체 후보"), "검사 결과에 대체 후보 영역이 없습니다.");
    ...
    assert(pickerText.includes("호환") && pickerText.includes("후보"), "부품 후보 선택기에 호환 후보 설명이 없습니다.");
    ...
    assert(await selectLabel(client, "후보", "no_blocker"), "후보 모드를 차단 없음으로 전환하지 못했습니다.");
    await waitForValue(client, "(document.body?.innerText ?? '').includes('차단 오류 없는 후보')", "차단 없음 후보 모드");
    ...
    await waitForValue(client, "document.querySelector('[aria-label=\"후보 비교\"]') !== null", "후보 비교표");
   ...
   assert(pickerShareProbe.candidates.some((candidate) => candidate?.similarityEvidence?.comparedDimensions !== undefined), "부품 선택기 후보 공유 요청에 구조화된 성능 비교 근거가 전달되지 않았습니다.");
    ...
    await waitForValue(client, "document.querySelector('[role=dialog] [data-testid=\"picker-similarity-evidence\"]') !== null && (document.body?.innerText ?? '').includes('현재') && (document.body?.innerText ?? '').includes('후보')", "후보 지표별 성능 근거");
    ...
    await waitForValue(client, "(document.body?.innerText ?? '').includes('현재 견적 호환 후보') && document.querySelector('.catalog-part-list [data-testid^=\"catalog-part-\"]') !== null", "카탈로그 호환 후보 상세 화면");
    ...
    await waitForValue(client, "document.querySelector('[data-testid=\"catalog-spec-comparison\"]')?.textContent?.includes('성능 근거') === true && document.querySelector('[data-testid=\"catalog-spec-comparison\"]')?.textContent?.includes('Cinebench R23 싱글') === true && document.querySelector('[data-testid=\"catalog-spec-comparison\"]')?.textContent?.includes('성능 출처') === true", "카탈로그 비교 성능 점수·출처");
    ...
    await client.send("Page.navigate", { url: `${baseUrl}/` });
    await waitForValue(client, "(document.body?.innerText ?? '').includes('오류 시연 견적')", "결과 route 복귀 기준 demo 홈");
    assert(await clickText(client, "오류 시연 견적"), "결과 route 복귀 기준 오류 시연 견적을 불러오지 못했습니다.");
    ...
    if ((gpuBenchmarkCoverage?.total ?? 0) > 0) {
      ...
      await waitForValue(client, "(document.body?.innerText ?? '').includes('그래픽카드 목록') && document.querySelector('[data-testid=\"catalog-benchmark-evidence-summary\"]') !== null && (document.body?.innerText ?? '').includes('점수 없음') && (document.body?.innerText ?? '').includes('원문 확인 필요')", "카탈로그 GPU 성능 근거 없음 안내");
    }
    ...
    await client.send("Page.navigate", { url: `${baseUrl}/` });
    await waitForValue(client, "(document.body?.innerText ?? '').includes('오류 시연 견적')", "적용 후 검사 비교 기준 홈");
    assert(await clickText(client, "오류 시연 견적"), "적용 후 검사 비교용 오류 시연 견적을 불러오지 못했습니다.");
    ...
    await client.send("Page.navigate", { url: `${baseUrl}/` });
    await waitForValue(client, "(document.body?.innerText ?? '').includes('오류 시연 견적')", "모바일 홈 화면");
    ...
    assert(await clickText(client, "요구사항 해석"), "모바일 보완 안내 해석 버튼을 찾지 못했습니다.");
    ...
    await waitForValue(client, "document.querySelector('[data-testid^=\"accessory-price-evidence-\"]') !== null && document.querySelector('[data-testid=\"accessory-price-evidence-legend\"]') !== null", "주변 부품 목록 가격 근거");
    ...
    await client.send("Page.navigate", { url: `${baseUrl}/accessories` });
    await waitForValue(client, "(document.body?.innerText ?? '').includes('주변 부품 카탈로그') && document.querySelector('[data-testid^=\"accessory-price-evidence-\"]') !== null", "주변 부품 캐시 준비 목록 복귀");
    ...
    const historyMonitorAlertLatestRouteProbe = await client.evaluate(`(async () => {
      ...
      try {
        ...
        const readAll = [...document.querySelectorAll("button")].find((button) => (button.textContent ?? "").includes("모두 읽음") && !button.disabled);
