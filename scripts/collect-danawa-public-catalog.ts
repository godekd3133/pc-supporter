...
if ([...args].some((arg) => !["--dry-run", "--apply", "--pilot"].includes(arg))) throw new Error("Allowed flags: --dry-run (default), --apply, --pilot.");
...
async function main() {
  ...
  const targets = pilot ? TARGETS.slice(0, 3) : TARGETS.slice(0, MAX_LIST_PAGES);
  let detailBudget = pilot ? 15 : MAX_DETAILS;
  ...
  for (const target of targets) {
    ...
    const counts = categoryCounts.get(key) ?? {
      listedRows: 0, uniqueListedCodes: 0, detailPagesFetched: 0,
  ...
  console.log(JSON.stringify({
    ...
    pilot,
    requestsBound: { maxListPages: MAX_LIST_PAGES, maxUniqueDetailPages: MAX_DETAILS, delayMs: MIN_DELAY_MS },
    listPages: targets.length,
    detailPagesFetched: details,
    uniqueProductsParsed: collected.length,
    ...
    newEligibleProducts: newParts.length,
  ...
  if (apply && newParts.length > 0) {
    ...
    await writeCatalogRecords(mergeCatalog(existing, newParts));
    ...
    console.log(`Applied ${newParts.length} new eligible products; benchmark override file unchanged.`);
  }
}
