...
const revokeShareId = "shared-budget-ladder-revoke-abort";
const nextShareId = `${revokeShareId}-next`;
const savedId = "shared-budget-ladder-revoke-saved";
...
const profileDir = await mkdtemp(join(tmpdir(), "pc-supporter-shared-budget-ladder-revoke-abort-probe-"));
...
try {
  ...
  const result = await client.evaluate(`(async () => {
    ...
    const revokeShareId = ${JSON.stringify(revokeShareId)};
    const nextShareId = ${JSON.stringify(nextShareId)};
    const savedId = ${JSON.stringify(savedId)};
    ...
    const selection = { cpu: { partId: "cpu-revoke", quantity: 1 }, motherboard: { partId: "board-revoke", quantity: 1 }, gpu: { partId: "gpu-revoke", quantity: 1 }, memory: [], ssd: [], hdd: [], case: { partId: "case-revoke", quantity: 1 }, psu: { partId: "psu-revoke", quantity: 1 }, accessories: [], useIntegratedGraphics: false };
    const payload = { type: "pc-supporter-budget-ladder", version: 1, exportedAt: timestamp, items: [
      ...
      { id: "headroom", label: "여유형", description: "probe", budgetWon: 1800000, status: "호환 가능", totalPriceWon: 1700000, budgetDeltaWon: 100000, withinBudget: true, priceComplete: true, blockerCount: 0, unknownCount: 0, analysisScore: 84, lines: [] }
    ], changes: [] };
    ...
    const state = { deleteCalls: 0, deleteSignalCalls: 0, deleteAbortedCalls: 0 };
    window.fetch = async (input, init) => {
      const requestUrl = new URL(typeof input === "string" ? input : input.url, location.href);
      if (requestUrl.pathname === "/api/budget-ladders/" + revokeShareId || requestUrl.pathname === "/api/budget-ladders/" + nextShareId) {
        const id = requestUrl.pathname.split("/").at(-1);
        return response(snapshotFor(id, id === revokeShareId ? "취소 대상 A" : "취소 후 B"));
      }
      if (requestUrl.pathname.endsWith("/lineage")) return response({ lineageId: "shared-budget-ladder-revoke-abort-lineage", currentId: requestUrl.pathname.split("/").at(-2), entries: [] });
      ...
      if (requestUrl.pathname === "/api/budget-ladders" && init?.method === "POST") return response({ ...snapshotFor(savedId, "저장 결과"), ownerToken: "probe-owner", parentId: revokeShareId, versionNumber: 2 });
      if (requestUrl.pathname === "/api/budget-ladders/" + savedId && init?.method === "DELETE") {
        state.deleteCalls += 1;
        if (init?.signal) state.deleteSignalCalls += 1;
        return new Promise((resolve, reject) => {
          const signal = init?.signal;
          const abort = () => { state.deleteAbortedCalls += 1; const error = new Error("요청이 취소되었습니다."); error.name = "AbortError"; reject(error); };
          if (signal?.aborted) return abort();
          signal?.addEventListener("abort", abort, { once: true });
          setTimeout(() => { signal?.removeEventListener("abort", abort); resolve(response({ ok: true })); }, 2000);
        });
      }
    ...
    try {
      push(revokeShareId);
      const mounted = await waitForHeading("취소 대상 A");
      ...
      if (!(save instanceof HTMLButtonElement)) return { stage: "missing-save", mounted, ...state };
      save.click();
      for (let index = 0; index < 160 && !document.querySelector(".shared-budget-ladder-refresh-share-preview"); index += 1) await wait(25);
      window.confirm = () => true;
      const revoke = [...document.querySelectorAll("button")].find((candidate) => (candidate.textContent ?? "").includes("공유 취소"));
      if (!(revoke instanceof HTMLButtonElement)) return { stage: "missing-revoke", mounted, ...state };
      revoke.click();
      for (let index = 0; index < 160 && state.deleteCalls < 1; index += 1) await wait(25);
      const requestStarted = state.deleteCalls >= 1;
      push(nextShareId);
      for (let index = 0; index < 240 && (!(document.querySelector("h1")?.textContent ?? "").includes("취소 후 B") || document.querySelector(".shared-budget-ladder-page") === null); index += 1) await wait(25);
      return { stage: "checked", requestStarted, deleteSignalCalls: state.deleteSignalCalls, abortedCalls: state.deleteAbortedCalls, calls: state.deleteCalls, path: location.pathname, nextMounted: (document.querySelector("h1")?.textContent ?? "").includes("취소 후 B"), stalePreview: document.querySelector(".shared-budget-ladder-refresh-share-preview") !== null };
    } finally {
  ...
  if (result?.stage !== "checked" || result.requestStarted !== true || result.deleteSignalCalls !== 1 || result.abortedCalls < 1 || result.calls !== 1 || result.path !== "/budget-ladder/${nextShareId}" || result.nextMounted !== true || result.stalePreview !== false) process.exitCode = 1;
} finally {
  client?.close();
  if (!chromeExited) {
