...
export function recommendAccessories(build: BuildSelection, catalog: Part[], accessories: AccessoryItem[]): AccessoryRecommendation[] {
  ...
  if (m2Count > 0) {
    addRecommendations(
      ...
      `선택한 SSD 규격 ${m2FitLabel}과 일치하거나 규격이 일반 M.2로 표시된 부품을 우선합니다.`,
      "optional",
      "high",
      (item) => {
  ...
  if (m2OverflowCount > 0) {
    ...
    addRecommendations(
      ...
      `선택한 SSD의 폼팩터·NVMe/SATA 신호를 직접 지원하고, 한 어댑터에 최소 ${m2OverflowCount}개를 장착할 수 있다는 정보가 있는 부품만 보여줘요. PCIe 슬롯·레인 공유·부팅 지원은 별도 확인이 필요합니다.`,
      "recommended",
      "medium",
      (item) => {
  ...
  if (memoryCoolingTriggered) {
    ...
    addRecommendations(
      ...
      `메모리·DDR·DIMM 표기와 팬·쿨링 정보가 함께 있는 부품을 우선합니다. DIMM 높이, CPU 쿨러 간섭, 메모리 세대별 장착 가능 여부는 구매 전에 확인해야 합니다.`,
      "optional",
      "medium",
      (item) => {
  ...
  if (fanSizes.length > 0) {
    ...
    addRecommendations(
      ...
      accessories,
      "cooling_fan",
      `케이스 페이지에서 ${sizeLabel} 팬 장착 규격이 확인되어 추가·교체 흡·배기 팬 선택지를 제안합니다.`,
      `케이스 페이지 팬 규격 ${sizeLabel}과 일치하는 팬 크기를 우선합니다.`,
      fanPriority,
      "high",
  ...
  if (expectedLoadW > 0) {
    ...
    addRecommendations(
      ...
      `UPS 출력 ${targetOutputW}W 이상 부품을 우선하며, 실제 모니터·주변기기 소비전력은 별도로 더해야 합니다.`,
      "optional",
      "medium",
  ...
  if (fanHeaderDeficit > 0 && caseFanCount !== undefined && fanPortCount !== undefined) {
    ...
    addRecommendations(
      ...
      `팬 허브 분배 포트 ${requiredFanCount}개 이상과 외부 전원 입력(SATA/IDE/Molex)을 제품 페이지에서 확인한 부품만 보여줘요.`,
      "recommended",
      "high",
  ...
  if (rgbNeedsController && caseRgbDeviceCount !== undefined && caseRgbVoltage !== undefined) {
    ...
    addRecommendations(
      ...
      `필요 전압 ${requiredVoltages.join(" + ")}을 지원하고 RGB 분배 포트 ${requiredRgbDeviceCount}개 이상, 외부 전원 입력까지 확인된 부품만 표시합니다.`,
      "recommended",
      "high",
      (item) => {
