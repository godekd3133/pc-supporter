import type { BenchmarkSourceCheckBatchItem, BenchmarkSourceCheckBatchResponse, PhysicalSourceCheck } from "../shared/types";
...
export type BenchmarkSourceCheckBatchTarget = {
  partId: string;
  partName: string;
  category: "cpu" | "gpu";
  sourceUrl: string;
...
export type BenchmarkSourceCheckBatchOptions = {
  limit?: number;
  concurrency?: number;
  persist?: boolean;
  now?: () => string;
...
function fallbackSourceCheck(sourceUrl: string, checkedAt: string): PhysicalSourceCheck {
  return {
    ...
    redirectCount: 0,
    detail: "벤치마크 출처 확인 중 예기치 않은 오류가 발생했습니다."
  };
}

export async function benchmarkSourceCheckBatchFor(targets: BenchmarkSourceCheckBatchTarget[], options: BenchmarkSourceCheckBatchOptions = {}): Promise<BenchmarkSourceCheckBatchResponse> {
  ...
  const items: BenchmarkSourceCheckBatchItem[] = [];
