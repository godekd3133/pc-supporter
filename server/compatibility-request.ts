...
export async function prepareCompatibilityRequest(
  ...
  if (validation.invalidSelections.length > 0) {
    return { snapshot, requestKey, error: { statusCode: 400, body: { error: "카탈로그에 존재하지 않는 부품이 포함되어 있습니다.", partIds: validation.invalidSelections.map((selection) => selection.partId) } } };
  }
  if (validation.invalidAccessories.length > 0) {
    return { snapshot, requestKey, error: { statusCode: 400, body: { error: "카탈로그에 존재하지 않는 주변 부품이 포함되어 있습니다.", accessoryIds: validation.invalidAccessories.map((selection) => selection.accessoryId) } } };
  }
  if (validation.invalidAccessoryTargets.length > 0) {
    return { snapshot, requestKey, error: { statusCode: 400, body: { error: "주변 부품 연결 대상 SSD가 현재 견적에 없습니다.", targetPartIds: validation.invalidAccessoryTargets } } };
  }
  if (validation.invalidAccessoryHubTargets.length > 0) {
    return { snapshot, requestKey, error: { statusCode: 400, body: { error: "주변 부품 연결 대상 팬 허브가 현재 견적에 없습니다.", targetAccessoryIds: validation.invalidAccessoryHubTargets } } };
  }
  if (validation.invalidRgbControllerAccessoryIds.length > 0) {
    return { snapshot, requestKey, error: { statusCode: 400, body: { error: "RGB 연결 컨트롤러가 현재 견적에 선택되어 있지 않습니다.", rgbControllerAccessoryId: validation.invalidRgbControllerAccessoryIds[0] } } };
  }
