...
import { BENCHMARK_SOURCE_CHECK_HISTORY_PATH, readJson, withSerializedFileMutation, writeJson } from "./storage";
...
export function benchmarkSourceCheckHistoryEntriesFromUnknown(value: unknown): PhysicalSourceCheckHistoryEntry[] {
...
export async function readBenchmarkSourceCheckHistory(partId?: string, limit = MAX_PART_HISTORY_ENTRIES) {
  const entries = benchmarkSourceCheckHistoryEntriesFromUnknown(await readJson<unknown>(BENCHMARK_SOURCE_CHECK_HISTORY_PATH, []));
...
export async function appendBenchmarkSourceCheckHistory(partId: string, sourceCheck: PhysicalSourceCheck) {
  return withSerializedFileMutation(BENCHMARK_SOURCE_CHECK_HISTORY_PATH, async () => {
    const entries = benchmarkSourceCheckHistoryEntriesFromUnknown(await readJson<unknown>(BENCHMARK_SOURCE_CHECK_HISTORY_PATH, []));
    ...
    await writeJson(BENCHMARK_SOURCE_CHECK_HISTORY_PATH, [...entries, entry].slice(-MAX_HISTORY_ENTRIES));
    return entry;
  });
}
