import type { CatalogSpecSourceCheckBatchItem, CatalogSpecSourceCheckBatchResponse, PartCategory, PhysicalSourceCheck } from "../shared/types";
...
export type CatalogSpecSourceCheckBatchTarget = {
  partId: string;
  partName: string;
...
export type CatalogSpecSourceCheckBatchOptions = {
  limit?: number;
  offset?: number;
  concurrency?: number;
  persist?: boolean;
...
export async function catalogSpecSourceCheckBatchFor(targets: CatalogSpecSourceCheckBatchTarget[], options: CatalogSpecSourceCheckBatchOptions = {}): Promise<CatalogSpecSourceCheckBatchResponse> {
  ...
  const selectedTargets = targets.slice(offset, offset + limit);
  const items: CatalogSpecSourceCheckBatchItem[] = [];
  ...
  return {
    ...
    offset,
    ...(offset + selectedTargets.length < targets.length ? { nextOffset: offset + selectedTargets.length } : {}),
