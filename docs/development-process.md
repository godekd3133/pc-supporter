...
- 자동 구성·분석의 부품 성능 점수는 `shared/objective-score.ts`의 고정 기준 지수(`objective-index-v1`)를 사용함. 범주별 측정 벤치마크·확인 스펙을 모델 버전이 고정한 기준값과 비교해 0~100 지수를 만들며, 카탈로그에 다른 부품이 추가·제거되어도 같은 부품의 점수는 바뀌지 않음. 순차 처리량·IOPS처럼 체감이 포화되는 차원은 `scale: "log"`로 눌러 프리미엄 부품이 실익 없이 예산을 흡수하지 않게 함. 용도별 가중치는 이 지수 위에 얹는 작업 부하 정책으로 분리하고, 점수 결과에는 벤치마크/스펙 근거 구분과 coverage가 함께 기록됨
- 생성기의 작업부하 정책(측정값이 아닌 선택 규칙): CUDA/NVENC 생태계 작업(영상·개발+GPU, RT 요청 게이밍)은 NVIDIA 후보 우선, 크리에이터·개발+GPU는 VRAM 12GB 이상 우선, 게이밍은 싱글코어·L3 캐시 중심 가중치, RAM은 요청 용량을 채우는 최소 킷 수량(2×16GB 킷이면 수량 1)+듀얼채널·플랫폼 프로파일(AM=EXPO/LGA=XMP) 우선, 지속부하 프로필·고발열 CPU는 25% 냉각 여유 쿨러 우선, CPU 스톡 쿨러 포함 저발열 부품은 사제 쿨러 생략 가능, "PC Supporter" 브랜드 스펙-기준 부품은 실구매 제품이 있을 때 후보에서 제외, 장식성 unknown(케이스 팬/RGB 헤더·메모리 프로파일 지원표 공백)은 상태를 내리지 않음
- 페르소나 기반 견적 품질 감사는 `scripts/persona-quote-audit.mts`로 재현함 — 7개 페르소나 요청으로 생성 견적 전체 라인·예산·경고를 마크다운으로 출력
- 온보딩으로 생성된 견적 화면은 부품명·수량·가격·핵심 사양과 총액·예산 관계·확인 필요 경고만 표시함. 선택 이유·판정 근거·분석 점수·게임 FPS 근거 패널은 payload에는 유지하되 사용자 견적 표면에는 렌더링하지 않으며, 비교해서 고르기·예산 구간 비교 표면의 기준 비교는 별도 도구로 유지함
